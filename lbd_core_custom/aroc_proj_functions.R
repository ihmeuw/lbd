...
make_aroc <- function(ind_gp, ind, rd,
  ...
  if('admin' %in% type){
    ...
    for(aa in 0:2){ ## for each admin type
      ...
      #if want an aroc for a baseline year not equal to first modeling year, delete years prior to baseline year from this data table
      if (alt_baseline_year){
        cell_pred <- filter(cell_pred, year >= baseline_year)
      }
...
############# Version for tridalys paper that allows specification of outdir #####################################

make_dalys_aroc <- function(ind_gp, ind, rd,
                      matrix_pred_name = NULL,
                      type,
                      measure="mortality",
                      skip_cols = NULL,
    ...
    if (grepl('admin', pred_type, fixed=TRUE)) {
      ...
      for(aa in 0:2) { ## for each admin type
        ...
        if (abs_rel == "relative") {
          ...
          ## grab baseline year preds
          baseline_year_draws <- cell_pred[which(cell_pred[, 1] == baseline_year), ] ## indexed by actual year
          baseline_year_draws <- baseline_year_draws[, -(1:2)]
