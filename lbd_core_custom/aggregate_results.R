...
if(raked){
  ...
  filename_rdata <- paste0(share_dir, indicator, "_", ifelse(raked, "raked", "unraked"), "_", "cell_draws_eb_bin0_", reg, "_", holdout,'_', measure, '.RData')
  if (file.exists(filename_rdata)) cell_pred<-readRDS(filename_rdata)
...
save(admin_0,admin_1,admin_2,sp_hierarchy_list,
     file=paste0(share_dir, indicator, "_", measure, "_", ifelse(raked, "raked_", "unraked_"), "admin_draws_eb_bin", age, "_", reg, "_", holdout, ".RData"))

# Finish up
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# Write file to signal that this region is done
write(NULL, file = paste0(share_dir, "/fin_agg_", reg, "_", holdout, "_", age, "_", raked, "_", measure))