...
#load packages for custom xgboost optimization
personal_lib <- '<<<< FILEPATH REDACTED >>>>'
if(!dir.exists(personal_lib)) dir.create(personal_lib, recursive = TRUE)
Sys.setenv(R_LIBS_USER = personal_lib)
.libPaths(c(Sys.getenv("R_LIBS_USER"), .libPaths()) )
p_load(caretEnsemble, ccaPP, fasterize, fst, mgsub, randtoolbox)
...
if(as.logical(skiptoinla) == FALSE){
  ...
  if(as.logical(use_stacking_covs)){
    ...
    source('<<<< FILEPATH REDACTED >>>>/lbd_core_custom/run_child_stackers.R')
  ...
  if(xg_second_best %>% as.logical & grepl('xgboost', all_fixed_effects)) all_fixed_effects <- paste(all_fixed_effects, 'xgboost2', sep = " + ")
...
if ('gbm' %in% child_model_names){
  if (exists("gbm_bounded_0_1")) {
    if (as.logical(gbm_bounded_0_1) == T) {
      ...
      message("Truncating GBM values = 0 to 0.0000001")
      values(cov_list[["gbm"]])[values(cov_list[["gbm"]]) == 0 & !is.na(values(cov_list[["gbm"]]))] <- 0.0000001
      gbm_cols <- grep(paste0("(gbm)(.*_pred)"), names(df), value=T)
      replace_one <- function(x) {
        x[x == 0 & !is.na(x)] <- 0.0000001
        return(x)
      }
      df[, (gbm_cols) := lapply(.SD, replace_one), .SDcols = gbm_cols]
    }
...
source('<<<< FILEPATH REDACTED >>>>/mbg_central/mbg_functions.R')
source('<<<< FILEPATH REDACTED >>>>/lbd_core_custom/mbg_fx.R')
...
if (drop_xgboost2 == TRUE){
  child_model_names <- child_model_names[1:3]
  all_fixed_effects <- "gam + xgboost + enet"
  message('dropping second xgboost from INLA')
}
...
mbg_formula <- build_mbg_formula_with_priors(fixed_effects               = all_fixed_effects,
                                             ...
                                             use_nid_res                 = use_nid_res) ##### NEW THING
## Are nid res on?
if (use_nid_res == TRUE){
  message('Using NID random effects!')
}

# Is space only gp on?
if (use_space_only_gp == TRUE){
  message('Adding a space-only GP!')
}
...
input_data <- build_mbg_data_stack(df              = df, # note that merge (if using TMB) will return data in a different (but internally consistent) order, just different than df
                                   ...
                                   zcol            = zcol,
                                   use_nid_res = use_nid_res)   # must not be null if z_list is present
...
if(!as.logical(skipinla)) {
  if(fit_with_tmb == FALSE) {
    ...
    model_fit <- fit_mbg(indicator_family = indicator_family,
                         ...
                         intstrat         = intstrat,
                         fe_sd_prior      = 1 / 9,
                         verbose = TRUE) ## this actually sets precision!. prec=1/9 -> sd=3
  }
  print("-------------------------------------")
  print("INLA FIT TIME:")
  print(Sys.time() - inla_time); rm(inla_time)
  print("-------------------------------------")
