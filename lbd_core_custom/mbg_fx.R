...
predict_mbg <- function(res_fit, cs_df, mesh_s, mesh_t, cov_list,
  ...
  if(coefs.sum1){
    ...
    l_idx <- sapply(res_fit$names.fixed, grep, par_names)
...
build_mbg_formula_with_priors <- function(fixed_effects,
  ...
  f_res <- as.formula(paste0('~f(CTRY.ID, model = "iid", hyper = list(theta=', ctry_re_prior, '), constr = TRUE)'))
  ...
  if(use_nid_res == TRUE) f_mbg <- f_mbg + f_nid_res
