...
load(file = paste0(<<<< FILEPATH REDACTED >>>>))
...
save(cell_pred, file = (paste0(<<<< FILEPATH REDACTED >>>>)), compress = TRUE)
...
for (i in 1:length(outputs)) {
  if (!is.null(outputs[[i]])) {
    ...
    writeRaster(median_ras, file = (paste0(<<<< FILEPATH REDACTED >>>>)), overwrite = TRUE, format = "GTiff")
  }
}
...
writeRaster(upper_95_raster, file = paste0(<<<< FILEPATH REDACTED >>>>), format='GTiff', overwrite = TRUE)
...
writeRaster(lower_95_raster, file = paste0(<<<< FILEPATH REDACTED >>>>), format='GTiff', overwrite = TRUE)

message("Starting range raster summary")
range_raster <- upper_95_raster - lower_95_raster

message("Saving range raster summary")
writeRaster(range_raster, file = paste0(<<<< FILEPATH REDACTED >>>>), format='GTiff', overwrite = TRUE)
