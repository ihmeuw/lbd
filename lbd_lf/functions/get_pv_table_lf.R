...
get_pv_table_lf <- function(d,
  ...
  if (plot == TRUE) {
    ...
    for (i in 1:nrow(plot_table)) {
      ...
      if (agg_on == "Master_UID") agg_title <- "Point"
      ...
      if (length(unique(d$country)) > 1) {
        ...
        plot_filename <- paste0(
          indicator, "_validation_plot_", unique(d$country), "_",
          paste(c(
            as.character(agg_on),
            setdiff(result_agg_over, "oos")
          ),
      ...
      if (page_per_year) {
        pdf(file = paste0(out.dir, plot_filename, ".pdf"), width = 12, height = 12)
      } else {
        png(paste0(out.dir, plot_filename, ".png"), width = 12, height = 12, units = "in", res = 350)
      }

      # Subset data
      fdata <- res[oos == oosindic, ]
      fdata <- merge(fdata, d[, .(Master_UID, country, data_collect_method)], by = "Master_UID")
      if (!is.na(pb_val)) {
      ...
      if (plot_ci) {
        fdata[, upper := apply(.SD, 1, quantile, p = 0.01 * (plot_ci_level + (100 - plot_ci_level) / 2), na.rm = TRUE), .SDcols = paste0("draw", 1:draws)]
        fdata[, lower := apply(.SD, 1, quantile, p = 0.01 * ((100 - plot_ci_level) / 2), na.rm = TRUE), .SDcols = paste0("draw", 1:draws)]
        limits <- fdata[, range(c(p, mean_draw, lower, upper))]
      } else {
        limits <- fdata[, range(c(p, mean_draw))]
      }

      if (page_per_year == F) { # The plot code itself
        if (plot_details) {
          gg <- ggplot(fdata, aes(x = p, y = mean_draw, size = weight)) +
            geom_abline(intercept = 0, slope = 1, color = "red") +
            geom_point(aes(color = country, shape = data_collect_method), alpha = .75) +
            scale_size_area() +
            ggplot2::xlim(limits) +
            ggplot2::ylim(limits) +
            coord_equal() +
          ...
          if (length(unique(d$country)) > 1) {
            gg <- ggplot(fdata, aes(x = p, y = mean_draw)) +
              geom_abline(intercept = 0, slope = 1, color = "red") +
              geom_point(aes(color = country), alpha = .75) +
              scale_size_area() +
              ggplot2::xlim(limits) +
              ggplot2::ylim(limits) +
              coord_equal() +
        ...
        for (y in sort(unique(fdata$year))) {
          gg <- ggplot(fdata[year == y], aes(x = p, y = mean_draw, size = weight)) +
            geom_abline(intercept = 0, slope = 1, color = "red") +
            geom_point(aes(color = country, shape = data_collect_method), alpha = .75) +
            scale_size_area() +
            ggplot2::xlim(limits) +
