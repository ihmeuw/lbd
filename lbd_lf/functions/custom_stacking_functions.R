...
fit_gam_child_model = function(df, model_name = 'gam', fold_id_col = 'fold_id', covariates = all_fixed_effects,
  ...
  for(fff in folds){
    baby_model = fit_gam(df = df[get(fold_id_col) != fff & !is.na(get(indicator)),],
                         covariates=covariates,
                         additional_terms = additional_terms,
                         bam = bam,
                         weight_column = weight_column,
...
#adapted from BRT covs function-- took away the year specific bit. GBMs==BRTs
...
fit_gbm_child_model = function(df, model_name = 'gbm', fold_id_col = 'fold_id', covariates = all_fixed_effects, additional_terms = NULL, weight_column = NULL,
  ...
  baby_models = mclapply(folds, function(fff)
    fit_gbm(df = df[get(fold_id_col) != fff & !is.na(get(indicator)),],
            covariates = covariates,
            additional_terms   = additional_terms,
            weight_column      = weight_column,
            tc                 = tc,
  ...
  full_model = fit_gbm(df = df[!is.na(get(indicator))],
                       covariates = covariates,
                       additional_terms   = additional_terms,
                       weight_column      = weight_column,
                       tc                 = tc,
...
fit_xgboost_child_model <- function(df,
  ...
  if (xg_model_tune == T){
    ...
    saveRDS(xg_fit, paste0(<<<< FILEPATH REDACTED >>>>))
    ...
    write.csv(xg_fit$bestTune, paste0(<<<< FILEPATH REDACTED >>>>))
  }
  
  if (xg_model_tune == T) {
    # Extract best parameters
    xg_best_tune <- read.csv(paste0(<<<< FILEPATH REDACTED >>>>))
  } else {
  ...
  cov_plot <-
    ...
    theme_bw()
  ggsave(filename = paste0(<<<< FILEPATH REDACTED >>>>),
         plot = cov_plot)
...
fit_rf_child_model = function(df,model_name = 'rf', fold_id_col = 'fold_id', covariates = all_fixed_effects, additional_terms = NULL, ntree = 1000, indicator = indicator, indicator_family = indicator_family, cores = 'auto'){
  ...
  full_model = fit_rf(df[!is.na(get(indicator))],
                      covariates = covariates,
                      additional_terms = additional_terms,
                      ntree = ntree,
                      indicator = indicator,
  ...
  baby_models = mclapply(folds, function(fff) fit_rf(df[get(fold_id_col) != fff & !is.na(get(indicator)),],
                                                     covariates = covariates,
                                                     additional_terms = additional_terms,
                                                     ntree = ntree,
                                                     indicator = indicator,
...
fit_earth_child_model = function(df, model_name = 'earth', fold_id_col = 'fold_id', covariates = all_fixed_effects,
  ...
  full_model = fit_earth(df = df[!is.na(get(indicator))],
                         covariates = covariates,
                         additional_terms = additional_terms,
                         weight_column = weight_column,
                         indicator = indicator,
  ...
  # Determine appropriate number of cores to use in `mclapply()`
  if(cores == 'auto') cores <- get_max_forked_threads(nobjs = length(folds))
  baby_models = mclapply(folds, function(fff) fit_earth(df = df[get(fold_id_col) != fff & !is.na(get(indicator)),],
                                                        covariates = covariates,
                                                        additional_terms = additional_terms,
                                                        weight_column = weight_column,
                                                        indicator = indicator,
