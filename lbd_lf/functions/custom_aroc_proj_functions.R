## #######################################################################
## take cell pred type objects and make analagous (matrix where
...
get_cell_pred_for_aroc <- function(ind_gp,
  ...
  rds_file <- sprintf(<<<< FILEPATH REDACTED >>>>)
  ...
  if (parallel_pred_agg) {
    ...
    cell_pred_all <- copy(cell_pred)
    # cell_pred <- matrix(nrow)
    for (i in 2:length(year_list)) {
      print(i)
      load(<<<< FILEPATH REDACTED >>>>)
      cell_pred_all <- rbind(cell_pred_all, cell_pred)
      rm(cell_pred)
    }
    
    cell_pred <- cell_pred_all
  }
...
make_aroc <- function(ind_gp, ind, rd,
  ...
  if('admin' %in% type){
    ...
    file_1 <- sprintf(<<<< FILEPATH REDACTED >>>>)
    file_2 <- paste0(<<<< FILEPATH REDACTED >>>>)
    ...
    for(aa in 0:2){ ## for each admin type
      ...
      # Redid the below line to warn if mismatches in case of non-unique admin codes
...
make_proj <- function(ind_gp, ind, rd,
  ...
  # define directories
  share_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
  aroc_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
  output_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
  ...
  if ('admin' %in% type) {
    ...
    for(aa in 0:2){ ## for each admin type
      ...
      cell_pred <- get(<<<< FILEPATH REDACTED >>>>)
      cell_pred <- as.data.table(cell_pred)
...
compare_to_target <- function(ind_gp,
  ...
  share_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
  aroc_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
  proj_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
  output_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
  ...
  for (n in 1:nrow(goal_obj)) {
    ...
    if (grepl('cell', pred_type, fixed=TRUE)) {
      ...
      for(ii in 1:length(regions)){
        ...
        proj_draws <- readRDS(sprintf(<<<< FILEPATH REDACTED >>>>))
        ...
        if (abs_rel == "absolute") {
          ...
          cell_target_raster <- insertRaster(simple_raster, matrix(absolute_goal_prob, ncol = 1))
          
          # cell_target_raster <- raster::mask(cell_target_raster, pop_raster[[1]])
          ...
          cell_target_raster <- insertRaster(simple_raster, matrix(relative_goal_prob, ncol = 1))
          writeRaster(
            cell_target_raster,
            file = paste0(<<<< FILEPATH REDACTED >>>>),
            format = "GTiff",
    ...
    if (grepl('admin', pred_type, fixed=TRUE)) {
      ...
      for(aa in 0:2) { ## for each admin type
        ...
        if (abs_rel == "absolute") {
          ...
          master_shape <- readOGR(get_admin_shapefile(admin_level = aa, version = shapefile_version))
          master_shape_merged <- merge(master_shape, as.data.frame(absolute_goal_prob), by = paste0("ADM", aa, "_CODE"))
          prob_raster <- fasterize(st_as_sf(master_shape_merged), simple_raster, field = "absolute_goal_prob")
          writeRaster(
            prob_raster,
            file = paste0(<<<< FILEPATH REDACTED >>>>),
            format = "GTiff",
        ...
        if (abs_rel == "relative") {
          ...
          master_shape <- readOGR(get_admin_shapefile(admin_level = aa, version = shapefile_version))
          master_shape_merged <- merge(master_shape, as.data.frame(relative_goal_prob), by = paste0("ADM", aa, "_CODE"))
          prob_raster <- fasterize(st_as_sf(master_shape_merged), simple_raster, field = "relative_goal_prob")
          writeRaster(
            prob_raster,
            file = paste0(<<<< FILEPATH REDACTED >>>>),
            format = "GTiff",
