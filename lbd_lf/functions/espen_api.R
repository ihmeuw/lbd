...
espenAPIData <- function(disease, level) {
  ...
  for (i in 1:nrow(iso)) {
    ...
    if (nrow(isoData) > 0 ) {
      ...
      if (file.exists(paste0(<<<< FILEPATH REDACTED >>>>)) == F) {
        dir.create(paste0(<<<< FILEPATH REDACTED >>>>))
      }
      fwrite(isoData, paste0(<<<< FILEPATH REDACTED >>>>))
  ...
  fwrite(data, paste0(<<<< FILEPATH REDACTED >>>>))
  ...
  if (outDir != "None") {
    fwrite(data, paste0(<<<< FILEPATH REDACTED >>>>))
    message(paste0("Data Saved Here : ", <<<< FILEPATH REDACTED >>>>))
  }
}
...
espenAPIMap <- function(disease, level, type, year = 2017, subtype = "nosubtype") {
  ...
  for (n in 1:nrow(iso)) {
    ...
    if (nrow(sfPull) > 0 & "value" %in% colnames(sfPull)) {
      ...
      if (file.exists(paste0(<<<< FILEPATH REDACTED >>>>)) == F) { dir.create(paste0(<<<< FILEPATH REDACTED >>>>)) }
      
      ### Saving shapefile and combining
      if (type == "endemicity") {
        st_write(sfPull, paste0(<<<< FILEPATH REDACTED >>>>))
  ...
  if (type == "endemicity") {
    st_write(data, paste0(<<<< FILEPATH REDACTED >>>>))
  }
...
pullISO <- function() {
  ...
  allData <- as.data.table(fread(paste0(<<<< FILEPATH REDACTED >>>>)))
...
pullDir <- function(disease, date = "today") {
  ...
  incDir <- paste0(<<<< FILEPATH REDACTED >>>>)
...
comparePulls <- function(disease, level, new_date, old_date
  ...
  if (type == "endemicity") {
    ...
    oldData <- fread(paste0(<<<< FILEPATH REDACTED >>>>))
    newData <- fread(paste0(<<<< FILEPATH REDACTED >>>>))
  ...
  if (type == "endemicity") {
    ...
    # Saving csv
    fwrite(diffData, paste0(<<<< FILEPATH REDACTED >>>>))
  }
}
