save_mbg_input <- function(indicator = indicator, indicator_group = indicator_group, df = df , simple_raster = simple_raster, simple_raster2 = NULL, simple_raster_raf1 = simple_raster,
                           mesh_s = mesh_s, mesh_s_ppp = mesh_s_ppp, mesh_t = mesh_t, cov_list = all_cov_layers, run_date = NULL, pathaddin="",
  ...
  ## Save all inputs
  to_save <- c("df", "simple_raster", "mesh_s", "mesh_s_ppp", "mesh_t", 'cov_list', 'child_model_names', 'all_fixed_effects','period_map')
