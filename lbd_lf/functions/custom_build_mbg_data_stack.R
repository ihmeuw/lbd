...
build_mbg_data_stack <- function(df, 
  ...
  if(tmb == TRUE){
    ...
    if(coefs.sum1){
      ...
      if (covars_in_pp) {
        mesh_locs <- mesh_s_ppp$loc
        covars <- covariates[f.e.v]
        covar_values <- copy(covars)
        
        for (i in 1:length(covars)) {
          covars[[i]] <- covars[[i]][[1]]
          covar_values[[i]] <- raster::extract(covars[[i]], mesh_locs[, 1:2])
          covar_values[[i]][is.na(covar_values[[i]])] <- logit(0.0000001) # how to deal with NA values? does the model break if we leave them as NA? currently setting them to essentially zero (in probability space)
        }
        
        A.covar.pp <- do.call(cbind, lapply(covar_values, function(x) x))
        A.covar.pp <- rbind(A.covar, A.covar.pp)
      }
    }
