...
predict_mbg <- function(res_fit, cs_df, mesh_s, mesh_s_ppp, mesh_t, cov_list,
  ...
  cell_l <- unname(as.matrix(as(data.matrix(vals), "dgeMatrix") %*% pred_l[!(rownames(pred_l) %in% c("int.pp", "int.pp.mapping", "int.pp.sssc", "int.pp.tas", "int.training")),]))
  ...
  if (return_mean_sd) {# get prediction mean (integrated probability)
    ...
    return_list <- list(mean_ras, sd_ras, cell_pred, cell_s, cell_s_no_t)
  } else {
    
    return_list <- list(NULL, NULL, "cell_pred" = cell_pred, "cell_all" = cell_all, "cell_s" = cell_s,
                        "cell_s_no_t" = cell_s_no_t, "cell_l" = cell_l, "cell_n" = cell_n,
                        "cell_ctry_res" = cell_ctry_res, "cell_subnat_res" = cell_subnat_res,
                        "cell_time_res" = cell_time_res)
  }
