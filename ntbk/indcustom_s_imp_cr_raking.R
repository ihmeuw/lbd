...
all_mod <- list()
	for (iso in regions) {
		...
		for (a1 in unique(mydat$ADM1_CODE)) {
			...
			if (a1 %in% setdiff(unique(mydat$ADM1_CODE),
				c(1105, 2105, 3105, 15105,
					20105, 26105, 29105, 31105,
					32105, 34105))) {
				if (a1 %in% c()) {
					gam_k <- 4
				} else {
	...
	write.csv(all_mod, paste0("<<<< FILEPATH REDACTED >>>>",
		indi,	'_raked.csv'), row.names = FALSE)
}
