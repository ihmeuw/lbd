...
load(paste0('/share/geospatial/mbg/', indicator_group, '/', indicator,
            '/model_image_history/pre_run_tempimage_', run_date, pathaddin,
            '.RData'))
...
if(as.logical(skiptoinla) == FALSE){
  ...
  if(as.logical(use_stacking_covs)){
    ...
    child_model_run <- run_child_stackers(models = child_model_names,
                                          input_data = the_data,
                                          indicator = indicator,
