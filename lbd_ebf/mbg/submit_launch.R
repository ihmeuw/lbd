...
if (!is.null(resub_date)) {
  ...
  models <- list(# standard model
    c('Standard model',                       'config_ebf_main',               'covs_ebf',  'old_geos'),

    # alternate covariate strategies
    c('Covariate Test: GP only',              'config_ebf_gp_only',            'covs_ebf',  'old_geos'),
    c('Covariate Test: Stack only',           'config_ebf_stack_only',         'covs_ebf',  'prod'),
    c('Covariate Test: Raw cov only',         'config_ebf_raw_only',           'covs_ebf',  'prod'),
    c('Covariate Test: Raw cov + GP',         'config_ebf_raw_and_gp',         'covs_ebf',  'old_geos'),

    # alternate 24 hour recall period
    c('Recall Period Test: 24 h only',         'config_ebf_main_24h_only',     'covs_ebf',  'new_geos')
  )
