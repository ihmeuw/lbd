###############################################################################
## Launch EBF (Exclusive Breastfeeding) Models for Africa
###############################################################################
...
for(i in 1:nrow(loopvars)){
  ...
  qsub <- make_qsub_share(age           = loopvars[i,2],
                          ...
                          indic         = indicator,
                          saveimage     = TRUE,
                          memory        = ifelse(as.character(loopvars[i,1]) == "wssa", 20, 10),
                          cores         = slots,
...
gbd <- fread(paste0(sharedir, "/ebf_est_gbd_stage1.csv"))
...
for (s in strata) {
  ...
  qsub <- make_qsub_postest(code          = "postest_script",
                            ...
                            memory        = ifelse(s == "essa-yem", 20, 10),
                            cores         = slots,
                            geo_nodes     = use_geos_nodes,
                            use_c2_nodes  = FALSE,
                            proj          = project,
...
## Check to make sure post-estimation is done before continuing
waitformodelstofinish(lv = cbind(strata, 0), sleeptime=60)

## Combine post-estimation files across regions and save needed outputs
