...
# load files
ad1 <- fread(paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_admin_1_raked_', measure, '_summary.csv'))
ad2 <- fread(paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_admin_2_raked_', measure, '_summary.csv'))
...
write.csv(ad2, paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_admin_2_raked_', measure, '_summary.csv'))
...
if (measure == 'prevalence') {
  ...
  mbg <- list(fread(paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_admin_0_unraked_summary.csv')),
              fread(paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_admin_1_unraked_summary.csv')))
  names(mbg) <- c('ad0', 'ad1')

  # Load raked estimates
  if (indicator == 'had_diarrhea') {
    mbg_raked <- list(fread(paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_admin_0_raked_prevalence_summary.csv')),
                      fread(paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_admin_1_raked_prevalence_summary.csv')))
  ...
  for (a in c('ad0', 'ad1')) {
    ...
    # save
    write.csv(mbg[[a]], paste0('<<<< FILEPATH REDACTED >>>>/', indicator, '_mbg_data_stackers_', a, '.csv' ))
  }
}
...
if (measure == 'prevalence') {
  ...
  # Combine, clean, and save input data for IS plotting
  ...
  reg <- fread('<<<< FILEPATH REDACTED >>>>/dia_region_iso.csv')
  csv_master <- merge(csv_master, reg[, c('iso3', 'dia_reg')], by.x = 'country', by.y = 'iso3')
  setnames(csv_master, 'dia_reg', 'region')
  csv_master[country == 'IND', region := 'IND']
  csv_master[country == 'MNG', region := 'MNG']
  ...
  write.csv(pvtab, file = paste0('<<<< FILEPATH REDACTED >>>>/pv_metrics.csv'), row.names=F)
