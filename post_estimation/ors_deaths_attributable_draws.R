# -----------------------------------------------------------------------------------------------------------
# Perform deaths attributable analysis at the draw level for ORS
#------------------------------------------------------------------------------------------------------------
...
# subset data table
dt <- dt[agg_level == 'ADM2' & year == 2017]
...
dt[, no_ors := 1 - ors, by = c('code', 'draw')]

# get ORS paf and deaths attributable to lack of ORS in 2017
dt[, deaths_attributable := diarrhea_deaths*get_paf(no_ors, no_ors_rr)]

# reshape wide
tic('Reshaping back to wide')
dt[, c('ors', 'had_diarrhea', 'diarrhea_deaths', 'no_ors') := NULL]
dt <- dcast(dt, ... ~ draw, value.var = c('deaths_attributable'))
toc(log = TRUE)
...
out.path <- sprintf('<<<< FILEPATH REDACTED >>>>/%s_deaths_attributable_adm2_draws_%s.rds',
                    share_dir, indicators[[1]], sensitivity_factor)

dir.create('<<<< FILEPATH REDACTED >>>>')

message('-- finished making deaths attributable across draws. now saving as \n', out.path)
...
out.path <- sprintf('<<<< FILEPATH REDACTED >>>>/%s_deaths_attributable_admin_2_summary_%s.csv',
                    share_dir, indicators[[1]], sensitivity_factor)
