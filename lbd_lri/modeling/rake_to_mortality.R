## Step 2: Rake all new cell preds to 2017 GBD values for MORTALITY

#load in required functions
source('<<<< FILEPATH REDACTED >>>>') #raking fucntions
...
gbd_lri_mort <- get_outputs(topic= 'cause',
                            cause_id=322,
                            location_id=country,
                            year_id=2000:2017,
                            age_group_id=1,
...
gbd_lri_mort$val <- ifelse(is.na(gbd_lri_mort$val), gbd_lri_mort$inter_val, gbd_lri_mort$val)
gbd_lri_mort <- gbd_lri_mort[complete.cases(gbd_lri_mort),]


#rename columns in gbd_lri_mort to be read correctly by rake_cell pred
#gaul_code column named "nqme"
#year column named "year"
#value column named "mean"
gbd_lri_mort$name <- gbd_lri_mort$location_id 
gbd_lri_mort$year <- gbd_lri_mort$year_id
gbd_lri_mort$mean <- gbd_lri_mort$val
