...
plot_stackers <- function(reg,
  ...
  if (parallel_pred_agg) {
    result_brick <- brick(<<<< FILEPATH REDACTED >>>>)
    # addLayer(s, r/2, r*2)
    for (i in 2:length(predict_years)) {
      result_brick <- addLayer(result_brick, raster(<<<< FILEPATH REDACTED >>>>))
    }
  } else {
    ...
    } else if (file.exists(paste0(sh_dir, ind, "_", reg, "_", holdout, "_unraked_mean_raster.tif"))) {
      result_brick <- brick(paste0(sh_dir, ind, "_", reg, "_", holdout, "_unraked_mean_raster.tif"))
  ...
  # load input data
  input_df <- read.csv(paste0(sh_dir, "input_data.csv"),
    stringsAsFactors = F
  ) %>%
    as.data.table()
    ...
    for (i in 1:length(predict_years)) {
      ...
      make_gg_map <- function(rbrick, title, i, color_table = col_table) {
        ...
        gg_result <- ggplot() +
          ...
          theme_void() +
          labs(title = title) +
          scale_fill_manual(values = col_vec, labels = color_table$categories, name = "Prevalence") +
          geom_path(
      ...
      gg_data <- ggplot() +
        ...
        coord_equal() +
        theme_void() +
        scale_color_manual(values = col_vec, labels = col_table$categories, name = "Prevalence") +
        geom_path(
          data = master_shape_df,
          aes(x = long, y = lat, group = group)
        ) +
      ...
      if (is.null(ctry)) {
        ...
        fn <- paste0(reg_dir, "stacker_map_", reg, "_", predict_years[i], ".png")
      } else if (!is.null(ctry)) {
        ctry_dir <- paste0(o_dir, ctry, "/")
        dir.create(ctry_dir, recursive = T, showWarnings = F)
        fn <- paste0(ctry_dir, "stacker_map_", ctry, "_", predict_years[i], ".png")
      }
