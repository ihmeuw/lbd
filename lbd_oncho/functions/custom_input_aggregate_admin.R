input_aggregate_admin <- function(indicator,
  ...
  # Correct mismatches in country names
  # countries_temp <- copy(countries)
  if ("Cote d'Ivoire" %in% countries) {
      countries[[which(countries == "Cote d'Ivoire")]] <- "Côte d'Ivoire"
  }
  if ("Congo" %in% countries) {
      countries[[which(countries == "Congo")]] <- "Republic of Congo"
  }
  ...
  input_admin[location_name == "Cote d'Ivoire", location_name := "Côte d'Ivoire"]
  input_admin[location_name == "Congo", location_name := "Republic of Congo"]
