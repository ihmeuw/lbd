check_resampling <- function(check_resamp_rd, indicator_group = "lf", indicator_poly = "had_lf_poly", shp_path = <<<< FILEPATH REDACTED >>>>,
                             ignore_warnings = FALSE, return_list = FALSE) {
  ...
  ### testing ####################################################
  if (FALSE) {
    check_resamp_rd <- "2019_04_08_14_32_59"
    indicator_group <- "lf"
    indicator_poly <- "had_lf_poly"
  ...
  require(snow)
  require(doSNOW)
  resamp_input <- <<<< FILEPATH REDACTED >>>>
  resamp_output <- <<<< FILEPATH REDACTED >>>>
