...
build_mbg_formula_with_priors <- function(fixed_effects,
  ...
  if (!is.null(rw1_raw_covar_list)) {
    f.e.v <- f.e.v[which(!(f.e.v %in% rw1_raw_covar_list))]
    f_rw1_raw_covar <- list()
    for (c in rw1_raw_covar_list) {
      if (!(c %in% rw_covariate_n_groups_config$covariate)) {
          rw_covariate_n_groups_config <- rbind(rw_covariate_n_groups_config, data.table("covariate" = c, "rw_groups" = rw_covariate_n_groups))
      }
      
      if (is.null(rw_covariate_n_groups_config[covariate == c, rw_groups])) {
        rw_covariate_n_groups_config[covariate == c, rw_groups] <- rw_covariate_n_groups
      }
      
      f_rw1_raw_covar <- c(f_rw1_raw_covar, as.formula(paste0('~f(inla.group(', c, ', n = ', rw_covariate_n_groups_config[covariate == c, rw_groups], '), model = "', rw_covariate_model, '", scale.model = TRUE, hyper = list(theta = list(prior="pc.prec", param=c(0.5, 0.01))))')))
    }
  } else {
    f_rw1_raw_covar <- NULL
  }
  ...
  if (!nullmodel) {
    if (coefs.sum1) {
      ...
      if (use_rw1_crosswalk_covariates) {
        f_lin <- character()
        for (i in 1:(length(f.e.v) - 1)) {
          if (!(f.e.v[[i]] %in% c("allmda", "lfmda"))) { # Don't add additional terms for allmda and lfmda
            f_lin <- paste0(f_lin, "f(inla.group(", f.e.v[[i]], ", n = 10), model = 'rw1', scale.model = TRUE) + ")
          }
        }
        f_lin <- paste0(f_lin, "f(inla.group(", f.e.v[[length(f.e.v)]], ", n = 10), model = 'rw1', scale.model = TRUE)")
        f_lin <- reformulate(f_lin)
      } else {
        fixed_effects_temp <- strsplit(fixed_effects,' \\+ ')[[1]]
        fixed_effects_temp <- fixed_effects_temp[which(!(fixed_effects_temp %in% rw1_raw_covar_list))]
        if (length(fixed_effects_temp) > 0) {
          f_lin <- reformulate(fixed_effects_temp)
        } else {
          f_lin <- NULL
        }
      }
  ...
  if (!is.null(f_rw1_raw_covar)) {
    for(i in 1:length(f_rw1_raw_covar)) {
      f_mbg <- f_mbg + f_rw1_raw_covar[[i]]
    }
  }
