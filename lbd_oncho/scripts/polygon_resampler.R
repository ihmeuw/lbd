...
if (pop_ras == "default") {
  path.to.pop.rast <- <<<< FILEPATH REDACTED >>>>
  p_ras <- NULL
} else {
...
write.csv(df_poly_data, file = <<<< FILEPATH REDACTED >>>>)
...
if (exists("other_weight")) {
  if (other_weight != "") {
    # mbg_ready <- mbg_ready[, c("nid", "Master_UID", "original_year", "data_collect_method", "country", "source", "cluster_id", "point", "latitude", "longitude", "N", new_indic, "weight", "year", "diagnostic", "shapefile", "location_code", "age_start", "age_end", "age_midpoint", "cohort_id", other_weight), with = FALSE]
  ...
  # mbg_ready <- mbg_ready[, c("nid", "Master_UID", "original_year", "data_collect_method", "country", "source", "cluster_id", "point", "latitude", "longitude", "N", new_indic, "weight", "year", "diagnostic", "shapefile", "location_code", "age_start", "age_end", "age_midpoint", "cohort_id"), with = FALSE]
...
if (use_share == FALSE) {
  if (file.exists(<<<< FILEPATH REDACTED >>>>)) {
    ...
    write.csv(mbg_ready, file = <<<< FILEPATH REDACTED >>>>)
  } else {
    write.csv(mbg_ready, file = <<<< FILEPATH REDACTED >>>>)
  }
} else {
