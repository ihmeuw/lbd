#####################################################################
## Parallel script for running diarrhea MBG models                 ##
#####################################################################
...
if(as.logical(skiptoinla) == FALSE){
  ...
  ## shuffle the data into folds indicated in config
  ...
  file.copy(from = paste0('<<<< FILEPATH REDACTED >>>>/', skiptoinla_from_rundate, pathaddin, '.RData'),
            to = paste0('<<<< FILEPATH REDACTED >>>>/', run_date, pathaddin, '.RData'))
...
if(!as.logical(skipinla)) {
  ...
  ## skipped fitting INLA so load model and move to predict
...
# set measures
measures <- c('deaths', 'prevalence', 'incidence')
