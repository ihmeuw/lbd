###############################################################################
## MBG parallel model launch script for diarrhea
###############################################################################
...
config <- set_up_config(repo            = core_repo,
                        indicator_group = '',
                        indicator       = '',
                        config_name     = paste0('had_diarrhea/config_', config_par),
                        covs_name       = paste0('had_diarrhea/', cov_par))
...
## Set BRT optimizer file/parameters based on whether this is a test or full model
if (stacked_fixed_effects %like% 'gbm') {

  gbm_params <- fread(paste0(core_repo, '/had_diarrhea/gbm_params_dia_', cov_version, '.csv'), stringsAsFactors = F)
...
for(i in 1:nrow(loopvars)){
  ...
  region_cores <- 8
  if(r == 'dia_central_asia') region_cores <- 4
  if(r == 'dia_se_asia' | r == 'dia_sssa' | r == 'MNG') region_cores <- 6
  if(r == 'dia_malay' | r == 'dia_name' | r == 'dia_afr_horn') region_cores <- 10
  if(r == 'dia_wssa' | r =='dia_south_asia') region_cores <- 12
  ...
  if (region_cores > 9 | r == 'dia_malay') region_rt <- '16:00:00:00'
