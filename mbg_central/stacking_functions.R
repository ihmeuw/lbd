...
extract_covariates = function(df, covariate_list, id_col = NULL, reconcile_timevarying = T, period_var = NULL, return_only_results = F, centre_scale = F, period_map = NULL){
  #Load libraryd packages. Should be redundant
  ...
  #make this less likey to cause an error
...
fit_gam = function(df, covariates = all_fixed_effects, additional_terms = NULL, weight_column = NULL, bam = F, spline_args = list(), auto_model_select =F, indicator, indicator_family = 'binomial', cores = 1){
  ...
  df = copy(df) #in case data table scoping causes issues
  ...
  covariates = format_covariates(covariates) #additional terms handled below
...
fit_gbm= function(df, covariates = all_fixed_effects, additional_terms = NULL, weight_column = NULL, tc = 4, lr = .005, bf = .75, indicator, indicator_family = 'binomial',
                  empirical_logit = F, plot.main = F){

  # Ensure some logit functions available
  logit <- function(x) {
    log(x/(1-x))
  }
  invlogit <- function(x) {
  ...
  if ((indicator_family %in% c('binomial','poisson')) & empirical_logit == F)  {
  ...
  } else if ((indicator_family %in% c('binomial')) & empirical_logit == T) {
    #set up empirical logit outcome structure for binomial and poisson data
    indicator_family <- 'gaussian'
    offset <- NULL
    df[, pre_logit := get(indicator)]
    df[, paste0(indicator) := pre_logit/N]
    df[, paste0(indicator) := logit(get(indicator))]
    # GBM step doesn't handle infinity values well, so set by hand
    # Use epsilon of 1/2 of the smallest distance between a non-zero-or-one value and 0 or 1
    epsilon_low <- min(df[pre_logit/N != 0, pre_logit/N])
    epsilon_high <- 1-max(df[pre_logit/N != 1, pre_logit/N])
    epsilon <- min(epsilon_low, epsilon_high) / 2
    df[get(indicator) == Inf, paste0(indicator) := logit(1-epsilon)]
    df[get(indicator) == -Inf, paste0(indicator) := logit(epsilon)]
  } else {
    offset <- NULL
  }
  ...
  message(paste('Fitting GBM/BRT with tc:',tc,'lr:',lr,'bf:',bf))
    ...
    # throw a try-catch so if some years work it at least will return that, if it doesnt it will try different things (like changing the learning rate. )
    mod <- try(
...
fit_gbm_child_model = function(df, model_name = 'gbm', fold_id_col = 'fold_id', covariates = all_fixed_effects, additional_terms = NULL, weight_column = NULL,
                                tc = 4, lr = 0.005, bf = 0.75, indicator = indicator, indicator_family = indicator_family, cores = 1, empirical_logit = F){

#######The function#######
  library(parallel)

  #prevent df scoping problems
  df = copy(df)
  ...
  baby_models = mclapply(folds, function(fff)
            ...
            lr                 = lr,
            bf                 = bf,
            empirical_logit    = empirical_logit,
            indicator          = indicator,
            indicator_family   = indicator_family,
  ...
  # If empirical logit, convert back to counts in untransformed space
  if (empirical_logit == T) {
    df$gbm_cv_pred <- ilogit(df$gbm_cv_pred)*df$N
    df$gbm_full_pred <- ilogit(df$gbm_full_pred)*df$N
  }
  #print(return_cols)
  #set up with for the return call
  return(setNames(list(df[,return_cols,with = F], full_model),c('dataset',paste0(model_name))))
...
make_stack_rasters = function(covariate_layers = all_cov_layers, #raster layers and bricks
  ...
  save(child_models, file=sprintf('<<<< FILE PATH REDACTED >>>>/%s/%s/output/%s/child_model_list_%s_%i.RData',
                                  ind_gp, ind, rd, re, ho))
...
#indicator_family: model family. Specifies what sorts of transformations need to be done
...
predict_model_raster = function(model_call, covariate_layers, constants = NULL, indicator_family = 'binomial', centre_scale_df = NULL){
  ...
  if(inherits(model_call, 'gam') | inherits(model_call, 'bam')){
    ...
    #glmnet wants a matrix for new data
...
gam_stacker = function(df, #the dataset in data table
  ...
  #copy dataset to avoid data table scoping issues
  df = copy(df)
...
fit_earth = function(df, covariates = all_fixed_effects, additional_terms = NULL, weight_column = NULL, indicator, indicator_family = 'binomial'){
  ...
  df = copy(df) #in case data table scoping causes problems
