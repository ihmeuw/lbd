...
graph_data_coverage_values <- function(df,
  ...
  ## Define root
  assign("j_root", "<<<< FILEPATH REDACTED >>>>", envir = .GlobalEnv)

  if (Sys.info()["sysname"] != "Linux") stop("This function should be run on the cluster only.")

  # Load packages (custom lib loc depending on which type of node is being used)
  message("\nLoading packages...")
  ...
  if (!("master_shape_all" %in% ls())) {
    ...
    message("\nOpening master shapefile...")
    assign("master_shape_all",
            readOGR(dsn = paste0("<<<< FILEPATH REDACTED >>>>/template_shp"),
  ...
  if (is.null(out_dir)) out_dir <- paste0('<<<< FILEPATH REDACTED >>>>/data_coverage_plots/', indicator, extra_file_tag, '/')
  if(save_on_share==TRUE) out_dir <- paste0('<<<< FILEPATH REDACTED >>>>>/data_coverage_plots/', indicator, extra_file_tag, '/')
...
pull_polys <- function(shape_loc, fast_shapefiles) {
  ...
  if (fast_shapefiles == T) {
    ...
    master_shape <- readOGR(dsn = paste0("<<<< FILEPATH REDACTED >>>>"),
                            layer = shape)
  }
...
check_for_missing_shapefiles <- function(df_graph_poly) {
  ...
  real_shapefiles <- gsub('.shp',
                          '',
                          list.files(paste0("<<<< FILEPATH REDACTED >>>>/Shapefile directory"),
                                     pattern = '.shp'))
