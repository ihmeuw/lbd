...
load_gbd_data     <- function(gbd_type,
  ...
  gaul_to_loc_id <- fread("<<<< FILE PATH REDACTED >>>>/gaul_to_loc_id.csv")
  names(gaul_to_loc_id)[names(gaul_to_loc_id)=="loc_id"] <- "location_id"

  if(gbd_type=="covariate") {

    library('dbplyr', lib.loc = '<<<< FILE PATH REDACTED >>>>/mbg_pkgs_conda')
    metadata <- get_covariate_metadata()

    source('<<<< FILE PATH REDACTED >>>>/get_covariate_estimates.R')
    ...
    if(collapse_age_sex==TRUE & length(unique(gbd_estimates[, age_group_id]))!=1) {
      ...
      source('<<<< FILE PATH REDACTED >>>>/get_population.R')
  ...
  if(gbd_type=="output") {
    ...
    source('<<<< FILE PATH REDACTED >>>>/get_outputs.R')
...
get_population_data     <- function(simple_raster){
  ...
  pop<- brick('<<<< FILE PATH REDACTED >>>>/pop_stack.tif')
...
load_admin_raster  <- function(admin_level, simple_raster, disag_fact=NULL){
  ...
  if(admin_level %in% c(0,1)) shapes <- shapefile(paste0("<<<< FILE PATH REDACTED >>>>GAUL_admin/admin", admin_level, "/g2015_2014_", admin_level, "/g2015_2014_", admin_level, ".shp"))
  if(admin_level == 2) shapes <- shapefile(paste0("<<<< FILE PATH REDACTED >>>>GAUL_admin/admin", admin_level, "/g2015_2014_", admin_level, "/g2015_2014_", admin_level, "_modified.shp"))
...
summarize_admins <- function(ind = indicator,
  ...
  sharedir       <- sprintf('<<<< FILE PATH REDACTED >>>>/%s/%s',ig,ind)
...
save_post_est   <- function(x,
  ...
  output_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indic, '/output/', run_date)
...
load_cell_preds <- function(indicator_group,
  ...
  if(u5m){
    if(ageasindic==FALSE){
      load(paste0('<<<< FILE PATH REDACTED >>>>/',indicator_group,'/',indicator,'/output/',rd,'/',
                  indicator,'_cell_draws_eb_bin',agebin,'_',region,'_0',other,'NA.RData')) # the 0 are no holdout
    } else {
      load(paste0('<<<< FILE PATH REDACTED >>>>/',indicator_group,'/',indicator,'_age',agebin,'/output/',rd,'/',
...
#################################################################################
### Combines U5M Fractions
## Inputs:
## Outputs:
#################################################################################

combine_agebins <- function(age_bins=1:4,
                            reg,
                            u5m=FALSE,
                            other='',
                            ageasindic=TRUE,
                            fractions = list(neonatal = 1,
                                             #infant = 1:2,
                                             child = 1:4)){

  # initialize some values I need
  lengths <- c(1, 11, 24, 24)
  ...
  for (bin in age_bins) {
    ...
    ## object name
    name <- sprintf('pred_all_bin%i',bin)

    ## load the object (named cell_preds)
    assign(name,load_cell_preds(indicator_group=indicator_group,
                                indicator=indicator,
                                rd = run_date,
                                region=reg,
                                agebin=bin,
    ...
    #message(paste0("Dimensions are: ", dim(get(name))[1], " by ", dim(get(name))[2]))
  }


  message('\nCalculating age bin survivals for:')
  for (bin in age_bins) {

    message(sprintf('age bin %i', bin))

    ## names to convert from/to
    name_from <-  sprintf('pred_all_bin%i',bin)
    name_to   <-  sprintf('surv_all_bin%i',bin)

    ## switch to survival and apply number of months to get whole-window survival
    assign(name_to, (1 - get(name_from)) ^ lengths[bin])
  }


  message('\nConstructing mortality probability samples for fractions:')
  ...
  for (f in 1:length(fractions)) {
    ...
    if (n_bins > 1) {
      for (j in 2:n_bins) {
        x <- x * get(names_from[j])
      }
    }

    ## convert back to mortality probability and assign to the object
    assign(name_to, 1 - x)
  }


  return(list(neonatal_all=neonatal_all,
              #infant_all=infant_all,
              child_all=child_all))
...
load_u5m_gbd <-function(gaul_list,
  ...
  gbd <- fread("<<<< FILE PATH REDACTED >>>>/as_withshock.csv"))

  gbd <- rename(gbd, c(year_id="year",
                       sex_id ="sex",
                       age_group_id='age',
                       ihme_loc_id='iso3'))
  ...
  if(exact==TRUE){
    ...
    # CIs are just an approx this way, but not using for analysis just plots so ok
  ...
  #gbd$name[gbd$name==227]=1013965 # ZAF
...
fit_stats <- function(is_data,
  ...
  ### Fit statistics
  model_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, '/output/', run_date)
  image_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, '/model_image_history')
  ...
  fit_folder <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, '/output/', run_date, '/fit_stats')
...
calc_logit_raking_factors <- function(pop_wts_object = pop_wts_adm0,
                                      cell_pred = cell_pred,
                                      admin_level = 0,
                                      years     = year_list,
  ...
  cell_pred[, country_year := adm_code_all]
  cell_pred[, weights := pop_wt]

  # Subset to good cells only
  cell_pred <- cell_pred[good_cells,]

  # Set up raking  
  rake_to = rake_to[,country_year:= paste0(name,'_',year)]

  # # Apply logit raking function
 
  get_raking_factor <- function(x) {
   message(paste0("\n", x)) # For debugging
   output <- try(NewFindK(gbdval = rake_to[country_year == x, mean],
                          pixelval = subset(cell_pred, 
                                     country_year == x, 
                                     select = names(cell_pred)[grepl("V[0-9]+", names(cell_pred))]),
                          weightval = cell_pred[country_year ==x, weights],
                          MaxJump = max_jump, NumIter = num_iter, FunTol = fun_tol,
                          country_year = x))
   if (is.null(output)) {
    message(paste0("Your GBD and MBG estimates are REALLY far apart for country_year ", x))
    ...
    output <- try(NewFindK(gbdval = rake_to[country_year == x, mean],
                           pixelval = subset(cell_pred, 
                                     ...
                                     select = names(cell_pred)[grepl("V[0-9]+", names(cell_pred))]),
                           weightval = cell_pred[country_year ==x, weights],
                           MaxJump = max_jump*10, NumIter = num_iter*10, FunTol = fun_tol,
                           country_year = x))
   }
   if (is.null(output)) {
    stop(paste0("Your GBD and MBG estimates are WAY too far away for country_year ", x, " - investigate this."))
   }
   return(data.table(raking_factor = output, country_year = x))
  }

  start_time <- proc.time()
  rf <- mclapply(unique(cell_pred[,country_year]), get_raking_factor, mc.cores = cores_to_use)

  end_time <- proc.time()

  rf <- rbindlist(rf)
 
  # Catch any missing country years by trying mclapply with fewer cores (not parallel)
  missing_country_years <- unique(cell_pred$country_year)[!(unique(cell_pred$country_year) %in% unique(rf$country_year))]
  
  if (length(missing_country_years) > 0) {
    message(paste0("mclapply resulted in ", length(missing_country_years), " broken country_years. Trying with fewer cores..."))
    additional_rfs <- mclapply(unique(cell_pred[,country_year]), get_raking_factor, mc.cores = floor(cores_to_use/2))
    additional_rfs <- rbindlist(rf)
    rf <- rbind(rf, additional_rfs, use.names = T)
  }

  # Catch any missing country years with a slower loop (not parallel)
  missing_country_years <- unique(cell_pred$country_year)[!(unique(cell_pred$country_year) %in% unique(rf$country_year))]
  
  if (length(missing_country_years) > 0) {
    message(paste0("mclapply #2 completed but still ", length(missing_country_years), " broken country_years. Trying serially..."))
    additional_rfs <- lapply(unique(cell_pred[,country_year]), get_raking_factor)
    additional_rfs <- rbindlist(rf)
    rf <- rbind(rf, additional_rfs, use.names = T)
  }
  ...
  missing_country_years <- unique(cell_pred$country_year)[!(unique(cell_pred$country_year) %in% unique(rf$country_year))]
  if (length(missing_country_years) > 0) {
    warning(paste0("Unable to calculate raking factors for ", length(missing_country_years), " country-years"))
    message(paste0("No raking factor calculated for the following country-years:", paste(missing_country_years, collapse = ",")))
  }

  rf = rf[, c('name', 'year') := tstrsplit(country_year, "_")]
  return(rf)
}

NewFindK <- function(gbdval, pixelval, weightval, MaxJump = 10, NumIter = 10, FunTol = 1e-5, country_year = NULL){
  # message(paste0("\n",country_year)) # For debugging
  ...
  if (F_a * F_b > 0){
    warning(paste0("Your estimates are WAY too far away from GBD for country_year ", country_year))
    return(NULL)
  } else {
    i <- 1
    c <- (a + b) / 2
    ...
    while (!Success & i < NumIter){
      ...
      F_c <- EvalDiff(gbdval, NewPixelVal(c, pixelval), weightval)
      Success <- (abs(F_c) <= FunTol)
      # message(paste0(i, ": ", F_c), " | c = ", c) #for debugging
      i <- i + 1
    }
    if (Success){
      return(c)
...
NewEst <- function(vals, weightval){
  # Column-wise since that's how it's done for aggregation
  col_means <- vals[ , lapply(.SD, weighted.mean, w=weightval, na.rm = T), .SDcols = names(vals)]
  return(mean(as.numeric(col_means)))
}
...
save_best_model <- function(indicator_group, indicator, run_date, measure) {
  ...
  raked <- brick(paste0("<<<< FILE PATH REDACTED >>>>/", indicator_group, "/", indicator, "/output/", run_date, "/", indicator, "_mean_raked_raster.tif"))
  unraked <- brick(paste0("<<<< FILE PATH REDACTED >>>>/", indicator_group, "/", indicator, "/output/", run_date, "/", indicator, "_mean_raster.tif"))
  ...
  root <- '<<<< FILE PATH REDACTED >>>>'
  central_cov_dir <- '<<<< FILE PATH REDACTED >>>>' # central folder
  ...
  for(i in 1:length(names(unraked))) {
    ...
    writeRaster(subset_raked,
                filename = paste0('/<<<< FILE PATH REDACTED >>>>', indicator, '_raked/', measure, '/1y/', indicator, '_raked_', measure, '_1y_', year, "_00_00.tif"),
                format = "GTiff",
                overwrite = TRUE)
    writeRaster(subset_unraked,
                filename = paste0('/<<<< FILE PATH REDACTED >>>>', indicator, '_unraked/', measure, '/1y/', indicator, '_unraked_', measure, '_1y_', year, "_00_00.tif"),
                format = "GTiff",
                overwrite = TRUE)
  }

  message('All layers successfully saved in /<<<< FILE PATH REDACTED >>>>')
...
get.cov.wts <- function(rd, ## run_date
  ...
  this_config <- fread(paste0('<<<< FILE PATH REDACTED >>>>/', ind_gp, '/', ind, '/output/', rd, '/config.csv'))
  ...
  load(paste0('<<<< FILE PATH REDACTED >>>>/', ind_gp, '/', ind, '/model_image_history/', rd, pathaddin, '.RData'))
  ...
  load(paste0('<<<< FILE PATH REDACTED >>>>/', ind_gp, '/', ind, '/output/', rd,
  ...
  if(!is.null(gam) & class(gam)[1] != 'try-error'){
    ...
    ## if p values have rounded to 0, use approximation as 1/2 of zero, min value)
    all.p[all.p == 0] <- 1/2 * min(all.p[all.p > 0])
  ...
  ## 'Standardize' the coefs as per a suggestion in this thread:
  ...
  if (this_config[V1 == "gbm_bounded_0_1", V2] == TRUE) {
    inla.X[gbm_cv_pred >= 1, gbm_cv_pred := 0.999]
  }
  ...
  if ("covar" %in% names(fit$summary.random)) {
  ...
  save(imp.mat,
       file = paste0('<<<< FILE PATH REDACTED >>>>/', ind_gp, '/', ind, '/output/', rd,
...
plot.cov.wts <- function(rd, ## run_date
  ...
  # Undo string->factor
  cw.m[, Covar := as.character(Covar)]
  cw.m[, Model := as.character(Model)]

  # Set up some labels
  cw.m[Model == "gam", Model := "GAM"]
  cw.m[Model == "gbm", Model := "GBM"]
  cw.m[Model == "lasso", Model := "Lasso"]

  cw.m[Covar == "access2", Covar := "Travel Time to Nearest Settlement > 50k pop"]
  cw.m[Covar == "dmspntl", Covar := "Nighttime Lights"]
  cw.m[Covar == "elevation", Covar := "Elevation"]
  cw.m[Covar == "evi", Covar := "Enhanced Vegetation Index"]
  cw.m[Covar == "irrigation", Covar := "Irrigation"]
  cw.m[Covar == "worldpop", Covar := "Population"]
  cw.m[Covar == "distriverslakes", Covar := "Distance to Rivers/Lakes"]
  cw.m[Covar == "mapincidence", Covar := "Malaria Incidence"]
  cw.m[Covar == "ghslurbanicity", Covar := "Urbanicity"]
  cw.m[Covar == "wdpa", Covar := "Global Protected Areas"]
  cw.m[Covar == "mswep", Covar := "Precipitation"]
  cw.m[Covar == "crutsdtr", Covar := "Diurnal Temperature Range"]
  cw.m[Covar == "crutspet", Covar := "Potential Evapotranspiration"]
  cw.m[Covar == "crutstmx", Covar := "Avg Daily Max Temperature"]
  cw.m[Covar == "crutswet", Covar := "Wet day frequency"]
  cw.m[Covar == "growingseason", Covar := "Growing Season Length"]
  cw.m[Covar == "lstday", Covar := "Daytime LST"]
  cw.m[Covar == "lstdiurnaldiff", Covar := "Diurnal difference in LST"]
  cw.m[Covar == "nexndvi", Covar := "Normalized Difference Vegetation Index"]
  cw.m[Covar == "tcb", Covar := "Tassled Cap Brightness"]
  cw.m[Covar == "tcw", Covar := "Tassled Cap Wetness"]
  cw.m[Covar == "fertility", Covar := "Fertility"]
  cw.m[Covar == "edu_mean", Covar := "Maternal Education"]
  cw.m[Covar == "ldi_pc", Covar := "Lag-distributed Income Per Capita"]
  cw.m[Covar == "anc4_coverage_prop", Covar := "ANC4 Coverage"]
  cw.m[Covar == "mx_warterror_10years", Covar := "Mortality rate due to war/terror"]
  ...
  p <- ggplot(cw.m, aes(Covar, Model)) +
    ...
    theme(axis.text.x = element_text(size = base_size * 0.8,
                                     ...
                                     colour = "grey50")) +
    labs(fill = "Relative\nImportance",
         title = paste0("Region: ", toupper(reg))) +
...
submit_results_table <- function(reg, indicator, indicator_group, run_date, 
                                 pop_measure, repo, log_dir, baseline_year, 
                                 goal_threshold, indicator_measure, 
                                 target_type='less', geos_node = F, raked = T) {
  dir.create(log_dir)
  ...
  qsub <- paste0('qsub -e ', log_dir, '/errors -o ', log_dir, '/output -cwd -pe multi_slot 30 ',proj.flag, ' -N ',indicator, '_', reg, '_table ', repo, shell.file, ' ', repo, '/mbg_central/run_results_table.R ', indicator, ' ', indicator_group, ' ', run_date, ' ', pop_measure, ' ', repo, ' ', reg, ' ', baseline_year, ' ', goal_threshold, ' ', indicator_measure, ' ', target_type, ' ', raked)
...
compile_results_table <- function(indicator_group,
                                  ...
                                  raked = T) {
  if (raked == T) {
    results_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, '/output/', run_date, '/table_', baseline_year)
  }

  if (raked == F) {
    results_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, '/output/', run_date, '/table_', baseline_year, "_unraked")
  }
  ...
  gaul_to_loc <- fread('<<<< FILE PATH REDACTED >>>>/gaul_to_loc_id.csv')
  all_gauls <- unique(all_admins[, ADM0_CODE])
  for(admin_level in c(0,1,2)) {
    if(admin_level %in% c(0,1)) admin_names <- read.dbf(paste0("<<<< FILE PATH REDACTED >>>>GAUL_admin/admin", admin_level, "/g2015_2014_", admin_level, "/g2015_2014_", admin_level, ".dbf"))
    if(admin_level == 2) admin_names <- read.dbf(paste0("<<<< FILE PATH REDACTED >>>>GAUL_admin/admin", admin_level, "/g2015_2014_", admin_level, "/g2015_2014_", admin_level, "_modified.dbf"))
...
prep_postest <- function(indicator,
  ...
  main_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, '/output/', run_date, '/')
...
get_weighted_correlation_raw_estimated<-function(indicator_group,
  ...
  source('<<<< FILE PATH REDACTED >>>>/lbd_core/mbg_central/prep_functions.R')
  load_libs(c('data.table','raster','sp','weights'))

  input_folder<-"<<<< FILE PATH REDACTED >>>>/input_data/")
  ...
  results_tif<-raster::stack(paste0("<<<< FILE PATH REDACTED >>>>/",indicator_group,"/",indicator,"/output/",rundate,"/",indicator,"_mean_raster.tif"))
...
post_load_combine_save <- function(regions    = strata,
  ...
  if ("unraked" %in% raked) {
    lookup_dir <- paste0(sdir, "/output/", run_date, "/")
  ...
  for(rake in rake_addin){
    ...
    for(ss in summstats){
      ...
      for(reg in regions){
        message(paste0('    ',reg))
        rlist[[reg]] <-
          brick(sprintf('%s/output/%s/%s_%s%s_%s_raster.tif',sdir,run_date,indic,reg,rake,ss))
      }
  ...
  if(rf_table){
    ...
    for(reg in regions){
      rflist[[reg]] <-
        read.csv(sprintf('%s/output/%s/%s_%s_rf.csv',sdir,run_date,indic,reg))
    }
...
submit_aggregation_script <- function(indicator, indicator_group, run_date, raked,
  ...
  for (i in 1:nrow(qsubs_to_make)) {
    ...
    qsub <- paste0('qsub -e ', log_dir, '/errors -o ', log_dir, '/output',
                   ...
                   repo, shell.file, ' ', repo, '/mbg_central/share_scripts/aggregate_results.R ',
                   indicator, ' ',         # arg 4
