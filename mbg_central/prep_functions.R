...
#     indicator       = Specific outcome to be modeled within indicator category, i.e. "edu_0"
  load_config <- function(repo, indicator_group, indicator, config_name=NULL, covs_name = NULL, post_est_only=FALSE, run_date = '') {
    ...
    if(is.null(config_name)) {
      ...
      if(post_est_only==TRUE) config <- fread(paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, '/output/', run_date, '/config.csv'))
    }
...
#     indicator       = Specific outcome to be modeled within indicator category, i.e. "edu_0"
  create_dirs <- function(indicator_group, indicator) {
    ...
    dir.create(paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group))
    dir.create(paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator))

    indicator_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator)
...
#   Returns: Empty raster over modeling area. To be used for cropping covariates quickly and projecting model.
  get_template_raster <- function(simple) {
    ...
    analysis_raster_dir <- '<<<< FILE PATH REDACTED >>>>'
...
#   Returns: Input data subset to modeling area.
   load_input_data <- function(indicator, simple = NULL, agebin = 0, removeyemen = FALSE, pathaddin = "", 
    ...
    if(use_share==FALSE) load_dir <- '<<<< FILE PATH REDACTED >>>>/input_data/'
    if(use_share==TRUE) load_dir <- '<<<< FILE PATH REDACTED >>>>/input_data/'
    if(!withdate & !withtag) d = read.csv(paste0(load_dir, indicator, '.csv'))
    if(withtag) d = read.csv(paste0(load_dir, indicator, datatag, '.csv'))
    if(withdate) d = read.csv(paste0('<<<< FILE PATH REDACTED >>>>',rd,'/', indicator, '.csv'))
    ...
    if(update_run_date == TRUE) {
      if(dir.exists(paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, "/output/", run_date)) == TRUE) {
        existing_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, "/output/", run_date)
      ...
      if(dir.exists(paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, "/output/", run_date)) == FALSE) {
        run_date_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, "/output/", run_date)
    ...
    if(update_run_date == FALSE) {
      if(agebin==0){
        ...
        run_date_dir <- paste0('<<<< FILE PATH REDACTED >>>>/', indicator_group, '/', indicator, "_age",agebin,"/output/", run_date)
        dir.create(run_date_dir, showWarnings = FALSE)
        write.csv(d, file=paste0(run_date_dir, "/input_data", pathaddin, ".csv"))
      }
      return(d)
...
load_simple_polygon <- function(gaul_list, buffer, tolerance = 0.2,
  ...
  if (use_premade == T) {
    ...
    if(identical(gaul_list, c(29, 42, 45, 47, 50, 66, 90, 94, 106, 105, 144, 155, 159, 181, 182, 214, 217, 221, 243))) {
      ...
      load('<<<< FILE PATH REDACTED >>>>/wssa.RData')
    ...
    if(identical(gaul_list, c(8,49,59,68,76,89))) {
      ...
      load('<<<< FILE PATH REDACTED >>>>/cssa.RData')
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(43,58,70,77,79,133,150,152,170,205,226,74,257,253,270))) {
      ...
      load('<<<< FILE PATH REDACTED >>>>/essa.RData')
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(4,40762,40765,145,169,6,248))) {
      ...
      load('<<<< FILE PATH REDACTED >>>>/name.RData')
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(35,142,172,227,235,271))) {
      ...
      load('<<<< FILE PATH REDACTED >>>>/sssa.RData')
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(4,6,8,29,35,42,43,45,47,49,50,58,59,66,68,70,
      ...
      load('<<<< FILE PATH REDACTED >>>>/africa.RData')
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
  ...
  master_shape <- readRDS('<<<< FILE PATH REDACTED >>>>/gbd_2016/master_shape_all.rds')
...
build_simple_raster_pop <- function(subset_shape,u5m=FALSE) {
  ...
  if(u5m==FALSE){
    master_pop <- brick('<<<< FILE PATH REDACTED >>>>/WorldPop_total_global_stack.tif') #WorldPop_allStages_stack.tif')
  } else {
    master_pop <- brick(raster('<<<< FILE PATH REDACTED >>>>/worldpop/archive/replaced_20170623/a0004t/5y/worldpop_a0004t_5y_2000_00_00.tif'),
    raster('<<<< FILE PATH REDACTED >>>>/worldpop/archive/replaced_20170623/a0004t/5y/worldpop_a0004t_5y_2005_00_00.tif'),
    raster('<<<< FILE PATH REDACTED >>>>/worldpop/archive/replaced_20170623/a0004t/5y/worldpop_a0004t_5y_2010_00_00.tif'),
    raster('<<<< FILE PATH REDACTED >>>>/worldpop/archive/replaced_20170623/a0004t/5y/worldpop_a0004t_5y_2015_00_00.tif'))
  }
...
get_gaul_codes_subnat <- function(gaul_list, admin_level) {
  ...
  if(admin_level %in% c(0,1)) hierarchy <- read.dbf(paste0("<<<< FILE PATH REDACTED >>>>/GAUL_admin/admin", admin_level, "/g2015_2014_", admin_level, "/g2015_2014_", admin_level, ".dbf"))
  if(admin_level == 2) hierarchy <- read.dbf(paste0("<<<< FILE PATH REDACTED >>>>/GAUL_admin/admin", admin_level, "/g2015_2014_", admin_level, "/g2015_2014_", admin_level, "_modified.dbf"))
...
gaul_convert <- function(countries, from = "iso3", verbose = F) {
  ...
  table_file <- '<<<< FILE PATH REDACTED >>>>/gaul_to_loc_id.csv'
...
load_libs<-function(packages,stop=F){
  package_lib <- '<<<< FILE PATH REDACTED >>>>'
