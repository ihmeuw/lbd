## #######################################################################
## These functions  take cell pred type objects and make analagous (matrix 
## where rows are observations at some level and columns are wide on draws)
...
get_cell_pred_for_aroc <- function(ind_gp,
  ...
  rds_file <- sprintf(paste0('<<<< FILEPATH REDACTED >>>>/%s/%s/output/%s/',
                                      matrix_pred_name),
                               ind_gp, ind, rd,
                               reg, measure)
...
get_sp_hierarchy <- function() {
  ...
  admin_shp <- readRDS("<<<< FILEPATH REDACTED >>>>/g_2015_2014_2_modified/g_2015_2014_2_modified.rds")
...
make_aroc <- function(ind_gp, ind, rd, 
  ...
  if(weighting_res == 'country'){
    aroc_weight_gauls <- gaul_convert(aroc_weights$country, from = 'iso3')
  }
  ...
  if('cell' %in% type){
    ...
    for(ii in 1:length(regions)){
      ...
      if(weighting_res == 'country'){
        ...
        message('-- making simple raster')
        gaul_list <- get_gaul_codes(regions[ii])
        simple_polygon_list <- load_simple_polygon(gaul_list = gaul_list,
                                                   buffer = 0.4)
        subset_shape   <- simple_polygon_list[[1]]
        simple_polygon <- simple_polygon_list[[2]]
...
make_proj <- function(ind_gp, ind, rd, 
  ...
  if ('admin' %in% type) {
    ...
    message('Working on ADMIN level')
    ## load the admin objects
    ## try two different naming conventions
    ...
    if ('admin' %in% pred_type) {
      ...
      for(aa in 0:2) { ## for each admin type 
        ...
        if (abs_rel == "relative") {
          ...
          ## format this object to look like the one we used for cell level 
          str_match <- stringr::str_match
          draw_cols <- names(cell_pred)[grep("V[0-9]*", names(cell_pred))]
          keep_cols <- c("year", paste0("ADM", aa, "_CODE"), draw_cols)
          cell_pred <- subset(cell_pred, select = keep_cols)
...
compare_admin_years <- function(ind_gp, 
  ...
  out_df <- make_admin_pred_summary(admin_pred = ad_draws,
                                    sp_hierarchy_list = sp_h[[paste0("ADM",aa)]], 
