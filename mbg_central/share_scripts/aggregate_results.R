...
GetAdmin<-function(admin_level,simple_raster, region_adm0_list){
  message(paste0("Loading admin level ",admin_level))
    ...
    if (admin_level %in% c(0,1)) {
      admin_shp<-readOGR(dsn=paste0("<<<< FILEPATH REDACTED >>>>/GAUL_admin/admin", admin_level, "/g2015_2014_", admin_level, "/g2015_2014_", admin_level, ".shp"),
                         layer=paste0("g2015_2014_", admin_level),
                         stringsAsFactors = FALSE, 
                         GDAL1_integer64_policy=TRUE)
    } else if (admin_level == 2) {
      # Use most up to date admin2 code
      admin_shp <- readOGR(dsn="<<<< FILEPATH REDACTED >>>>/GAUL_admin/admin2/g2015_2014_2/archive/replaced_20180626/g2015_2014_2_modified.shp",
