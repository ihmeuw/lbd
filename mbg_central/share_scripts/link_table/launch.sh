...
# [1] "/snfs1/WORK/11_geospatial/admin_shapefiles/"
# so we just need to find [1] and trim the quotes
echo "Getting base shapefile directory ..."
base_shapefile_dir=$(/share/singularity-images/lbd/shells/singR.sh -e r "$script_dir"/run --no-save --args -c shapedir | grep -o '\[1].*' | cut -d '"' -f2)
