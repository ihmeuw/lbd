...
indicator_group          <- as.character(commandArgs()[10])
pathaddin <- paste0('_bin',age,'_',reg,'_',holdout)
outputdir <- file.path('<<<< FILEPATH REDACTED >>>>',indicator_group,indicator,'output',run_date,'/')
...
psample_file <- paste0("psample_", reg, "_", holdout, "_", indicator, "_", run_date, ".csv")
system(sprintf('dos2unix -n %s/mbg_central/share_scripts/psample %s/mbg_central/share_scripts/psample',core_repo,core_repo))
system(paste0(core_repo, '/mbg_central/share_scripts/psample --showthreads --pid=',PID,' ',
              sprintf('<<<< FILEPATH REDACTED >>>>/%s/%s/output/%s/%s ',
...
if(holdout!=0) {
  ...
  # Load from preprepared stratum.rds file
...
## separated out into a different script
...
#### Note that when the stacking covs are used the oos-stackers (used in `fit_mbg()`), they 
...
if (as.logical(use_raw_covs) == T) {
  centre_scale_covs <- FALSE
} else {
  centre_scale_covs <- TRUE
}

## Save all inputs for MBG model into correct location on <<<< FILEPATH REDACTED >>>>
save_mbg_input(indicator         = indicator,
  ...
  file.copy(from = paste0('<<<< FILEPATH REDACTED >>>>/', indicator_group, '/', indicator, '/model_image_history/', skiptoinla_from_rundate, pathaddin, '.RData'),
            to = paste0('<<<< FILEPATH REDACTED >>>>/', indicator_group, '/', indicator, '/model_image_history/', run_date, pathaddin, '.RData'))
}

## reload data and prepare for MBG
...
## for stacking, overwrite the columns matching the model_names so that we can use inla as our stacker
if(as.logical(use_stacking_covs)){
...
if(!as.logical(skipinla)){
  ...
  saveRDS(object = model_fit, ## save this here in case predict process is killed due to memory issues
...
# Run predict_mbg on chunks of 50 samples (to mitigate memory issues)
...
for (i in 1:length(chunks)) {
  message("\n##################################################################")
  message(paste0("Predicting for chunk ", i, " out of ", length(chunks), "..."))
  message("##################################################################\n")
  samp <- chunks[i]
  pm <- predict_mbg(res_fit       = model_fit,
                    cs_df         = cs_df,
                    mesh_s        = mesh_s,
                    mesh_t        = mesh_t,
                    cov_list      = cov_list,
  ...
  saveRDS(object = pm,
          file = paste0("<<<< FILEPATH REDACTED >>>>/", indicator_group, "/", indicator, "/output/", run_date, "/", 
                        "inla_draws_", reg, "_holdout_", holdout, "_agebin_", age, "_chunk_", i, ".rds" ))
  rm(pm)
}

## Make cell preds and a mean raster from disk-written preds
cell_pred <- do.call(cbind, lapply(1:length(chunks), function(i) {
  pm <- readRDS(file = paste0("<<<< FILEPATH REDACTED >>>>/", indicator_group, "/", indicator, "/output/", run_date, "/", 
                              "inla_draws_", reg, "_holdout_", holdout, "_agebin_", age, "_chunk_", i, ".rds" ))
  return(pm)
}))
...
## Clean up disk space if successful cell_pred save
cell_pred_file <- paste0('<<<< FILEPATH REDACTED >>>>/', indicator_group, '/', 
                         indicator, '/output/', run_date, "/",
                         indicator,'_cell_draws_eb',pathaddin,'.RData')

if (file.exists(cell_pred_file)) {
  message("Succesfully created cell_pred RData file. Deleting intermediate files...")
  cell_pred_tempfiles <- paste0(paste0("<<<< FILEPATH REDACTED >>>>/", indicator_group, "/", indicator, "/output/", run_date, "/", 
                        "inla_draws_", reg, "_holdout_", holdout, "_agebin_", age, "_chunk_", 1:length(chunks), ".rds" ))
  unlink(cell_pred_tempfiles)
}
