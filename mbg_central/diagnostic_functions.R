...
# Details: Use with "prep_diagnostic_shiny.R" script
#************************************************************************** 
...
plot_stackers <- function(reg, 
  ...
  master_shape <- subset(master_shape, ADM0_CODE %in% get_gaul_codes(reg))
  ...
  save_maps <- function(input_df, stacker_list, master_shape, result_brick, zmin, zmax, yl, ind, ig, sh_dir, highisbad, o_dir, ctry = NULL) {
    
    if (!is.null(ctry)) {
      input_df <- subset(input_df, country == ctry)
      master_shape <- subset(master_shape, ADM0_CODE == get_gaul_codes(ctry))
  ...
  save_maps(input_df, stacker_list, master_shape, result_brick, zmin, zmax, yl, ind, ig, sh_dir, highisbad, o_dir, ctry = NULL)
  
  # Save maps for individual countries
  if (individual_countries == T) {
    gaul_to_loc_id <- fread("<<<< FILEPATH REDACTED >>>>/gaul_to_loc_id.csv")
    gaul_list <- data.table(GAUL_CODE = get_gaul_codes(reg))
...
# Function to submit a qsub to run the model_diagnostics.R script
make_model_diagnostics <- function(user = Sys.info()['user'],
   ...
   if(log_location=='sharedir'){
     logloc = sharedir
     dir.create(sprintf('%serrors',logloc))
     dir.create(sprintf('%soutput',logloc))
   }

  ## do we want to submit to geo nodes? if so, there are a few tweaks
  if(geo_nodes == TRUE){
    proj      <- "<<<< PROJECT NAME REDACTED >>>>"    ## correct proj for geos nodes
    node.flag <- "-l <<<< NODE FLAG REDACTED >>>>" ## send the job to geo nodes
