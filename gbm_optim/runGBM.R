...
lrnr_type <- commandArgs()[6]
bounds_version <- commandArgs()[7]
experiment_version <- commandArgs()[8]
indicator <- commandArgs()[9]
cv_fold <- as.numeric(commandArgs()[10])
bag_fraction <- as.numeric(commandArgs()[11])
file_addin <- commandArgs()[12]
train_fraction <- 1-(1/cv_fold)
experiment_num <- 1

code_dir <- '<<<< FILEPATH REDACTED >>>>/gbm_optim/'
