...
for (i in 1:length(pjnz_files)) {
  ...
  art <- input$epp.art
  art$name <- paste0(breaks[[2]], "_", breaks[[3]]) ## this is specific to the naming convention of the UNAIDS files and needs to be set manually
  input_list[[f]] <- art
}
...
if (!dir.exists(dir)){
  ...
  message(paste0("Folder", dir, " already exists."))
}
#write.csv(df, paste0(dir,"/",loc,"_province_unaids",y,".csv"), row.names = FALSE)
