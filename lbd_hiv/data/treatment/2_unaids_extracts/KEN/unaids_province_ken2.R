...
AD1_KEN <- ad1[which(ad1$ADM0_NAME == "Kenya"), ]
...
locs_list <- grep("KEN", loc.table$ihme_loc_id, value = T)
...
UNAIDS_KEN <- UNAIDS[which(UNAIDS$ISO3 %in% grep("KEN",UNAIDS$ISO3, value = T)), ]
UNAIDS_KEN <- UNAIDS_KEN[which(UNAIDS_KEN$...1 != "J- Total number receiving ART (0-14) - (Dec 31) Male+Female - (Dec 31)"), ]
UNAIDS_KEN <- UNAIDS_KEN[which(UNAIDS_KEN$ISO3 != "KEN"), ]

UNAIDS_KEN$sex_id <- NA

UNAIDS_KEN$sex_id[which(UNAIDS_KEN$...1 == "J- Total number receiving ART (15+) - (Dec 31) Male - (Dec 31)")] <- 1
UNAIDS_KEN$sex_id[which(UNAIDS_KEN$...1 == "J- Total number receiving ART (15+) - (Dec 31) Female - (Dec 31)")] <- 2

UNAIDS_KEN$...1 <- NULL
UNAIDS_KEN$name <- UNAIDS_KEN$...3
UNAIDS_KEN$...3 <- NULL
UNAIDS_KEN$ISO3 <- NULL

data_long <- reshape2::melt(UNAIDS_KEN, id.vars = c("name", "sex_id"))
data_long$name <- as.character(data_long$name)
data_long$name <- substr(data_long$name, 15, nchar(data_long$name))


data_long$name[which(data_long$name == "Central")] <- "CENTRAL_UNAIDS"
data_long$name[which(data_long$name == "Coast")] <- "COAST_UNAIDS"
data_long$name[which(data_long$name == "Eastern")] <- "EASTERN_UNAIDS"
data_long$name[which(data_long$name == "Nairobi")] <- "NAIROBI_SPECTRUM"
data_long$name[which(data_long$name == "North Eastern")] <- "NORTH_EASTERN"
data_long$name[which(data_long$name == "Nyanza")] <- "NYANZA_UNAIDS"
data_long$name[which(data_long$name == "Rift Valley")] <- "RIFT_VALLEY"
data_long$name[which(data_long$name == "Western")] <- "WESTERN_UNAIDS"
