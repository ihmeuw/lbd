####################################################################################################
## Obtain people alive on ART based on PLHIV for each GADM location in CMR
####################################################################################################
...
equal_coverage_split <- function(country, iso3, user, treat_input, lookup_input){
  ...
  final_data_with_pop[, site_memo:= paste0(ADM2_NAME, " (Department) |",
                                           ADM1_NAME, " (Region) |", " Cameroon (Country)")]
  final_data_with_pop[, shapefile_ref:= "lbd_standard_admin_2_stage_1"]
  final_data_with_pop[, location_name:= "Cameroon|CMR"]
  final_data_with_pop[, location_id:= 202]
  final_data_with_pop[, ihme_loc_id:= "CMR"]
  ...
  ## 9. Read in admin2 CMR file separately and combine with admin1 data that had been split
  CMR_adm2 <- fread(paste0("<<<< FILEPATH REDACTED >>>>"))
  final_data_with_pop <- rbind(final_data_with_pop, CMR_adm2, fill = T)
  ## 9.Save the file -------------------------------------------------------------------------------
  final_data_with_pop <<- final_data_with_pop
