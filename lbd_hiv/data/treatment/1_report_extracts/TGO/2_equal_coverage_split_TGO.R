...
equal_coverage_split <- function(country, iso3, user, treat_input, lookup_input){
  ...
  final_data_with_pop[, site_memo:= paste0(ADM2_NAME, " (Prefecture) |",
                                           ADM1_NAME, " (Region) |", " Togo (Country)")]
  final_data_with_pop[, shapefile_ref:= "lbd_standard_admin_2_stage_1"]
  final_data_with_pop[, location_name:= "Togo|TGO"]
  final_data_with_pop[, location_id:= 218]
  final_data_with_pop[, ihme_loc_id:= "TGO"]
