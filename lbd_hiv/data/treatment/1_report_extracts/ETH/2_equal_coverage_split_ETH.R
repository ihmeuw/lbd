####################################################################################################
## Obtain people alive on ART based on PLHIV for each GADM location in ETH
####################################################################################################
...
equal_coverage_split <- function(country, iso3, user, treat_input, lookup_input){
  ...
  treat_file$location_code <- as.numeric(as.character(treat_file$location_code))
  treat_file$occ_id = NULL
  if ("V1" %in% colnames(treat_file)){
    treat_file$V1 = NULL
  }
  if ("ADM2_NAME"  %in% colnames(treat_file)){
    treat_file$ADM2_NAME = NULL
  }
  ...
  treat_file[, (new_cols) := NA]
  look_up_final <- fread(lookup_input)
  look_up_final = subset(look_up_final, select = c("ADM2_CODE", "other_id", "ADM0_CODE"))

  ######################################################################################
  #subset to admin 1  and admin 0, do separate merges based on diff columns, and then rbind
  data_adm1 <- treat_file[location_code != 72]
  data_adm1 <- merge(look_up_final, data_adm1, by.x = "other_id", by.y = "location_code", all = T, allow.cartesian = T)
  data_adm0 <- treat_file[location_code == 72]
  data_adm0 <- merge(look_up_final, data_adm0, by.x = "ADM0_CODE", by.y = "location_code", all = T, allow.cartesian = T)
  data_all <- rbind(data_adm0, data_adm1)
  ...
  data_year <- as.vector(unique(data_all$end_year))
  ...
  final_data <- data_all[age_group_category == "adults",]
  ...
  final_data_with_pop[ADM1_CODE.x != 72, adm1_hiv_pop := sum(lbd_hiv_pop, na.rm = T), by = c('year', "location_code_adm1", "Region")] # If under- or over-counting in final output, check this line first
  final_data_with_pop[ADM1_CODE.x == 72, adm1_hiv_pop := sum(lbd_hiv_pop, na.rm = T), by = c('year', "ADM1_CODE.x", "Region")] # If under- or over-counting in final output, check this line first
  ...
  final_data_with_pop[, site_memo:= paste0(ADM2_NAME, " (Zone) |",
                                           ADM1_NAME, " (State) |", " Ethiopia (Country)")]
  final_data_with_pop[, shapefile_ref:= "lbd_standard_admin_2_stage_1"]
  final_data_with_pop[, location_name:= "Ethiopia|ETH"]
  final_data_with_pop[, location_id:= 179]
  final_data_with_pop[, ihme_loc_id:= "ETH"]
  ...
  final_data_with_pop[, file_path:= "<<<< FILEPATH REDACTED >>>>"]
  final_data_with_pop[, page_num:= "21"]
  final_data_with_pop[, page_num:= "Table 5-1"]
  final_data_with_pop[, extractor:= "USER"]
  final_data_with_pop[, start_year:= 2010]
  final_data_with_pop[, start_month:= 1]
  final_data_with_pop[, end_year:= 12]
