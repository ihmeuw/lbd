...
equal_coverage_split <- function(country, iso3, user, treat_input, lookup_input){
  ...
  extract_temp <- as.data.frame(read_excel(paste0("<<<< FILEPATH REDACTED >>>>")))
  final_data_with_pop[, site_memo:= paste0(ADM2_NAME, " (County) |",
                                           ADM1_NAME, " (District) |", " Uganda (Country)")]
  final_data_with_pop[, shapefile_ref:= "lbd_standard_admin_2_stage_1"]
  final_data_with_pop[, location_name:= "Uganda|UGA"]
  final_data_with_pop[, location_id:= "190"]
  final_data_with_pop[, ihme_loc_id:= "UGA"]
...
pepfar <- fread(paste0("<<<< FILEPATH REDACTED >>>>"))
pepfar$V1 <- NULL
report <- fread(paste0("<<<< FILEPATH REDACTED >>>>"))
report$V1 <- NULL
final_data <- rbind(pepfar, report, fill = T)
