####################################################################################################
## Obtain people alive on ART based on PLHIV for each GADM location in COD
####################################################################################################
...
iso3 <- c("COD")
country <- c("Democratic Republic of the Congo")
username <- "USER"
...
equal_coverage_split <- function(country, iso3, user, treat_input, lookup_input){
  ...
  final_data_with_pop[, site_memo:= paste0(ADM2_NAME, " (Territory) |",
                                           ADM1_NAME, " (Province) |", " Democratic Republic of the Congo (Country)")]
  final_data_with_pop[, shapefile_ref:= "lbd_standard_admin_2_stage_1"]
  final_data_with_pop[, location_name:= "Democratic Republic of the Congo|COD"]
  final_data_with_pop[, location_id:= 171]
  final_data_with_pop[, ihme_loc_id:= "COD"]
