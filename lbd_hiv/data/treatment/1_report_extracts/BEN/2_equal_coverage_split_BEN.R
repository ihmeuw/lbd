####################################################################################################
## Obtain people alive on ART based on PLHIV for each GADM location in BEN
####################################################################################################
...
## Choose which extract you are doing ##
...
equal_coverage_split <- function(country, iso3, user, treat_input, lookup_input){
  ...
  final_data_with_pop[, site_memo:= paste0(ADM2_NAME, " (Commune) |",
                                           ADM1_NAME, " (Department) |", " Benin (Country)")]
  final_data_with_pop[, shapefile_ref:= "lbd_standard_admin_2_stage_1"]
  final_data_with_pop[, location_name:= "Benin|BEN"]
  final_data_with_pop[, location_id:= 200]
  final_data_with_pop[, ihme_loc_id:= "BEN"]
...
#combine adm05, adm1, and adm2 data together, run after adm05 and adm1 are ran first

adm1 <- fread(paste0("<<<< FILEPATH REDACTED >>>>"))
adm1$V1 <- NULL
adm05 <- fread(paste0("<<<< FILEPATH REDACTED >>>>"))
adm05$V1 <- NULL
adm2 <- fread(paste0("<<<< FILEPATH REDACTED >>>>"))
adm <- rbind(adm1, adm05)
adm <- rbind(adm, adm2, fill = T)

write.csv(adm, paste0("<<<< FILEPATH REDACTED >>>>"))
