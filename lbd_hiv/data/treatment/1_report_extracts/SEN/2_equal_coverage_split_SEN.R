####################################################################################################
## Obtain people alive on ART based on PLHIV for each GADM location in SEN
####################################################################################################
...
equal_coverage_split <- function(country, iso3, user, treat_input, lookup_input){
  ...
  final_data_with_pop[, adm1_hiv_pop := sum(lbd_hiv_pop, na.rm = T), by = c('year', "location_code_adm1", "site_memo")]
  ...
  final_data_with_pop[, site_memo:= paste0(ADM2_NAME, " (Department) |",
                                           ADM1_NAME, " (Region) |", " Senegal (Country)")]
  final_data_with_pop[, shapefile_ref:= "lbd_standard_admin_2_stage_1"]
  final_data_with_pop[, location_name:= "Senegal|SEN"]
  final_data_with_pop[, location_id:= 216]
  final_data_with_pop[, ihme_loc_id:= "SEN"]
