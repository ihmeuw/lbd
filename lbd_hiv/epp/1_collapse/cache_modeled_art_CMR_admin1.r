#### CMR admin 1 script
#### need to create an admin 1 time series for CMR
...
for (loc in countries) {
  ...
  fs <- grep("ADM1_CODE", fs, value = TRUE)
...
for (c in countries) {
  ...
  temp$hiv_treat_count <- as.numeric(temp$hiv_treat_count)
  temp <- temp[which(temp$age_group_category %in% c("adults", "Adults") | is.na(temp$age_group_category)), ] #remove the kids
...
data_s <- merge(data_s, ad1, by.x = c("location_code"), by.y = c("ADM1_CODE"), all.x = TRUE)

##_calculate ADM1 and ADM0 totals
data_s$hiv_treat_count <- data_s$V1


nat_tot <- data_s[,lapply("hiv_treat_count", function(x) sum(get(x), na.rm = TRUE)), by = c("end_year", "age_group_category", "ADM0_NAME")]
...
data_s <- merge(data_s, nat_tot, by = c("end_year", "ADM0_NAME"))
...
pred_frame <- merge(pred_frame, plhiv_0, by.x = c("ADM0_NAME", "end_year"), by.y = c("ADM0_NAME", "year"))
...
for (c in (countries)) {
  ...
  pred_frame_small <- merge(pred_frame_small, c_nb_lookup, by.x = "location_code", by.y = c("ADM1_CODE"), all = T)
  pred_frame_small <- merge(pred_frame_small, plhiv, by.x = c("location_code", "end_year"), by.y = c("ADM1_CODE", "year"), all.x = TRUE)
...
codes <- as.data.table(unique(data_final[, c("ADM0_NAME","ADM1_NAME", "location_code")]))
setorder(codes, "ADM0_NAME", "ADM1_NAME")
...
export_data <- data_final[ ,c("end_year", "location_code", "pred4_gaus_INLA_s", "plhiv")]
