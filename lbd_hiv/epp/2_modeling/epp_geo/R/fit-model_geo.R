fitmod <- function(obj, ..., B0 = 2e5, B = 1e3, B.re = 1e3, number_k = 500, D = 4, opt_iter = 3, no.anc, likelihood_type = likelihood_type, prev_type = prev_type){
#obj <- dt[["RWA"]]
#equil.rprior = eq.prior
#B0 = 2e5
#B = 1e3
#B.re = 1e3
#number_k = 500
...
#no.anc = no.anc
  ...
  while(inherits(fit, "try-error")){
    ...
    fit <- try(IMIS(B0, B, B.re, number_k, D, opt_iter, fp=fp, likdat=likdat, no.anc = no.anc, likelihood_type = likelihood_type, prev_type = prev_type))
    fit.time <- proc.time() - start.time
    message("end time recorded IMIS is over")
  }
  fit$fp <- fp
  fit$likdat <- likdat
  fit$time <- fit.time
...
simfit.eppfit <- function(fit, rwproj=FALSE){
  fit$param <- lapply(seq_len(nrow(fit$resample)), function(ii) fnCreateParam(fit$resample[ii,], fit$fp, no.anc))
  ...
  if (no.anc == FALSE) {fit$pregprev <- mapply(fnPregPrev.epp, mod.list, fp.list)}
...
prepare_epp_fit <- function(pjnz, proj.end=2019.5, num_knots = num_knots, no.anc = no.anc){
  #################################################
  #pjnz = pjnz
  #proj.end=2018.5
  #num_knots = num_knots
  ...
  if(loc %in% c("KEN_35623", "KEN_35640", "KEN_35662")) {
    temp.pjnz <- paste0("<<<< FILEPATH REDACTED >>>>")
  ...
  if((length((grep("NGA_",loc))) >=1 ) & (nrow(epp.input$art.specpop) < 1)) {
