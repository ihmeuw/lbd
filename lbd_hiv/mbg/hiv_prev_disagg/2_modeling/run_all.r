...
if (!is.null(resub_date)) {
  ...
  models <- list(
  c('0 Final model',                             '0_config_main',                        'cov_list_std_and_hiv')
)
...
qsub_launch <- function(config_file, covs_file, run_date =NULL) {
  ...
  outdir <- paste0('<<<< FILEPATH REDACTED >>>>/output/', run_date, '/')
  dir.create(outdir, showWarnings = F)
  dir.create(paste0(outdir, 'errors'), showWarnings = F)
  dir.create(paste0(outdir, 'output'), showWarnings = F)
  
  qsub <- paste('qsub -e', paste0(outdir, 'errors'), '-o', paste0(outdir, 'output'), '-l m_mem_free=50G -l fthread=4 -N', paste0('launch_', run_date),
                '-P proj_geo_nodes -q geospatial.q -l h_rt=05:00:00:00 -l archive=TRUE',
                ...
                paste0(<<<< FILEPATH REDACTED >>>> '/lbd_hiv/mbg/hiv_prev_disagg/2_modeling/launch.r'),
...
if (!is.null(resub_date)) {
  apply(models[1:5,], 1, function(x) qsub_launch(x[2], x[3]))
