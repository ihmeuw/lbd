...
geomatched_version <- most_recent_date(paste0("<<<< FILEPATH REDACTED >>>>"))
prev_version <- most_recent_date("<<<< FILEPATH REDACTED >>>>")
...
report <- fread(paste0(indic_repo, "/data/covariate_surveys/male_circumcision/reports/circumcision_extraction.csv")) 
...
write.csv(crosswalked, file = paste0('<<<< FILEPATH REDACTED >>>>'), row.names = F)
