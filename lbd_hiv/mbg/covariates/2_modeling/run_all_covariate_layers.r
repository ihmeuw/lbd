...
if (!is.null(resub_date)) {
  models <- read.csv(paste0("<<<< FILEPATH REDACTED >>>>/male_circumcision/output/models_submitted_', resub_date, '.csv"))

} else {
  models <- rbind(
    # Base model
    data.table("male_circumcision",      c("config_default_pc"),  "a1549m",  "none"),
    data.table("sti_symptoms",           c("config_default_pc"),  "a1549t",  "none"),
    data.table("condom_last_time",       c("config_default_pc"),  "a1549t",  "_BOTH"),
    data.table("had_intercourse",        c("config_default_pc"),  "a1549f",   "_WN"),
    data.table("in_union",               c("config_default_pc"),  "a1549t",  "_conservative"),
    data.table("multiple_partners_year", c("config_default_pc"),  "a1549f",   "_WN"),
    data.table("multiple_partners_year", c("config_default_pc"),  "a1549m",  "_MN"),
    data.table("partner_away",           c("config_default_pc"),  "a1549f",  "none"))
...
qsub_launch <- function(indicator, prior, population, data_tag, run_date = NULL) {
  ...
  qsub <- paste('qsub -e', paste0(outdir, 'errors'), '-o', paste0(outdir, 'output'), '-l m_mem_free=15G -l fthread=5 -N', paste0('launch_', run_date),
                ...
                '-v sing_image=<<<< FILEPATH REDACTED >>>>/lbd_full_20200128.simg -v SET_OMP_THREADS=1 -v SET_MKL_THREADS=1',
