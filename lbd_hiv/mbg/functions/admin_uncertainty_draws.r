####################################################################################################
## Make table of uncertainty in admin estimates from model
####################################################################################################
...
make_admin_change_uncertainty_tables <-
  function(run_date,
    ...
    # Load in admin draws
    load(paste0("<<<< FILEPATH REDACTED >>>>", indicator, "_raked_admin_draws_eb_bin0_0.RData"))
    lvl <- c(0, 1, 2)
    ...
    lapply(lvl, function(x) {
      ...
      write.csv(admin_dif, paste0(outdir, "/pred_derivatives/admin_summaries/admin", x, "_diff_uncertainty.csv"))
    })
  }
...
make_pixel_difference_uncertainty_rasters <-
  function(run_date,
    ...
    lapply(regions, function(reg) {
      ...
      # Load simple raster
      simple_polygon_list <-
        load_simple_polygon(gaul_list = get_gaul_codes(reg),
                            buffer = 0.4,
                            subset_only = FALSE)
      ...
      for (stat in summary_stat) {
        ...
        writeRaster(summ, file = paste0(outdir_raster, '/', indicator, '_', reg,
                                        "_raked_2000_2017_dif_", stat, "_raster.tif"),
                    format = 'GTiff',overwrite = TRUE)}
      perc_pos_draws <- insertRaster(simple_raster,  matrix(perc_pos_draws,  ncol = 1))
      writeRaster(perc_pos_draws, file = paste0(outdir_raster, '/', indicator, '_', reg,
    ...
    lapply(summary_stat, function(stat) {
      reg_list <- lapply(regions, function(reg) {
        brick(paste0(outdir_raster, '/', indicator, '_', reg, "_raked_2000_2017_dif_", stat, "_raster.tif"))
      })
    ...
    writeRaster(rlist,
                file = paste0(outdir_raster,'/',indicator,'_',"africa","_raked_2000_2017_dif_",stat,"_raster.tif"),
                format = 'GTiff',
                overwrite = TRUE)
    })

    perc_draws <- lapply(regions, function(reg) {
      brick(paste0(outdir_raster, '/', indicator, '_', reg, "_raked_2000_2017_dif_perc_pos_draws_raster.tif"))
    })
    perc_d <- do.call(raster::merge, unname(perc_draws))
    writeRaster(perc_d,
                file = paste0(outdir_raster,'/',indicator,'_',"africa","_raked_2000_2017_dif_perc_pos_draws_raster.tif"),
                format = 'GTiff',
                overwrite = TRUE)
  }
