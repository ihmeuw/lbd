...
make_admin_change_uncertainty_tables <-
  function(run_date,
    ...
    share_dir <- paste("<<<< FILEPATH REDACTED >>>>",indicator_group, indicator, "output", run_date, "/", sep="/")
    ...
    # Load in admin draws
    if (raked == T) {
      load(paste0("<<<< FILEPATH REDACTED >>>>", indicator, "/output/", run_date, "/", indicator, "_raked_admin_draws_eb_bin", age, "_0.RData"))
    } else {
      load(paste0("<<<< FILEPATH REDACTED >>>>", indicator, "/output/", run_date, "/", indicator, "_unraked_admin_draws_eb_bin", age, "_0.RData"))
    }
    lvl <- c(0, 1, 2)
    ...
    lapply(lvl, function(x) {
      ...
      write.csv(admin_dif, paste0(outdir, "<<<< FILEPATH REDACTED >>>>", x, "_diff_uncertainty_", min(years), "_", max(years), "_bin", age, ".csv"))
    })
  }
