...
#' data by first seperating out the point data and then assigning it to the correct admin1 unit by its lat/long using a GAUL admin1 shapefile. It
...
crosswalk_data_prepare <- function(data, indicator = "hiv_test", weight = "hiv_weight"){

  # Load required packages
  library(sf)
  library(dplyr)
  library(data.table)
  source("<<<< FILEPATH REDACTED >>>>/mbg_central/util/location_metadata_functions.R")
  ...
  loc <- get_location_code_mapping() %>%
    filter(GAUL_CODE %in% get_gaul_codes("Africa")) %>%
    pull(ihme_lc_id)
  ...
  admin_shp <- st_read(get_admin_shapefile(1), quiet = T)
  ...
  gaul_to_loc_id <-
    ...
    filter(GAUL_CODE %in% get_gaul_codes(c("cssa", "wssa", "essa_sdn", "sssa")))
...
#'
#' PDF of linear modle in log space fitting microdata hiv prevalence in seperate age ranges ('<<<< FILEPATH REDACTED >>>>/crosswalk_output/fiting_standard_[age_range]')
#' PDF of new to old hiv prevalences for the crosswalked report data ('<<<< FILEPATH REDACTED >>>>/crosswalk_output/Adjusted_hiv_test_[age_range]')



crosswalk_reports <- function(crosswalk_data, report_data, indicator = "hiv_test", weight = "hiv_weight") {
  ...
  data_crosswalked <- data_crosswalked %>%
    group_by(nid, country, source, year, point, shapefile, admin1) %>%
    summarize(
  ...
  #add country gaul codes, then add region in seperate column to divide admin1 by region
  data_countries <- data_standard$country %>% unique
  data_gaul <- get_gaul_codes(data_countries)

  data_standard <-
    data.table(country = data_countries, gaul = data_gaul) %>%
    left_join(data_standard, ., by = "country")

  #Matrix of gaul codes matched with region
  regions <- c("cssa", "essa", "sssa", "wssa")
  region_gaul <- data.table(matrix(ncol = 2, nrow = 0))
  colnames(region_gaul) <- c('region', 'gaul')
  for (rr in regions) {
    rr_gauls <- get_gaul_codes(rr)
    region_gaul <- rbind(region_gaul,
                         data.table(region = rep(rr, length(rr_gauls)),
                                    gaul = rr_gauls))
  }

  region_gaul$region <- as.character(region_gaul$region)

  ## merge on region
  data_standard <- left_join(data_standard, region_gaul, by = "gaul")
  ...
  pdf(paste0('<<<< FILEPATH REDACTED >>>>/crosswalk_output/Adjusted_', indicator, '_', age_range[1], '_', age_range[2], '.pdf'),
      width = 16, height = 10)
  print(gg1)
  print(gg2)
