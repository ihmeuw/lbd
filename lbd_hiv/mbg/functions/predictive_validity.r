...
config <- set_up_config(repo            = outputdir,
                        ...
                        covs_file       = paste0(indic_repo, "<<<< FILEPATH REDACTED >>>>"),
                        post_est_only    = TRUE,
                        run_tests = FALSE)
...
if(indicator!='hiv_prev_disagg') {
  ...
  source(paste0(indic_repo, '<<<< FILEPATH REDACTED >>>>/3_functions/get_is_oos_draws_function.r'))
...
write.csv(pvtab, file = paste0(outputdir, "/summary_metrics/pv_metrics.csv"), row.names = F), row.names = F)
