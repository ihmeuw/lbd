...
# list indicators
indics <- c('had_diarrhea')

# list either all regions or one region/country
regions <- 'all'
...
for (region in regions ) {
  ...
  for (i in indics) {
    ...
    # set cropped covs RData file (will only be used if crop_covs is FALSE)
    cropped_covs_file <- '<<<< FILEPATH REDACTED >>>>'
    ...
    script <- paste0('<<<< FILEPATH REDACTED >>>>/covariate_selection/02_vif_selection_script.R')
