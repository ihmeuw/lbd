...
##              memory [character] -- the ammount of memory specified for this job
##              threads [numeric] -- number of threads
##              queue [character] -- job queue
##              run_time [character] -- How long job should run (max value)
...
qsub <- function(code, name = NULL, arguments = NULL, hold = NULL, 
                 memory = "10G", threads =  1, queue = "geospatial.q", run_time = "08:00:00:00",
                 intel = F, proj = NULL, sgeoutput = NULL, skip_if_exists = NULL, singularity_opts = NULL) {

  # Define shell, use default singularity image as a default
  corerepo  <- paste0("<<<< FILEPATH REDACTED >>>>/lbd_core/")
  ...
  x <- paste0("qsub -cwd",
             ...
             if (intel) " -l hosttype=intel " else " ",
             shell, " ", code, " ", 
