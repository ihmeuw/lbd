...
mean <- out$par.random
ii <- grep("^B|^re|pi", rownames(out$jointPrecision))
...
# If running completeness save completeness draws for viz
if (grepl("_c", model)) {
  pi <- data.table(inv.logit(as.matrix(sims[names(mean) == "logit_pi",])))
  
  # If fixed, make these estimates equal to 1
  if (!calc_child_comp | !calc_adult_comp) {
    # Place holder for completeness fixed to 1
    fixed_pi <- data.table(matrix(rep(inv.logit(20), prod(dim(pi))), nrow = dim(pi)[1], ncol = dim(pi)[2]))
    names(fixed_pi) <- names(pi)
    if (calc_child_comp) {
      pi <- rbind(pi, fixed_pi)
    } else {
      pi <- rbind(fixed_pi, pi)
    }
  }
  pi[, C := 1:nrow(pi) - 1L]
  setkeyv(pi, "C")
  saveRDS(pi, paste0(main_dir, "/temp_dir/completeness_draws_", sex)) # used for later plotting of posterior
  pi <- as.matrix(pi[J(data$C), simvars, with = F])
}
...
# If calculating save a version that incorporate completeness, should better fit data
if (grepl("_c", model)) {
  draws_pi <- data.table(exp(draws) * pi)
  draws_pi[, area := as.integer(data$area)]
  draws_pi[, year := as.integer(data$year + years[1])]
  draws_pi[, sex := as.integer(data$sex)]
  draws_pi[, age := as.integer(as.character(factor(data$age, labels=ages)))]
}
...
if (grepl("_c", model)) draws_pi <- merge(pop, draws_pi, by=c("area", "year", "sex", "age"), all=T)
rm(pop); gc()
...
if (grepl("_c", model)) {
  ...
  for (this_year in years) {
    ...
    # save draws and estimates
    save(draws, file=paste0(temp_dir, "/draws_pi_mort_", area_var, "_", this_year, "_", sex, ".rdata"))
    save(est, file=paste0(temp_dir, "/est_pi_mort_", area_var, "_", this_year, "_", sex, ".rdata"))
