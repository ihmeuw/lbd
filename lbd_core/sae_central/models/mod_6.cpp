// see fit_mod_6.r for model description
...
// Define parameters
  ...
  // RE6: area-age-level random intercept (IID Normal)
  PARAMETER_ARRAY(re6); 
  PARAMETER(log_sigma2_6); 
  Type sigma_6 = exp(0.5*log_sigma2_6); 
  Type prec_6 = exp(-1*log_sigma2_6);   
...
// NLL contribution from random effects  
  ...
  // RE5: area-year-level random intercept 
  PARALLEL_REGION nll -= dnorm(vector<Type>(re5), Type(0), sigma_5, true).sum(); 

  // RE6: area-age-level random intercept 
  ...
  PARALLEL_REGION nll -= dgamma(prec_6, Type(1), Type(1000), true);
...
// NLL contribution from data 
  ...
  for(size_t i = 0; i < Y.size(); i++)
    log_m[i] += re1(A[i], T[i]) + re2[J[i]] + re3[J[i]]*T[i] + re4[J[i]]*A[i] + re5(J[i], T[i]) + re6(J[i], A[i]);
