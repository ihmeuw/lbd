...
tmb_par <- list(
  ...
  logit_pi = rep(1, nrow(completeness_prior)))

## Fit model ---------------------------------------------------------------------------------------
# compile CPP code for objective function
openmp(16)
TMB::compile("models/mod_6_c.cpp")
dyn.load(dynlib("models/mod_6_c"))
config(tape.parallel=0, DLL="mod_6_c")
...
cat("\n\n***** Make objective function\n"); flush.console()
obj <- MakeADFun(tmb_data, tmb_par, random=c("B", paste0("re", 1:6), "logit_pi"), DLL="mod_6_c", map = map)
