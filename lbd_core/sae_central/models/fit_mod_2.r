...
tmb_par <- list(
  ...
  re5=array(rep(0, num_j * num_a * num_t), dim=c(num_j, num_a, num_t)), # area-age-year-level random intercept
  log_sigma2_5=-5)

## Fit model ---------------------------------------------------------------------------------------
# compile CPP code for objective function
openmp(16)
TMB::compile("models/mod_2.cpp")
dyn.load(dynlib("models/mod_2"))
config(tape.parallel=0, DLL="mod_2")

# make objective function
cat("\n\n***** Make objective function\n"); flush.console()
obj <- MakeADFun(tmb_data, tmb_par, random=c("B", paste0("re", 1:5)), DLL="mod_2")
