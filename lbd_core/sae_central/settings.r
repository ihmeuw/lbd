...
get_settings <- function(main_dir) {
  ...
  for (var in 1:nrow(settings)) {
    ...
    # change root to match file system, if necessary
    if(grepl("_file$|_files$|_dir$", arg_name)) {
      if(Sys.info()[1] == "Windows") {
        arg_value <- gsub("<<<< FILEPATH REDACTED >>>>", "<<<< FILEPATH REDACTED >>>>", arg_value)
      } else {
        arg_value <- gsub("<<<< FILEPATH REDACTED >>>>", "<<<< FILEPATH REDACTED >>>>", arg_value)
      }
    }
  ...
  optional <- c("covars", "covars_as", "covars_trans", "covar_file", "covar_as_file", "geoagg_files", "raked", "completeness_prior_file")
...
check_settings <- function(main_dir) {
  ...
  all_settings <- c("model", "area_var", "years", "ages", "sexes",
                    ...
                    "geoagg_files", "shape_file", "age_std_file", "temp_dir", "raked")
  ...
  if(!is.null(raked)) if(class(raked) != "character") stop("'raked' should be type character")
  ...
  if(!is.null(raked)) if(length(raked) != 4) stop("'raked' should be length = 4 or NULL")
  ...
  # check that raked arguments are formatted correctly 
  if(!is.null(raked)){
    if(!all.equal(sort(names(raked)), c("cause_id", "country", "gbd_round_id", "rake_level"))){
      stop("'raked' must be a named vector with country, cause_id, gbd_round_id and rake_level")
    }
    if(is.null(geoagg_files)) stop("'geoagg_files cannot be missing, it must be used to aggregate the areas to a rake_level when raking")
    if(!raked[["rake_level"]] %in% names(geoagg_files)) stop("rake_level in 'raked' must be included in the geoagg_files in order to aggregate estimates before raking")
    load(geoagg_files[raked[["rake_level"]]])
    weights[, .(raked[["rake_level"]])]
    if(!raked[["gbd_round_id"]] %in% 4:6) stop("GBD round should be either 4 (2016), 5 (2017) or 6 (2019)")
    if(nchar(raked[["country"]]) != 3) stop("Country must be an iso3 code")
    if (!all.equal(ages, seq(0, 80, 5))) stop("Ages must be in 5 year age bins from 0, 80 in order to rake")
    
    # Check to make sure population estimates are raked to GBD ---------------------------
    ...
    age_link <- 
      ...
      rbind(data.table(age_group_id = 1, age = 0), ., data.table(age_group_id = 21, age = 80)) # age group id's for under5 and 80+
    
    # Get country location code(s)
    source('<<<< FILEPATH REDACTED >>>>/lbd_core/mbg_central/misc_functions.R')
    loc_ids <- get_gbd_locs(raked[["country"]], shapefile_version = "current", rake_subnational = T)$location_id
    ...
    gbd_pop <- 
      ...
      dplyr::select(year = year_id, age, sex = sex_id, loc_id = location_id, pop = population) %>% 
      data.table()
    
    # Load the population with the corrrect weights
    load(geoagg_files[[raked[["rake_level"]]]])
    sae_pop <- weights
    setnames(sae_pop, raked[["rake_level"]], "loc_id")
    # 
    # if (!are_equal(gbd_pop[order(year, age, sex, loc_id)], sae_pop[, .(pop = sum(pop)), by = .(year, age, sex, loc_id)][order(year, age, sex, loc_id)])) {
    #   stop("Population must be raked to GBD in order to rake mortality correctly")
    # }
  }
