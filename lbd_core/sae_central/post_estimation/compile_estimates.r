...
##              rake [logical] -- are estimates raked? If raked, output file is tagged with `_raked.csv`
##
...
if (rake) {
  files <- files[grep("raked", files, invert = F)]
} else {
  files <- files[grep("raked", files, invert = T)]
}
...
if (!grepl("_c", model)) {
  ...
  if (length(issues) == 0) save(est, file = paste0(main_dir, "/est_all", if (rake) "_raked" else "", ".rdata"))
  if (length(issues) > 0) write.table(issues, file = paste0(main_dir, "/est_all_issues", if (rake) "_raked" else ".csv"), row.names = F, col.names = F)
  
} else {
  files_pi <- files[grep("pi", files, invert = F)]
  files    <- files[grep("pi", files, invert = T)]
  ...
  est_pi <- rbindlist(lapply(files_pi, function(f) {
    load(paste0(temp_dir, "/", f))
    est
  }))
  
  # format
  setkeyv(est_pi, c("level", "area", "year", "sex", "age"))
  setcolorder(est_pi, c(key(est_pi), "mean", "lb", "ub", "se"))
  
  # check for issues
  issues <- check(est_pi)
  
  # if there are no problems, save the estimates, otherwise save the list of issues
  if (length(issues) == 0) save(est_pi, file = paste0(main_dir, "/est_all_pi_mort_", if (rake) "_raked" else "", ".rdata"))
  if (length(issues) > 0) write.table(issues, file = paste0(main_dir, "/est_all_pi_mort_issues", if (rake) "_raked" else ".csv"), row.names = F, col.names = F)
