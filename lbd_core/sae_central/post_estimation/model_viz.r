...
##              rake [logical] -- are estimates raked? If raked, output graphs are tagged with `_raked.pdf`
##
...
# Estimates
load(paste0(main_dir,"/est_all", if (rake) "_raked" else "", ".rdata"))
est <- est[sex < 3,]
...
## Initialize PDF ----------------------------------------------------------------------------------
pdf(paste0(main_dir, "/model_results", if (rake) "_raked" else "", ".pdf"), width = 14, height = 8)
