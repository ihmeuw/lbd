...
save_cell_pred_summary <- function(summstat, raked, ...) {
  ...
  if (raked == "raked_c_complement") {
    ...
    load(paste0(sharedir, "/output/", run_date, "/", indicator, "_raked_c_complement_cell_draws_eb_bin0_", reg, "_0.RData" ))
    mask_raster <- "raked_simple_raster"
  }
  ...
  save_post_est(ras, 'raster', paste0(reg, case_when(raked == 'raked' ~ '_raked',
