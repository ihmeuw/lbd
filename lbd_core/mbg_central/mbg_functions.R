  ...
  # else do the inla version
    ...
    if(pc_prior){
      spde = inla.spde2.pcmatern(
        mesh = mesh_s,
        prior.range = c(range_lower_tail,0.05),
        prior.sigma = c(sigma_upper_tail,0.05)
      )

      message(paste("Building spde with pc prior, 5% probability that the range is lower than",
                     range_lower_tail,
                     "and a 5% probability that sigma is greater than",
                    sigma_upper_tail))
    } else {
      spde <- inla.spde2.matern(mesh = mesh_s,  alpha = 2)
      message("Building spde with default prior")
    }
...
predict_mbg <- function(res_fit, cs_df, mesh_s, mesh_t, cov_list,
  ...
  run_dir <- "<<<< FILEPATH REDACTED >>>>/inla_draws/"
