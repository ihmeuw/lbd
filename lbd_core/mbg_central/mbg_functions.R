...
#' @title Inverse logit
#' @description Function that computes the inverse logit (expit) of a number
#'
#' @param x number
...
#' @title Save Input into MBG Model
#' @description Save a number of objects that are needed for running the mbg model
#' 
#' @param indicator indicator name, corresponding to the response variable in df
#' @param indicator_group group the indicator belongs to
#' @param df data frame containing data
#' @param simple_raster simple raster used for modelling region
#' @param simple_raster2 simple raster used for modelling region, based on the subnational shapefile
...
#' @param cov_list named list of covariate bricks, each should have length equal to number of years
#' @param run_date run date
#' @param pathaddin additional thing to add to path, quite often: \code{paste0("_bin", age, "_", reg, "_", holdout)}
#' @param child_model_names string vector of child model names
#' @param all_fixed_effects a string of model fixed effects of form 'var1 + var2 + var3' or as string vector
...
#' 
#' @return NA. Processes and saves inputs.
save_mbg_input <- function(indicator = indicator, 
  ...
  # parse pathaddin to generate image file, remove '_bin' suffix using substring()
  path_args <- strsplit(substring(pathaddin, 5), '_')[[1]]
  pb <- RunPathBuilder$new(indicator_group = indicator_group,
                           ...
                           region = path_args[2],
                           holdout = path_args[3])
  save(list = to_save, file = pb$get_image_file())
  message(sprintf('All inputs saved to %s', pb$get_image_file()))
}


#' @title From theta to rho
#' @description Function that computes the transformation from theta to rho in the AR1 process.
#' See \code{inla.doc("ar1?")}
#'
#' @param theta theta
#' @return rho
...
#' See \code{inla.doc("ar1?")}
#'
#' @param temporal_model_theta1_prior string that is a list written in INLA notation 
#' (e.g. 'list(prior = 'normal', param = c(0, 1/(2.58^2)))')
#' 
#' @return None. Prints message about 95% range for rho in temporal model
...
#' @param spde NOT USED (since INLA data stack is?)
#' @param cov NOT USED (since INLA data stack is?)
...
#'   draws. Possible options are 'inverse-logit'. Can also take a function to apply e.g. "log"
#'   to apply to the draws. "" will result in the identity transform.
#'
...
predict_mbg <- function(res_fit, 
  ...
  run_dir <- paste0(fp_list['mbg_root'], indg, '/', ind, '/output/', rd, '/inla_draws')
  ...
  if(transform == 'inverse-logit') {
    ...
    cell_pred <- eval(parse(text=sprintf('%s(as.matrix(cell_all))',transform)))
  }
