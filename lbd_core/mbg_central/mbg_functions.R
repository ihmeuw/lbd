...
save_mbg_input <- function(indicator = indicator, indicator_group = indicator_group, df = df , simple_raster = simple_raster, simple_raster2 = NULL, 
                           mesh_s = mesh_s,
                           mesh_int = mesh_int,
                           mesh_t = mesh_t, cov_list = all_cov_layers, run_date = NULL, pathaddin="",
                           child_model_names = NULL, all_fixed_effects = NULL, period_map = NULL,centre_scale = T, reformat_df_covs=F) {
  
  if(reformat_df_covs==T){
  ...
  ## Save all inputs
  to_save <- c("df", "simple_raster", "mesh_s", "mesh_t", 'mesh_int', 'cov_list', 'child_model_names', 'all_fixed_effects','period_map')
  if(!is.null(simple_raster2)) to_save <- c(to_save, "simple_raster2")
  save(list = to_save, file = paste0(<<<< FILEPATH REDACTED >>>> '/model_image_history/', run_date, pathaddin, '.RData'))
  message(paste0('All inputs saved to <<<< FILEPATH REDACTED >>>>/model_image_history/', run_date, pathaddin,'.RData'))
...
build_mbg_data_stack <- function(df, 
  ...
  # else do the inla version
    ...
    ## This 'space' index is actually space indices in time
    ...
    if (anc_correction == 'gp') {
      site <- inla.spde.make.index('site', n.spde = spde$n.spde)
      A.site <- inla.spde.make.A(mesh = mesh_s,
                                 loc = data.locs,
                                 weights = df$anc)
    }
    ...
    if (anc_correction == 'iid') {
      design_matrix$anc <- df$anc
      design_matrix$site <- df$site
    ...
    if (anc_correction == 'gp') {
        A.list <- c(A.list, list(A.site = A.site))
        e.list <- c(e.list, list(site = site))
    }
...
predict_mbg <- function(res_fit, cs_df, mesh_s, mesh_t, cov_list,
  ...
  run_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', rd, '/inla_draws')
  ...
  pred_l <- pred_l[!rownames(pred_l) == "anc", , drop = F]
  ...
  if(!is.null(pred_ctry_res)){
    ...
    for(gg in reg.gauls){
      ...
      if(gg %in% gauls.with.res){
        ...
        # Get gaul code for that country 
        gg.idx  <- which(gauls.with.res == gg)
        # For every pixel that fall in that country, add in draws of country random effect 
...
save_mbg_preds <- function(config, time_stamp, run_date, mean_ras, sd_ras, res_fit, cell_pred, df ,pathaddin="") {
  ...
  if(time_stamp==TRUE) output_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date)
  if(time_stamp==FALSE) output_dir <- paste0(<<<< FILEPATH REDACTED >>>>'/output/scratch')
