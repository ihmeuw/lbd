...
#'                            outdir = sprintf("%s/u5m/%s_%s/output/%s", fp_list$mbg_root, indicator, age_group, run_date),
#'                            indicator,
#'                            age_group,
#'                            prefix = "custom_india",
#'                            reg = "south_asia")
...
#' @title LogitFindK
#' @description Main function for finding logit raking values. Called by \code{\link{calculate_raking_factors}} for each location-year. Unlike linear raking which calculates the raking target directly, logit raking searches for the raking target value iteratively by trying different values (max number of tries set by `MaxIter`). After each try, the new value is evaluated against the gbd value and the number is adjusted up or down by the average of the current value and `MaxJump`. This essentially functions like a binary search. Once the difference between the raking target and gbd value is less than `FunTol`, the value is returned. 
#'
#' @param gbdval numeric, gbd raking target for a location-year
#' @param pixelval matrix, i.e. cell draws for a location-year
#' @param weightval numeric vector of weights (typically from population)
#' @param MaxIter default `40`. Number of jumps towards the solution
#' @param MaxJump default `10`. Maximum size of a jump to the answer for logit raking.
#' @param FunTol default `1e-5`. Maximum allowed difference between the raking target and raked results
#' @param approx_0_1 Boolean. If T, Any values of zero will be replaced with 1e-10 and values of 1 will be replaced with (1-(1e-10)).  Otherwise, logit transformations will fail in \code{\link{`NewFindK`}}. Useful if some areas have very low or high predicted values in `cell_pred`,
#'                   such that some draws are either 0 or 1
#' 
#' @return Numeric value, raking value for a given location-year
#' 
#' @family Logit Raking
...
#' @title NewPixelVal
#' @description Add a value in logit space. converts `vals` to logit space, adds `K`, then applies inverse logit. Used in logit raking (\code{\link{LogitFindK}}) to apply a raking factor (`K`) to a pixel value. 
#'
#' @param K numeric, value to add in logit space
#' @param vals numeric, value that gets transformed into logit space
#' 
#' @return Numeric
#' 
#' @family Logit Raking
...
#' @title NewEst
#' @description Gets the weighted mean of a matrix `vals` with a vector of weights `weightval` and returns the mean of those values. In practice is used to aggregate draws then collapse the draws to get a national average from pixel values. Used in logit raking (\code{\link{EvalDiff}} and \code{\link{LogitFindK}}) to check how close a raking factor is to the target value.
#'
#' @param vals matrix, i.e. cell draws
#' @param weightval vector of weights 
#' 
#' @return Numeric, weighted mean of draws
#' 
#' @family Logit Raking
...
#' @title EvalDiff
#' @description Checks how close a raking factor is to the raking target (`gbd_val`). The raking fractor is calculated using \code{\link{NewEst}}. Used in logit raking (\code{\link{LogitFindK}}).
#'
#' @param gbdval numeric, gbd raking target
#' @param vals matrix, i.e. cell draws
#' @param weightval numeric vector of weights 
#' 
#' @return Numeric value, difference between `gbdval` and raking factor (calculate by `NewEst`)
#' 
#' @family Logit Raking
...
#' @title cellIdx
#' @description Given a raster `x`, returns a vector of ids for the non-na cells in the raster. Originally from the `seegMBG` library, included here to reduce outward dependencies.
#'
#' @param x a raster
#' 
#' @return a vector of ids for non-na pixels in raster
