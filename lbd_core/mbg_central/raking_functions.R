#' @description A function to rake mbg ouputs to GBD estimates. Can rake to either national level or subnational level (where available). 
#'              Supports linear and logit raking. Optionally outputs summary rasters of raked cell pred object. 
#' 
...
#' @param rake_subnational boolean default `T`. If true, uses the subnational raking shapefile, otherwise uses the adm0 shapefile. 
#' @param crosswalk Boolean default `T`, for models run before the new gaul shapefiles, the cell_pred needs to be crosswalked to match the subnational raking raster.
#' @param shapefile_path character string -Path to shapefile that will be used for raking.
#' @param field character string - Field in shapefile that has admin identifiers that match with rake_to. 
#' @param zero_heuristic Boolean default `F`.  If logit raking, this will automatically set rf = -9999 for any country-year with a target value of 0.
#' @param approx_0_1 Boolean default `F`. If logit raking, any values of zero will be replaced with 1e-10 and values of 1 will be replaced with (1-(1e-10)).
#' @param simple_raster default `NULL`, option to pass in simple raster to function if it's been loaded already.
#' @param simple_polygon default `NULL`, option to pass in simple polygon if its been loaded already. 
#' @param pop_raster default `NULL`, option to pass in pop raster if its been loaded already. 
#' 
#' Additional Parameters (...)
#' @param if_no_gbd default `return_na`, other option `return_unraked`. 
...
#' @param if_no_gbd default `return_na`, other option `return_unraked`. If return_na, any location-years without gbd estimates will return NA raking factors.
#' 
...
rake_cell_pred <- function(cell_pred, 
  ...
  if(is.null(pop_raster)){
    ...
    ## pull annual population brick using covariates function
...
#' @param save_vec default `NULL`. If null, uses names from subnational_rake_output
#' @param save_draws_as character string default `RDS`. additional supported arguments `rds` and `Rdata`. The file type to save raked draws
#'
#' @return NULL
#'
#' @examples    save_custom_raking_outputs(custom_rake_output,
#'                                         outdir = sprintf("<<<< FILEPATH REDACTED >>>>", indicator, run_date),
#'                                         indicator,
#'                                         age_group,
#'                                         prefix = "custom_india",
#'                                         reg = "south_asia")
...
#' @note this function loops through each pixel in the rasters and compares them. 
#'
...
#' @param lyv character vector of length 3. Denotes the gaul_code column, year column and value column
...
#' @param iterate default `F`. 
#' @param zero_heuristic default `F`.  If logit raking, this will automatically set rf = -9999 for any country-year with a target value of 0. 
#' @param approx_0_1 default `F`. If logit raking, any values of zero will be replaced with 1e-10 and values of 1 will be replaced with (1-(1e-10)).  
#' 
#' @usage a = calculate_raking_factors(cell_pred, aaa[[1]], year_list = 2000:2017, simple_raster = simple_raster, weight_brick = pop_raster_annual, rake_method = 'linear')
#' @usage b = calculate_raking_factors(cell_pred, aaa[[1]], year_list = 2000:2017, simple_raster = simple_raster, weight_brick = pop_raster_annual, rake_method = 'logit')
#' @import data.table
#' 
    ...
    rf = lapply(seq(nrow(cys)), function(x) {
      ...
      if (nrow(rake_targets[loc == theloc & year == theyear]) == 0) {
        ...
        ret <- try(LogitFindK(gbdval     = rake_targets[loc == theloc & year == theyear,.(target)],
                              pixelval   = cell_pred[raker[loc == theloc & year == theyear, cell_pred_id],], 
...
#' @param rake_dt data.table Data.table output of calculate_raking_factors. 
