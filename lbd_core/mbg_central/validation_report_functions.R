...
report_each_gaul <- function(gaul, quilt_plot_list, target_plot_list, shapefile_version = 'current') {
  ...
  if(indicator == 'edu_mean') {
    ...
    dhs_admin1_data <- fread(<<<< FILEPATH REDACTED >>>>)
...
summarize_admin2 <- function(gaul, indicator, indicator_group, run_date, nperiod, master_list) {
  ...
  ## Make plots
    rfs <- fread(paste0(<<<< FILEPATH REDACTED >>>>))
...
plot_varios_and_violins <- function(indicator,
  ...
  default_raked_results_path <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  results_raked <- brick(default_raked_results_path)
  default_results_path <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  ...
  default_raked_results_path <- paste0(<<<< FILEPATH REDACTED >>>>)
  results_raked <- brick(default_raked_results_path)
  default_results_path <- paste0(<<<< FILEPATH REDACTED >>>>)
  results <- brick(default_results_path)
...
raking_factors_map <- function(indicator,
  ...
  # extract admin0
  master_shape <- readOGR(dsn=<<<< FILEPATH REDACTED >>>>, layer='africa_ad0')
  ...
  rf <- fread(paste0(<<<< FILEPATH REDACTED >>>>))
...
is_oos_preds <- function(rd = run_date,
                         all.data = df,
                         cell_draws_filename = <<<< FILEPATH REDACTED >>>>, ## in sprintf notation
...
load_region_input_data <- function(indicator_group, indicator, run_date, reg) {
  ...
  model_path <- paste0(<<<< FILEPATH REDACTED >>>>)
