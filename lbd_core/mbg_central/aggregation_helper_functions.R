...
process_input_data <- function(df, 
  ...
  if(poly_ag) { # when using new polygon aggregation method
    ...
    message('make shape table')
    shape_table <- unique(df[, .(shapefile, location_code, country, point, z_ag, year, ANC, sex_id)])
    ...
    if(nrow(shape_table) > 0) {
      ...
      for(shape in unique(shape_table$shapefile)) {
        ...
        data_types <- unique(shape_table[shapefile==shape,ANC])
        
        if (length(data_types)==2) message(paste0('Sources for shape data in ', shape, ' are ANC & other'))
        if (length(data_types)==1 & data_types[1]==F) message(paste0('Sources for shape data in ', shape, ' are other'))
        if (length(data_types)==1 & data_types[1]==T) message(paste0('Sources for ag data in ', shape, ' are ANC'))
        ...
        shape_table <- process_single_shapefile(shape, shape_table, ref_raster, 
                                                ...
                                                auto_disaggregate, data_types[i])
        }
      }
      # add the new data back onto the datatable
      df <- merge(df, shape_table, by = c("shapefile", "location_code", "point", "z_ag", "year", "sex_id", "ANC", "country"), all.x = T)
  ...
  age_table <- unique(df[, .(country, latitude, longitude, ANC, z_ag, z_ag_id, year, sex_id)])
  ...
  if(nrow(age_table) > 0 ) {
    ...
    if(nrow(age_table) > 0) { # will need to use worldpop data
      age_table <- unique(age_table[, .(country, latitude, longitude, ANC, sex_id, z_ag, year)])
      for(reg in unique(age_table$country)) {
        message('working on age ag data for ', reg)
        ...
        #Set up use of ANC data sources (data from ANC gets special weighting, data from other sources goes through standard pop weighting)
        data_types <- unique(age_table[,ANC])
        if (length(data_types)==2) message(paste0('Sources for ag data in ', reg, ' are ANC & other'))
        if (length(data_types)==1 & data_types[1]==F) message(paste0('Sources for ag data in ', reg, ' are other'))
        if (length(data_types)==1 & data_types[1]==T) message(paste0('Sources for ag data in ', reg, ' are ANC'))
        for(ages in age_gps) {
          message('Working on age_gps ', ages)
          for(i in 1:length(data_types)) {
            age_table <- process_single_z_gp(ages, reg, age_table, z_map,
                                             simple_polygon,
                                             pop_release, interval_mo,
                                             type="age", data_types[i])
          }
        }
      }
      # add the new data back onto the datatable
      df[age_table, on= c("country", "latitude", "longitude", "z_ag", "year", "sex_id", "ANC"), 
         c("z","agg_weight"):=list(i.z,i.agg_weight)] 
    }
  }
...
process_single_z_gp <- function(ages, reg, age_table, z_map,
                                ...
                                pop_release, interval_mo,
                                type="age", dt) {
  connector        <- get_gbd_locs(rake_subnational = FALSE, reg = reg, shapefile_version=shapefile_version)
  # User does not specify weights
  # Need to read in from worldpop
  if(type == "age") {
    
    ages_needed <- eval(parse(text=as.character(ages)))
    for(sex_needed in unique(age_table[z_ag == ages & country == reg & ANC == dt]$sex_id)) {
      if(sex_needed==1) x = 'm' else if(sex_needed==2) x = 'f' else x = 't'
      message(paste0('working on sex ', x))
      for(year_needed in unique(age_table[z_ag == ages & country == reg & ANC == dt & sex_id == sex_needed]$year)) {
        message(paste0('working on year ', year_needed))
        worldpop_cov_rasters <- lapply(ages_needed, function(age) {
           suppressMessages(load_worldpop_covariate(simple_polygon,
                                                  covariate = 'worldpop',
                                                  pop_measure = paste0("a",z_map$value[z_map$z == age],x),
        ...
        longlats <- age_table[country==reg & z_ag==ages & year == year_needed & ANC == dt & sex_id == sex_needed, .(longitude, latitude)]
        loc_rel <- SpatialPoints(as.matrix(longlats), 
                                 crs(simple_polygon))
        pop_vals <- raster::extract(worldpop_stack, loc_rel)
        
        #For ANC data, multiply the population value by the fertility rate for that agebin
        if(dt){
          source("<<<< FILEPATH REDACTED >>>>get_covariate_estimates.R")
          asfr <- get_covariate_estimates(covariate_id = 13, gbd_round_id = 6, status = "best",
                                          sex_id = 2, age_group_id = ages_needed + 7, year_id = year_needed,
                                          location_id = connector$location_id, decomp_step='step4')
          asfr <- asfr$mean_value
          if(any(is.na(asfr))) stop(paste0('Error: asfr from ', reg, 'and year ', year, 'and sex ', sex_needed, ' is NA'))
          pop_vals <- pop_vals * asfr
        }
        
        #Set pop_vals to sum to one
        if(any(is.na(pop_vals))) {
          stop(paste0("There are missing population values for country: ", reg,
                      " in year ", year_needed, 'and sex ', sex_needed))
        }
        ...
        for(i in 1:nrow(longlats)) {
          age_table[country==reg & 
                      z_ag==ages & 
                      year == year_needed & 
                      longitude==longlats$longitude[i] &
                      latitude==longlats$latitude[i] & 
                      sex_id == sex_needed & 
                      ANC == dt, 
...
process_single_shapefile <- function(shape, shape_table, ref_raster, fast_shapefiles, 
                                     ...
                                     disaggregation_factor, auto_disaggregate, dt) {
  message(paste0('Working on ANC data: ', dt))
    ...
    if(nrow(coords) != 0 ) {
      ...
      for(ag_age in ag_ages) {
        if(is.na(ag_age)) {
          ...
          ages_needed <- NA
          sexes <-shape_table[shapefile == shape & location_code == code & is.na(z_ag) & ANC == dt]$sex_id
          countries <-shape_table[shapefile == shape & location_code == code & is.na(z_ag) & ANC == dt]$country
        } else {
          years <- shape_table[shapefile == shape & location_code == code & z_ag == ag_age & ANC == dt]$year
          ages_needed <- eval(parse(text=as.character(ag_age)))
          sexes <- shape_table[shapefile == shape & location_code == code & ANC == dt & z_ag == ag_age ]$sex_id
          countries <-shape_table[shapefile == shape & location_code == code & z_ag == ag_age & ANC == dt]$country
        }
        
        message(paste0('Countries: ', unique(countries)))
        message(paste0('Sexes: ', unique(sexes)))
        message(paste0('Years: ', unique(years)))
        
        for(country_needed in unique(countries)) {
          connector        <- get_gbd_locs(rake_subnational = FALSE, reg = country_needed, shapefile_version=shapefile_version)
        
        for(sex_needed in unique(sexes)) {
          if(sex_needed==1) x = 'm' else if(sex_needed==2) x = 'f' else x = 't'

          for(year_needed in unique(years)) {
             # Get population values
              worldpop_cov_rasters <- lapply(ages_needed, function(age) {
                                  suppressMessages(load_worldpop_covariate(sub_raster,
                                                    covariate = 'worldpop',
                                                    pop_measure = ifelse(is.na(age), "total", paste0("a",z_map$value[z_map$z == age],x)),
                                                    pop_release = pop_release,
                                                    start_year = year_needed,
                                                    end_year = year_needed,
                                                    interval = as.numeric(interval_mo))$worldpop)
          ...
          #For ANC data, multiply the population value by the fertility rate for that agebin
          if(dt){
            if (!is.na(ages_needed)) {
            source("<<<< FILEPATH REDACTED >>>>/get_covariate_estimates.R")
            asfr <- get_covariate_estimates(covariate_id = 13, gbd_round_id = 6, status = "best",
                                            sex_id = 2, age_group_id = ages_needed + 7, year_id = year_needed,
                                            location_id = connector$location_id, decomp_step='step4')
            asfr <- asfr$mean_value
            if(any(is.na(asfr))) stop(paste0('Error: asfr from ', shape, 'and year ', year, 'and sex ', sex_needed, ' is NA'))
            pop_vals <- pop_vals * asfr
          }
          }
          ...
          if(is.na(ag_age)) {
            shape_table[shapefile == shape & location_code == code & is.na(z_ag) & year==year_needed & ANC == dt & sex_id == sex_needed & country == country_needed, 
            ...
            shape_table[shapefile == shape & location_code == code & z_ag == ag_age & year==year_needed & ANC == dt & sex_id == sex_needed & country == country_needed, 
                        c("coordinates.long", "coordinates.lat", "agg_weight","z") := 
                          list(list(rep(coords[,1], length(ages_needed))),
                               list(rep(coords[,2], length(ages_needed))),
                               list(pop_vals),list(age_vals))]
