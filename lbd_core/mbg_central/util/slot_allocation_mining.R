...
saveRDS(dat, file = "<<<< FILEPATH REDACTED >>>>")
...
train.date <- '20181018'
train.dat <- readRDS("<<<< FILEPATH REDACTED >>>>")
...
png("<<<< FILEPATH REDACTED >>>>", width = 10, height = 8, res = 300, units = 'in')
print(gg.ram)
dev.off()
...
test.date <- '20181022'
test.dat <- readRDS("<<<< FILEPATH REDACTED >>>>")
...
png("<<<< FILEPATH REDACTED >>>>", units = 'in', res = 300, height = 9, width = 12)
par(mfrow = c(4, 3))
...
save(res.sd, percent.over, f, ace.r, file = "<<<< FILEPATH REDACTED >>>>")
