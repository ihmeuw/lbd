...
#' @export
#'
make_qsub_share <- function(user = 'USERNAME',
                            code = NULL,
                            code_path = NULL,
                            cores = 2,
                            memory = 100,
...
#' @return Returns a qsub string
#' @export
make_qsub_postest <- function(user = 'USERNAME',
                              code,
                              code_path = NULL,
                              cores = 2,
                              memory = 100,
...
plot_raster <- function(ras,
  ...
  if (region == 'africa') {
    background_shp <- readOGR("FILEPATH")
  } else {
...
check_config <- function(cr = core_repo) {
  ...
  must_haves <- read.csv(paste0('FILEPATH'))$V1
...
parallelize <- function(user = 'USERNAME',
                        slots,
                        memory,
                        script,
                        proj             = NULL,
...
#' (\'FILEPATH') if a fork is being used.
#'
