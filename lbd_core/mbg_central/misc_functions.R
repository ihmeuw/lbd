...
pullupstream<-function(){
  system('cd <<<< FILEPATH REDACTED >>>>/lbd_core\ngit pull origin master')
}
...
waitforpostesttofinish <- function(sleeptime = 100,
  ...
  path <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date, "/temp_post_est/")
...
waitforresultstable <- function(st = strata,
  ...
  while(length(r_left) > 0 & length(u_left) > 0) {
    ...
    results_dir_r <- paste0(<<<< FILEPATH REDACTED >>>>'/output/', rd, '/table_', baseline_year, '/')
    results_dir_u <- paste0(<<<< FILEPATH REDACTED >>>>'/output/', rd, '/table_', baseline_year, '_unraked/')
...
waitforaggregation <- function(rd = run_date,
  ...
  if (is.null(dir_to_search)) {
    dir_to_search <- paste0(<<<< FILEPATH REDACTED >>>>"/output/",run_date,"/")
  }
...
combine_aggregation <- function(rd = run_date,
  ...
  for(rake in raked) {
    for (holdout in holdouts) {
      for (age in ages) {
        for(sex in sexes) {
          message(paste0("\nWorking on age: ", age, " | sex:", sex, " | holdout: ", holdout, " | raked: ", rake))
        ...
        for (reg in regions) {
          ...
          load(paste0(dir_to_search, indic, "_", ifelse(rake == 'raked', "raked", ifelse(rake == 'raked_c', "raked_c", "unraked")),
                      "_admin_draws_eb_bin", age, "_sex", sex, "_", reg, "_", holdout, ".RData"))
        ...
        save(admin_0, admin_1, admin_2, sp_hierarchy_list,
             file = paste0(dir_to_search, indic, "_",
                           ...
                           "_admin_draws_eb_bin", age, "_sex", sex, "_",
                           holdout, ".RData"))
      }
    }
  }
  ...
  if (delete_region_files == T) {
    ...
    combos <- expand.grid(ifelse(rake == 'raked', "raked", ifelse(rake == 'raked_c', "raked_c", "unraked")), ages, sexes, holdouts)
    ...
    if (all(file.exists(files_to_check))) {
      ...
      combos <- expand.grid(ifelse(rake == 'raked', "raked", ifelse(rake == 'raked_c', "raked_c", "unraked")), ages, sexes, regions, holdouts)
...
make_qsub_share <- function(user = Sys.info()["user"],
  ...
  qsub <- generate_qsub_command(
    ...
    priority = priority,
    # Command to qsub
    shell, code, reg, age, rd, as.numeric(test), holdout, indic, ig, pred_chunk, "fin"
  )
...
combine_region_image_history <- function(indicator, indicator_group, run_date, fixed_effects) {
  ...
  load(paste0(<<<< FILEPATH REDACTED >>>>'/model_image_history/', run_date, '_bin0_wssa_0.RData'))
  new_cov_list <- list()
  for(cov in names(cov_list)[!grepl("gaul_code", names(cov_list))]) {
    pull_raster_covs <- function(region) {
      load(paste0(<<<< FILEPATH REDACTED >>>> '/model_image_history/', run_date, '_bin0_', region, '_0.RData'))
  ...
  save(list = c('df','covs',grep('^tv_*', ls(), value = TRUE)), file = paste0(<<<< FILEPATH REDACTED >>>> '/model_image_history/', run_date, '.RData'))
...
cleanup_inla_scratch <- function(run_date) {
  ...
  if(keep_inla_files==TRUE) {
    ...
    message(paste0('Files stored here: <<<< FILEPATH REDACTED >>>>/inla_', run_date))
...
region_violin <- function(indicator,
  ...
  results_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date)
...
plot_raster <- function(ras,
  ...
  package_lib <- paste0(<<<< FILEPATH REDACTED >>>>'/packages') # Library for all MBG versioned packages.
  ...
  if (region == 'africa') {
    background_shp <- readOGR(dsn="<<<< FILEPATH REDACTED >>>>/data_coverage", layer="africa_ad0")
  } else {
...
graph_run_summary <- function(run_date,
  ...
  dir <- paste0(<<<< FILEPATH REDACTED >>>> "/output/", run_date, "/")
...
clean_model_results_table <- function(rd   = run_date,
  ...
  sharedir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', rd, '/')
...
model_fit_table <- function(lv=loopvars[loopvars[,3]==0,],
  ...
  for(i in 1:nrow(lv)){
    ...
    f=paste0(<<<< FILEPATH REDACTED >>>>'/model_image_history/', rd, pathaddin, ".RData")
...
set_up_config <- function(repo, 
  ...
  if (is.null(config_name) & is.null(config_file)) {
    ...
    if (post_est_only == TRUE) 
      config <- data.table::fread(paste0(<<<< FILEPATH REDACTED >>>> "/output/", run_date, "/config.csv"))
  }
...
#'
#' # Not run
#' a1_df <- fread("<<<< FILEPATH REDACTED >>>>/pred_derivatives/admin_summaries/[indic]_admin_1_raked_summary.csv")
...
#' (\code{<<<< FILEPATH REDACTED >>>>/lbd_core}) if a fork is being used.
#'
...
get_output_regions <- function(in_dir) {
  return(unique(stringr::str_match(list.files(in_dir, pattern = paste0('_cell_draws_eb_')),
                                   '_cell_draws_[^_]+_[^_]+_[^_]+_(.*)_[0-9].RData')[,2]))
}
...
delete_model_outputs <- function(
  ...
  fileDF <- expand.grid(ind=indicator, ig=indicator_group, rd=run_date) %>%
    mutate(moddir=file.path(<<<< FILEPATH REDACTED >>>> 'output', rd))
...
load_stage_list <- function() {
  if (.in.package()) {
    ...
    data.table::fread("<<<< FILEPATH REDACTED >>>>/stage_master_list.csv")
  }
}
