...
waitformodelstofinishu5m<- function(sleeptime=100,
  ...
  paths <- list()
  if(ageasindic) {
    for(a in unique(lv[,2])) paths[[a]]<-paste0(fp_list['mbg_root'], indicator_group, '/', indicator,'_age',a,'/output/', run_date,'/')
  } else {
    paths[[1]] <-  paste0(fp_list['mbg_root'], indicator_group, '/', indicator, '/output/', run_date,'/')
  }
