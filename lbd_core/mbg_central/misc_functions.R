...
waitformodelstofinishu5m<- function(sleeptime=100,
  ...
  paths <- list()
  if(ageasindic) {
    for(a in unique(lv[,2])) paths[[a]]<-paste0('<<< FILEPATH REDACTED >>>')
  } else {
    paths[[1]] <-  paste0('<<< FILEPATH REDACTED >>>>')
  }
...
waitforresultstable <- function(st = strata,
  ...
  while(length(r_left) > 0 & length(u_left) > 0) {
    ...
    results_dir_r <- paste0('<<< FILEPATH REDACTED >>>>')
    results_dir_u <- paste0('<<< FILEPATH REDACTED >>>>')
...
get_singularity <- function(image = 'default') {
  if(image == 'default') {        # use default image
    ...
    sing_image <- paste0('<<< FILEPATH REDACTED >>>')
  }
...
#'   for LBD are kept at the default location of <<< FILEPATH REDACTED >>>
#'   [default = NULL]
...
make_qsub <- function(user                     = Sys.info()['user'],
  ...
  if(saveimage==TRUE)  save.image(paste0('<<< FILEPATH REDACTED >>>'))
  ...
  if(log_location=='sgeoutput')
    logloc = sprintf('<<< FILEPATH REDACTED >>>>')
  ...
  if(geo_nodes == TRUE){
    ...
    shell     <- sprintf('%s/mbg_central/share_scripts/shell_geos.sh',corerepo)
  }else{
    proj      <- "proj_geospatial"
    node.flag <- ""
    shell     <- sprintf('%s/mbg_central/share_scripts/shell_prod.sh',corerepo)
  }
...
plot_raster <- function(ras,
  ...
  if (region == 'africa') {
    background_shp <- readOGR(dsn="<<< FILEPATH REDACTED >>>")
  } else {
    #
...
proportional_area_map <- function(data, # Needs to have ADM0_CODE, ADM1_CODE, ADM2_CODE
  ...
  background_shp <- readRDS('<<< FILEPATH REDACTED >>>')
  background_map <- fortify(background_shp)

  message("Loading master admin shape...")
  if (ad_level == 2) {
    ad_shape <- readRDS("<<< FILEPATH REDACTED >>>")
...
#'                   [default = FALSE]
#' @param singularity instead of using the default R installation on the geos
#'                    or prod nodes, launch R from a Singularity image? This
#'                    arg currently takes three options: the default is NULL
#'                    indicating not to launch a Singularity container,
#'                    'default' if you wish to launch a Singularity container
#'                    from the default image, or you can provide a string which
#'                    can be either a complete path to a Singularity image that
#'                    is not located at the default image location or just the
#'                    name of the Singularity image that is assumed located at
#'                    the default image location.  If 'default' is chosen, the
#'                    default image is defined in the shell script executed by
#'                    this R script ('shell_sing.sh') so that no R code need be
#'                    updated when the default image is updated.  Different
#'                    versions of a Singularity image or test versions may be
#'                    specified by providing the name or path of the image.
#'                    Currently, all standard images for LBD are kept at the
#                     default location of <<< FILEPATH REDACTED >>>
#'                    [default = NULL]
#' @param singularity_opts pass in a named list of environmental variables.
#'   \code{qsub_sing_envs} will check that the names of the list members passed
#'   in match the environmental variables that the shell_sing.sh script knows
...
parallelize <- function(user = Sys.info()['user'],
  ...
  # Convoluted project/cluster matching and pointing to the correct shell
  # script. On geos nodes, always use 'proj_geo_nodes'. When on prod, use
  # 'proj_geospatial' by default but use user defined project name if prod
  # is requested and a different project is supplied. For each cluster, use the
  # appropriate shell script located in the lbd core repo.
  if(geo_nodes) {
    proj      <- 'proj_geo_nodes'
    node.flag <- '-l geos_node=TRUE'
    shell     <- paste0(corerepo, '/mbg_central/share_scripts/shell_geos.sh')
  } else {
    if(is.null(proj)) proj <- "proj_geospatial"
    node.flag <- ""
    shell     <- paste0(corerepo, '/mbg_central/share_scripts/shell_prod.sh')
  }
  gn <- as.character(geo_nodes)
...
monitor_jobs <- function(parallelize_output,
  ...
  while(n_finished < nrow(lv)) {
    ...
    check_if_nonzero_exit <- function(lv) {
      for (i in 1:nrow(lv)) {
        es <- lv[i, 'exit_status']
        if (!is.na(es) & (es != 0)) {
    ...
    lv <- check_if_nonzero_exit(lv)
    Sys.sleep(sleeptime)
  } # close while loop
  # Exiting function...
  if (return_lv == T) return(lv)
  if (keep_temp_file == F) unlink(paste0("<<< FILEPATH REDACTED >>>"))
}
...
load_from_parallelize <- function(fname = as.character(commandArgs()[4]),
  ...
  # Load loopvars
  this_lv <- subset(lv[rownumber, ], select = -jobname)

  # Assign loopvars
  for (n in names(this_lv)) {
    assign(n, this_lv[1, get(n)], envir = .GlobalEnv)
