...
pullupstream<-function(){
  system('cd <<<< FILEPATH REDACTED >>>> pull origin master')
}
...
waitformodelstofinish <- function(sleeptime=100,
      ...
      lv[, file := paste0(<<<< FILEPATH REDACTED >>>>)]
...
lv[, filename := paste0(<<<< FILEPATH REDACTED >>>>)]
...
make_qsub_share <- function(user = Sys.info()["user"],
  ...
  if (is.null(code)) {
    code <- sprintf(<<<< FILEPATH REDACTED >>>>)
  } else {
    code <- sprintf(<<<< FILEPATH REDACTED >>>>)
  }
...
region_violin <- function(indicator,
  ...
  preds <- raster(paste0(<<<< FILEPATH REDACTED >>>>))
  extract_region <- function(region) {
    input_data <- fread(paste0(<<<< FILEPATH REDACTED >>>>))
...
clean_model_results_table <- function(rd   = run_date,
  ...
  write.csv(all_mods, paste0(<<<< FILEPATH REDACTED >>>>),
            row.names = F)
...
check_config <- function(cr = core_repo) {
  ...
  # data(must_haves)
  must_haves <- read.csv(paste0(<<<< FILEPATH REDACTED >>>>), header = F, stringsAsFactors = F)$V1
...
set_up_config <- function(repo, 
  ...
  if (.in.package()) {
    ...
    default_config_values <- data.table::fread(file.path(<<<< FILEPATH REDACTED >>>>), header = TRUE, stringsAsFactors = FALSE)
  }
  ...
  if(run_tests) {
    ...
    if (.in.package()) {
      ...
      config_tests <- data.table::fread(paste0(<<<< FILEPATH REDACTED >>>>), header = TRUE, stringsAsFactors = FALSE)
    }
...
parallelize <- function(user = Sys.info()['user'],
  ...
  if(is.null(script_dir)) script_dir <- paste0(<<<< FILEPATH REDACTED >>>>)
...
monitor_jobs <- function(parallelize_output,
  ...
  if (keep_temp_file == F) unlink(<<<< FILEPATH REDACTED >>>>))
}
