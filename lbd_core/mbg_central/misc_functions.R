...
waitformodelstofinishu5m<- function(sleeptime=100,
      ...
      paths <- list()
      if(ageasindic) {
        for(a in unique(lv[,2])) paths[[a]]<-'<<<< FILEPATH REDACTED >>>>'
      } else {
        paths[[1]] <-  '<<<< FILEPATH REDACTED >>>>'
      }
...
waitforresultstable <- function(st = strata,
  ...
  while(length(r_left) > 0 & length(u_left) > 0) {
    ...
    results_dir_r <- '<<<< FILEPATH REDACTED >>>>'
    results_dir_u <- '<<<< FILEPATH REDACTED >>>>'
...
region_violin <- function(indicator,
  ...
  preds <- raster('<<<< FILEPATH REDACTED >>>>')
...
check_config <- function(cr = core_repo) {
  must_haves <- read.csv('<<<< FILEPATH REDACTED >>>>', header = F, stringsAsFactors = F)$V1
  ...
  for(confs in must_haves){
    ...
    } else if (confs == "geo_re_prior") {
      message("You are missing a 'geo_re_prior' argument in your config. Defaulting to 'list(prior = 'loggamma', param = c(2, 1))'")
      geo_re_prior <<- "list(prior = 'loggamma', param = c(2, 1))"
      message(paste0('  ', confs, ': ', get(confs)))
    ...
    } else if (confs == "use_geo_res") {
      message("You are missing a 'use_geo_res' argument in your config. Defaulting to FALSE")
      use_geo_res <<- FALSE
      message(paste0('  ', confs, ': ', get(confs)))
...
set_up_config <- function(repo, 
  ...
  if (is.null(config_name) & is.null(config_file)) {
    ...
    if (post_est_only == FALSE) 
      config <- fread('<<<< FILEPATH REDACTED >>>>', header = FALSE)
  ...
  if (!is.null(covs_name) & is.null(covs_file)) {
    ...
    covs <- fread('<<<< FILEPATH REDACTED >>>>', header = TRUE)
  }
  ...
  config_values <- fread('<<<< FILEPATH REDACTED >>>>'), header = TRUE, stringsAsFactors = FALSE)
  ...
  if(run_tests) {
    ...
    config_tests <- fread('<<<< FILEPATH REDACTED >>>>', header = TRUE, stringsAsFactors = FALSE)
...
#' ('<<<< FILEPATH REDACTED >>>>') if a fork is being used.
#'
...
delete_model_outputs <- function(
  ...
  fileDF <- expand.grid(ind=indicator, ig=indicator_group, rd=run_date) %>%
    mutate(moddir=file.path('<<<< FILEPATH REDACTED >>>>'))
