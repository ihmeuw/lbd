...
#   Returns: Input data subset to modeling area.
   load_input_data <- function(indicator, agebin = 0, removeyemen = FALSE, pathaddin = "",
    ...
    if(withdate)             filename <- paste0(root,<<<< FILEPATH REDACTED >>>>)
    ...
    if(update_run_date == TRUE) {
      if(dir.exists(paste0(<<<< FILEPATH REDACTED >>>> "/output/", run_date)) == TRUE) {
        existing_dir <- paste0(<<<< FILEPATH REDACTED >>>> "/output/", run_date)
...
#'
#' @param shapfile_version String indicating which shapefile version is being used. Used for 
#'   for checking which version of the empty_world_raster should be pulled
#'
...
build_simple_raster_pop <- function(subset_shape,
  ...
  pop_rast <- brick(paste0(<<<< FILEPATH REDACTED >>>>, pop_release, "/1y/worldpop_total_1y_2010_00_00.tif"))
...
#'
set_root<-function(){
  root<<-<<<< FILEPATH REDACTED >>>> # Setting "root" as global variable
}
