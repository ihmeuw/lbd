...
#' @title Make Time Stamp
#'
#' @description Make time stamp (typically used as `run_date`) of format YYYY_MM_DD_hh_mm_ss
#'
#' @param time_stamp Boolean, if FALSE returns 'scratch'.
#'
#' @return a time stamp of format YYYY_MM_DD_hh_mm_ss
...
#' @title Redirect Load Config
#'
#' @description Overloads the deprecated load_config to point to \code{\link{set_up_config}}. This function exists for the sake of backwards compatibility.
#'
#' @param ... args to be passed to `set_up_config`
#'
#' @return None
...
#'
#' @description Creates the standard directory structure for an indicator/indicator group (/{indicator_group}/{indicator}/output and /{indicator_group}/{indicator}/model_input_history).
#'
#' @param indicator_group Category of indicator, e.g. "education"
#' @param indicator Specific outcome to be modeled within indicator category, e.g. "edu_0"
#'
#' @return None
create_dirs <- function(indicator_group, indicator) {

  dir.create(paste0(fp_list['mbg_root'], indicator_group))
  dir.create(paste0(fp_list['mbg_root'], indicator_group, '/', indicator))

  indicator_dir <- paste0(fp_list['mbg_root'], indicator_group, '/', indicator)
...
#' @title Get Template Raster
#'
#' @description Make template rasters (pull from central analysis folders managed by Lucas based off the area to model specified in config). Currently limited to Stage 2 (low and middle income countries).
#'
#' @param simple Spatial object used to crop and mask the template raster. This can be a raster or a SpatialPolygons object. Note that masking a raster with a polygon will only take pixels whose centroid is covered by a polygon. 
#'
#' @return Empty raster over modeling area. To be used for cropping covariates quickly and projecting model.
get_template_raster <- function(simple) {
  ...
  analysis_raster_dir <- paste0(fp_list$mbg_store,'central_spatial_files/analysis_integer_rasters/')
...
#' @param indicator_group indicator_group for modelling run. Defaults to `indicator_group` in global environment
#' @param agebin Integer, default 0. When not 0, subsets input data to this agebin, assuming the input data has a column `age`. 
#' @param removeyemem Boolean default F. If T, removes Yemen data from dataset by subsetting `country` column by `Yemen` and `YEM`. Originated from discussion over whether to include Yemen in Stage 1 with Africa.
#' @param pathaddin character default "". Additional information to add to the filename of the saved out data. Typically `paste0('_bin',age,'_',reg,'_',holdout)`.
#' @param withdate Boolean default F. If FALSE, looks for `run_date` from global environment. If T, assigns `run_date` based on the `date` param. Will try to load data from a dated input data folder of structure `fp_list$mbg_store/input_data/dated/{rd}/{indicator}`.
#' @param date character default "". If `withdate` is T, uses this value as the run date for loading and saving the data. 
#' @param years character default 'five_year'. Will bin the `year` column in the data to 5 year bins centered around 2000, 2005, 2010, and 2015. If set to anything else will not do anything. Legacy of a time where we modeled in 5-year bins rather than yearly. 
...
#' @param withtag boolean default F. Determines where to load data from in conjunction with `with_date` and `use_share`. 
#' withtag == T: `filename <- paste0(load_dir, indicator, datatag)`
#' withtag == F & with_date == F: `filename <- paste0(load_dir, indicator)`
#' withtag == F & with_date == T: `filename <- paste0(fp_list$mbg_store,'input_data/dated/',rd,'/', indicator).`
#' @param datatag character default "". See use in `withtag`
#' @param use_share boolean default F. Where to load data from (see `load_dir` in `withtag`)
#' use_share == T: `load_dir  <- file.path(fp_list['mbg_root'], 'input_data/')`
#' use_share == F: `load_dir <- paste0(fp_list$mbg_store,'input_data/')`
#' @param yl numeric vector, default `year_list` from global env. Will drop any years of data outside the min/max of this vector. 
#' @param region character, default NULL. If regions/countries are provided (see `get_adm0_codes` for acceptable values) subsets the input data on the `ADM_CODE` column. Else not subset by region.
#' @param poly_ag Boolean, defaults to `poly_ag` in global env. Set in the config and used in polygon integration models. If T, only data where point == 0 and poly_ag is T will be kept
#' @param zcol_ag Boolean, defaults to `zcol_ag` in global env. If T runs a check to ensure that the input data has non-aggregated data. 
#'
#' @return cleaned data.table of input_data. If `update_run_date == T`, returns a list with the data and the new run date.
load_input_data <- function(indicator, 
  ...
  if(use_share==FALSE) load_dir <- paste0(fp_list$mbg_store,'input_data/')
  if(use_share==TRUE) load_dir  <- file.path(fp_list['mbg_root'], 'input_data/')

  if(!withdate & !withtag) filename <- paste0(load_dir, indicator)
  if(withtag)              filename <- paste0(load_dir, indicator, datatag)
  if(withdate)             filename <- paste0(fp_list$mbg_store,'input_data/dated/',rd,'/', indicator)
  ...
  if(update_run_date == TRUE) {
    if(dir.exists(paste0(fp_list['mbg_root'], indicator_group, '/', indicator, "/output/", run_date)) == TRUE) {
      existing_dir <- paste0(fp_list['mbg_root'], indicator_group, '/', indicator, "/output/", run_date)
    ...
    if(dir.exists(paste0(fp_list['mbg_root'], indicator_group, '/', indicator, "/output/", run_date)) == FALSE) {
      run_date_dir <- paste0(fp_list['mbg_root'], indicator_group, '/', indicator, "/output/", run_date)
  ...
  if(update_run_date == FALSE) {
    if(agebin==0){
      ...
      run_date_dir <- paste0(fp_list['mbg_root'], indicator_group, '/', indicator, "_age",agebin,"/output/", run_date)
      dir.create(run_date_dir, showWarnings = FALSE)
      write.csv(d, file=paste0(run_date_dir, "/input_data", pathaddin, ".csv"))
    }
    return(d)
...
#' @title Load Simple Polygon
#' @description Loads in a shapefile and simplifies it for use in creating the `simple_raster`. See also \code{\link{build_simple_raster_pop}}.
#' 
#' @param gaul_list a list of LBD standard location ids, pulled from \code{\link{get_adm0_codes}}. Defines which countries to keep when building polygon
#' @param buffer numeric, default 0.2. Defines how large a buffer to include around the polygon.
#' @param tolerance numeric, default 0.2. tol in the gSimplify function. Higher # = coarser polygon.
#' @param subset_only Boolean, default F. If T, returns subsetted shapefile without simplifying it. 
#' @param makeplots Boolean, default F. If T, prints plots of objects being returned. Must be in RStudio or some other setup that can see `plot()` calls.
#' @param use_premade Boolean, default F. Mostly deprecated. If T, checks gaul list against a common sets of codes (only for africa and it's 5 regions). Designed to save time but has not be maintained.
#' @param custom_shapefile_path default NULL. A filepath to a custom shapefile to use to generate the simple polygon. Used for special cases where the standard adm0 shapefile cannot be used. If set, `custom_shapefile` must be `NULL`.
#' @param custom_shapefile default NULL. Similar to `custom_shapefile_path`, except takes a SpatialPolygonsDataFrame directly rather than a path. If set, `custom_shapefile_path` must be `NULL`.
#' @param raking Boolean default F. If T, pulls the raking shapefile rather than the standard adm0 shapefile. Used in nonfractional raking (\code{\link{rake_cell_pred}}. 
#' @param shapefile_version default "current". Standard shapefile version of format `YYYY_MM_DD`. Typically `modeling_shapefile_version` set in config.  
#' 
#' @return A named list with `subset_shape` and `spoly_spdf`, a subsetted (but not simplified) object and the simplified SpatialPolygonsDataFrame respectively. 
#' 
#' @examples
#' \dontrun{
...
#' }
#'
load_simple_polygon <- function(gaul_list, 
                                buffer, 
                                tolerance = 0.2,
                                subset_only = F, 
                                makeplots = F, 
  ...
  if (use_premade == T) {
    ...
    if(identical(gaul_list, c(29, 42, 45, 47, 50, 66, 90, 94, 106, 105, 144, 155, 159, 181, 182, 214, 217, 221, 243))) {
      ...
      load(file.path(fp_list$geospatial_root, 'simple_polygons/wssa.RData'))
    ...
    if(identical(gaul_list, c(8,49,59,68,76,89))) {
      ...
      load(file.path(fp_list$geospatial_root, 'simple_polygons/cssa.RData'))
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(43,58,70,77,79,133,150,152,170,205,226,74,257,253,270))) {
      ...
      load(file.path(fp_list$geospatial_root, 'simple_polygons/essa.RData'))
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(4,40762,40765,145,169,6,248))) {
      ...
      load(file.path(fp_list$geospatial_root, 'simple_polygons/name.RData'))
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(35,142,172,227,235,271))) {
      ...
      load(file.path(fp_list$geospatial_root, 'simple_polygons/sssa.RData'))
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
    ...
    if(identical(gaul_list, c(4,6,8,29,35,42,43,45,47,49,50,58,59,66,68,70,
      ...
      load(file.path(fp_list$geospatial_root, 'simple_polygons/africa.RData'))
      assign("subset_shape", subset_shape, envir=globalenv())
      if(makeplots) plot(spoly_spdf)
      if(makeplots) plot(subset_shape, add = TRUE, border = grey(0.5))
      return(list(subset_shape=subset_shape,spoly_spdf=spoly_spdf))
...
#' @title Build Time Mesh
#'
#' @description Create a temportal mesh (defaulting to the four period U5M approach for now, come back and make more flexible later). Wrapper around `inla.mesh.1d`.
#'
#' @param periods integer vector, default 1:4. Number of periods in time mesh.
#'
#' @return A one dimensional INLA time mesh
#'
...
build_simple_raster_pop <- function(subset_shape,
  ...
  pop_rast <- brick(paste0(fp_list$covariate_root, "00_MBG_STANDARD/worldpop/total/", pop_release, "/1y/worldpop_total_1y_2010_00_00.tif"))
...
pull_custom_modeling_regions <- function(custom_regions){
  ...
  ref_reg_list <- list(
    ...
    # Custom regions for anemia
    'noaf_anemia' = 'noaf-dza-lby-esh',
    'sssa_anemia' = 'sssa-bwa',
    'essa_anemia' = 'essa-com-dji-eri-ssd',
    'wssa_anemia' = 'wssa-mrt',
    'ocea_anemia' = 'ocea-phl',
    'mide_anemia' = 'mide-irq-syr',
    'caca_anemia' = 'caca-blz-cri-cub-dom-jam-nic-pan-',
    'trsa_anemia' = 'trsa-guf-pry-sur',
    'ansa_anemia' = 'ansa-tto'
...
#' @title Get GAUL Codes (DEPRECATED)
#'
#' @description Pull Admin0 codes for the specified countries or regions,
...
#'   rather than 'gadm'.
#'
#' @note THIS FUNCTION IS DEPRECATED - ALWAYS USE get_adm0_codes() INSTEAD.
#'
...
#' @title Add Region to Data from Simple Raster
#'
#' @description Extracts the country code values from the `simple_raster` using the lat/longs in the data, then adds the `ADM0_CODE` and names of regions to the data using those values. Adds another column `good_records` corresponding to data that have gotten region added on properly and prints out how many did not. 
#' 
#' @param df input data, must have `latitude` and `longitude` columns. 
#' @param simple_raster simple_raster, as built by \code{\link{`build_simple_raster_pop`}}
#' @param shapefile_version String, default "current". which shapefile version to use (used in `get_adm0_codes`)
#'
#' @return df with ADM0_CODE, region, and good_records columns added. 
...
#' @title Convert GBD loc ids to LBD adm0 codes
#'
#' @description Convert a vector of countries (ihme_loc_id format) to vector of LBD adm0 codes. Uses \code{\link{get_location_code_mapping}} to convert. If using `from == "name"`, attempts an approximate match based on the `loc_name` and `loc_nm_sh` columns. The other options use the `loc_id` column. `From==iso3` uses the ihme_lc_id column.
#' 
#' @param countries vector of countries in ihme_loc_id format 
#' @param from default "iso3". What is the format of `countries`?  
#' options: "iso3" = "ihme_loc_id" = "ihme_lc_id", 
#'          "name" (the loc_name or loc_nm_short)
#' @param shapefile_version String, default "current". which shapefile version to use.
#'
#' @return a vector of LBD adm0 codes 
...
#' @title Merge With IHME Loc
#'
#' @description DEPRECATED? Merges the given datatable with the output of \code{\link{`get_location_code_mapping`}} to add on IHME loc ids using iso3 codes. Also adds on a region column to the datatable.
#' 
#' @param d datatable, must have a `country` column
#' @param re Character vector, regions in dataset
#' @param shapefile_version default "current", standard lbd shapefile version of format `YYYY_MM_DD`
#'
#' @return d with an `ihme_lc_id`, `loc_id`, and `region` columns
...
plot_custom_regions <- function(region_list,
  ...
  world_shape <- readRDS(file.path(fp_list$geospatial_root, 'rds_shapefiles/simplified_shapefiles/g2015_2014_0_simp_tol_0.1.rds'))
...
load_libs<-function(packages,stop=F){
  package_lib <- ifelse(grepl("geos", Sys.info()[4]),
                        paste0(fp_list$temp_root,'geospatial/geos_packages'),
                        paste0(fp_list$temp_root,'geospatial/packages'))
...
upload_config_to_db <- function(config, user, core_repo, indicator_group, indicator, run_date, verbose = FALSE, comment_long = "") {
  ...
  dbpath <- file.path(fp_list$geospatial_root, "run_logs/config_db/run_db_20190216.sqlite")
