...
#     indicator       = Specific outcome to be modeled within indicator category
  load_config <- function(repo, indicator_group, indicator, config_name=NULL, covs_name = NULL, post_est_only=FALSE, run_date = '') {
    ...
    if (is.null(config_name)) {
      ...
      if(post_est_only==FALSE) config <- fread(paste0('<<<< FILEPATH REDACTED >>>>', 'config.csv'), header=FALSE)
      ## If running analysis on existing model, use config from that model's outputs folder
      if(post_est_only==TRUE) config <- fread(paste0('<<<< FILEPATH REDACTED >>>>', '/config.csv'))
    } else {
