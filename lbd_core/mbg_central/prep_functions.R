...
#     indicator       = Specific outcome to be modeled within indicator category, i.e. "edu_0"
  load_config <- function(repo, indicator_group, indicator, config_name=NULL, covs_name = NULL, post_est_only=FALSE, run_date = '') {
    ...
    if (is.null(config_name)) {
      ...
      if(post_est_only==TRUE) config <- fread(paste0('<<< FILEPATH REDACTED >>>'))
    } else {
...
#   Returns: Empty raster over modeling area. To be used for cropping covariates quickly and projecting model.
  get_template_raster <- function(simple) {
    ...
    root <- ifelse('<<< FILEPATH REDACTED >>>>')

    # Centrally controlled folder of analysis shapefiles
    analysis_raster_dir <- paste0('<<< FILEPATH REDACTED >>>>')
...
#   Returns: Input data subset to modeling area.
   load_input_data <- function(indicator, simple = NULL, agebin = 0, removeyemen = FALSE, pathaddin = "",
    ...
    if('<<< FILEPATH REDACTED >>>>') load_dir <- '<<< FILEPATH REDACTED >>>'
    ...
    #yrs = data.table(year=1998:2017,y2=rep(c(2000,2005,2010,2015),each=5))
  #  d=merge(d,yrs,by='year',all.x=T)
    # d$y2=NULL
    # r2=range/2
    # for(y in years)
    #   d$y2[d$year>(y-r2) & d$year<(y+r2)] = y
    # d$year = d$y2; d$y2=NULL
    # # drop years below 1998
    # d <- d[year > 1997,]
    if(years=='five_year') {
    ...
    if(update_run_date == TRUE) {
      if(dir.exists(paste0('<<< FILEPATH REDACTED >>>'))) {
...
get_gaul_codes <- function(gauls) {
  ...
  # Set up list of gaul_codes
   ...
   gaul_ref[["south_asia"]] <- c(23,31,115,175,188, 231, 40781, 2, 52, 15 )  # last four are disputed territories
   ...
   gaul_ref[["stage2"]] <- c(145,75,162,28,57,107,73,261,138,147295,154,187,215,249,19,
                             ...
                             47,66,42,29,72,108,180,111,103,195,33,239,132,167,67,171,
                             44,264,139,153,240,196,116,242,175,188,115,117,231,31,23,
                             1,267,269,91,118,130,238,3,192,262,225,157,135,123,209)
   ...
   gaul_ref[["essa_sdn"]] <- c(43,58,70,77,79,133,150,152,170,205,226,74,257,253,270,6) # ESSA + Sudan
   ...
   gaul_ref[["sdn"]] <- 6
   gaul_ref[["tun"]] <- 248
   gaul_ref[["ind_all_territories"]] <- c(40781, 2, 52, 15, 115)
...
gaul_convert <- function(countries, from = "iso3", verbose = F) {
  ...
  table_file <- paste0('<<< FILEPATH REDACTED >>>>')
...
load_libs<-function(packages,stop=F){
  ...
  package_lib <- ifelse(grepl("geos", '<<< FILEPATH REDACTED >>>>'),
                        paste0('<<< FILEPATH REDACTED >>>>'),
                        paste0('<<< FILEPATH REDACTED >>>>'))
  .libPaths(package_lib)
  message(paste0('Loading packages from ',package_lib))

  if('<<< FILEPATH REDACTED >>>>'){stop("STOP! you will overwrite these packages if you run from windows")}
