...
################ THIS FUNCTION HAS BEEN DEPRECATED IN FAVOR OF SET_UP_CONFIG ###################
  load_config <- function(repo, indicator_group, indicator, config_name=NULL, covs_name = NULL, post_est_only=FALSE, run_date = '') {
    ...
    if (is.null(config_name)) {
      ...
      if(post_est_only==TRUE) config <- fread("<<<< FILEPATH REDACTED >>>>")
    } else {
...
build_simple_raster_pop <- function(subset_shape,
  ...
  pop_rast <- brick("<<<< FILEPATH REDACTED >>>>")
...
pull_custom_modeling_regions <- function(custom_regions){
  ...
  ref_reg_list <- list(
    ...
    'dia_mid_east' = 'afg+irq+jor+syr',
    'dia_essa' = 'bdi+com+ken+lso+mdg+moz+mwi+rwa+swz+syc+tza+uga+zmb+zwe',
    'dia_oceania' = 'asm+fji+fsm+kir+mhl+slb+ton+vut+wsm',
    'dia_ncaca' = 'dom+gtm+hnd+hti+nic+slv',
    'dia_scaca' = 'cri+pan',
    'dia_nessa' = 'ken+tza+uga',
    'dia_sessa' = 'bdi+com+lso+mdg+moz+mwi+rwa+swz+syc+zmb+zwe',
    'dia_sam_trop' = 'bra+guy+pry',
    'dia_sam_andes' = 'ven+col+ecu+per+bol',
    'ind_pak' = 'ind+pak',
    'ind_pak_chn' = 'ind+pak+chn',
    'dji_eri' = 'dji+eri'
  )
...
check_custom_regions <- function(region_list, shapefile_version = 'current') {
  ...
  info_table <- fread('<<< FILEPATH REDACTED >>>')
