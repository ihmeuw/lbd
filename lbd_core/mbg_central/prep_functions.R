...
#' @export
  load_config <- function(repo, indicator_group, indicator, config_name=NULL, covs_name = NULL, post_est_only=FALSE, run_date = '') {
    ...
    if (is.null(config_name)) {
      ...
      if(post_est_only==TRUE) config <- fread(paste0(fp_list['mbg_root'], indicator_group, '/', indicator, '/output/', run_date, '/config.csv'))
    } else {
...
#   Returns: Input data subset to modeling area.
   load_input_data <- function(indicator, indicator_group = use_global_if_missing("indicator_group"), agebin = 0, removeyemen = FALSE, pathaddin = "",
...
build_simple_raster_pop <- function(subset_shape,
  ...
  pop_rast <- brick(paste0(fp_list$covariate_root, "<<<< FILEPATH REDACTED >>>>"))
