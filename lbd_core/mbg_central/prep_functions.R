...
#' @export
  load_config <- function(repo, indicator_group, indicator, config_name=NULL, covs_name = NULL, post_est_only=FALSE, run_date = '') {
    ...
    if (is.null(config_name)) {
      ...
      if(post_est_only==FALSE) config <- fread(paste0(<<<< FILEPATH REDACTED >>>>), header=FALSE)
      ...
      config <- fread(paste0(<<<< FILEPATH REDACTED >>>>), header=FALSE)
    }
    ...
    if (!is.null(covs_name)) {
      ...
      covs <- fread(paste0(<<<< FILEPATH REDACTED >>>>), header = TRUE)
...
rasterize_check_coverage <- function(shapes, template_raster, field, ..., link_table = modeling_shapefile_version, id_raster = NULL) {
  ...
  if (is.data.table(link_table)) {  # link table provided
    ...
    link_table_file <- file.path(<<<< FILEPATH REDACTED >>>>)
    id_raster_file <- file.path(<<<< FILEPATH REDACTED >>>>)
