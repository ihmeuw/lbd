  ...
  for (rake in rr) {
    ...
    for (ad in ad_levels) {
      ...
      ad_summary_table <- make_admin_pred_summary(admin_pred = get(paste0("admin_", ad)),
                                                  ...
                                                  ...)
      if (!calc_complement) {
        fwrite(ad_summary_table, file = paste0(output_dir, ind, "_admin_", ad, "_", rake, file_addin, "_summary.csv"))
      } else {
        fwrite(ad_summary_table, file = paste0(output_dir, ind, "_admin_", ad, "_", rake, "_complement", file_addin, "_summary.csv"))
      }
    }
  }
}
...
save_best_model <- function(indicator_group, indicator, run_date, measure) {
  ...
  raked <- brick(paste0("<<<<FILEPATH REDACTED>>>>>", indicator_group, "/", indicator, "/output/", run_date, "/", indicator, "_mean_raked_raster.tif"))
  unraked <- brick(paste0("<<<<FILEPATH REDACTED>>>>>", indicator_group, "/", indicator, "/output/", run_date, "/", indicator, "_mean_raster.tif"))
...
get.cov.wts <- function(rd, ## run_date
  ...
  load(paste0("<<<<FILEPATH REDACTED>>>>>", ind_gp, '/', ind, '/output/', rd,
  ...
  save(imp.mat,
       file = paste0("<<<<FILEPATH REDACTED>>>>>", ind_gp, '/', ind, '/output/', rd,
...
get_weighted_correlation_raw_estimated<-function(indicator_group,
  ...
  input_folder<-"<<<<FILEPATH REDACTED>>>>>"
...
post_load_combine_save <- function(regions = strata,
  ...
  if ("unraked" %in% raked) {
    lookup_dir <- paste0(sprintf("<<<<FILEPATH REDACTED>>>>>/%s/%s/output/%s/", ig, indic, run_date))
  ...
  for (rake in rake_addin) {
    ...
    for (ss in summstats) {
      ...
      for (reg in regions) {
        ...
        rlist[[reg]] <-
          brick(ifelse(proj,
                       sprintf("<<<<FILEPATH REDACTED>>>>>%s/%s/output/%s/%s_%s%s_%s_raster_PROJ.tif", ig, indic, run_date, indic, reg, rake, ss),
                       sprintf("<<<<FILEPATH REDACTED>>>>>%s/%s/output/%s/%s_%s%s_%s_raster.tif", ig, indic, run_date, indic, reg, rake, ss)
          ))
      }
