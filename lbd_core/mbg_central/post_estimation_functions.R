...
get_population_data     <- function(simple_raster){
  ...
  pop<- brick(sprintf('<<<< FILEPATH REDACTED >>>>/pop_stack.tif'))
...
#' @param calc_complement Logical for calculating the counts of the complement, or pop(1 - prevalence of indicator). Used only for calculating counts
...
make_admin_pred_summary    <- function(admin_pred,
  ...
  # If calculating the population of the complement (1-prevalence of indicator), take the population minus the
  # number of people living with indicator
  if (calc_complement == T) {
    draws[, (names(draws)) := lapply(.SD, function(x) {pop[[1]] - x}), .SDcols = names(draws)]
  }
...
summarize_admins <- function(ind = indicator,
  ...
  for (rake in rr) {
    for (age in ages) {
      for(sex in sexes) {
    load(paste0(input_dir, ind, "_", rake, "_admin_draws_eb_bin", age, "_sex", sex, "_0.RData"))
    ...
    for (ad in ad_levels) {
      if (calc_complement == F) {
        message(paste0("Summarizing ", ind, ": admin ", ad, " (", rake, ")"))
      } else {
        message(paste0("Summarizing ", ind, ": admin ", ad, " (", rake, "_complement)"))
      }
      
      ad_summary_table <- make_admin_pred_summary(admin_pred = get(paste0("admin_", ad)),
                                                  sp_hierarchy_list,
                                                  summary_stats = summstats,
                                                  #calc_complement = calc_complement,
                                                  ...)
      if (!calc_complement) {
        fwrite(ad_summary_table, file = paste0(output_dir, ind, "_admin_", ad, "_", rake, "_bin", age, "_sex", sex, file_addin, "_summary.csv"))
      } else {
        fwrite(ad_summary_table, file = paste0(output_dir, ind, "_admin_", ad, "_", rake, "_bin", age, "_sex", sex, "_complement", file_addin, "_summary.csv"))
        }
      }
    }
  }
...
save_post_est   <- function(x,
  ...
  output_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date)
...
load_cell_preds <- function(indicator_group,
  ...
  if(u5m){
    if(ageasindic==FALSE){
      load(paste0(<<<< FILEPATH REDACTED >>>>'/output/',rd,'/',
...
fit_stats <- function(is_data,
  ...
  ### Fit statistics
  model_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date)
  image_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/model_image_history')
  ...
  fit_folder <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date, '/fit_stats')
...
save_best_model <- function(indicator_group, indicator, run_date, measure) {
  ...
  raked <- brick(paste0(<<<< FILEPATH REDACTED >>>> "/output/", run_date, "/", indicator, "_mean_raked_raster.tif"))
  unraked <- brick(paste0(<<<< FILEPATH REDACTED >>>> "/output/", run_date, "/", indicator, "_mean_raster.tif"))
  ...
  root <- <<<< FILEPATH REDACTED >>>>
  central_cov_dir <- paste0(<<<< FILEPATH REDACTED >>>>) 
  ...
  for(i in 1:length(names(unraked))) {
    ...
    writeRaster(subset_raked,
                filename = paste0(<<<< FILEPATH REDACTED >>>> indicator, '_raked_', measure, '_1y_', year, "_00_00.tif"),
                format = "GTiff",
                overwrite = TRUE)
    writeRaster(subset_unraked,
                filename = paste0(<<<< FILEPATH REDACTED >>>> indicator, '_unraked_', measure, '_1y_', year, "_00_00.tif"),
                format = "GTiff",
                overwrite = TRUE)
  }
...
get.cov.wts <- function(rd, ## run_date
  ...
  this_config <- fread(paste0(<<<< FILEPATH REDACTED >>>> '/output/', rd, '/config.csv'))
  ...
  load(paste0(<<<< FILEPATH REDACTED >>>> '/model_image_history/', rd, pathaddin, '.RData'))
  ...
  save(imp.mat,
       file = paste0(<<<< FILEPATH REDACTED >>>> '/output/', rd,
...
compile_results_table <- function(indicator_group,
  ...
  results_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date, '/table_', baseline_year)
...
get_weighted_correlation_raw_estimated<-function(indicator_group,
  ...
  results_tif<-raster::stack(paste0(<<<< FILEPATH REDACTED >>>> "/output/",rundate,"/",indicator,"_mean_raster.tif"))
...
post_load_combine_save <- function(regions = strata,
  ...
  if ("unraked" %in% raked) {
    lookup_dir <- paste0(sprintf("<<<< FILEPATH REDACTED >>>>/output/%s/", run_date))
  ...
  if ("raked_c" %in% raked) {
    rake_addin <- c(rake_addin, raked_count = "_raked_c")
  }
  
  if ("raked_c_uncirc" %in% raked) {
    rake_addin <- c(rake_addin, count_uncirc = "_raked_c_uncirc")
  }
  ...
  for (rake in rake_addin) {
    ...
    for (ss in summstats) {
      ...
      for (reg in regions) {
        ...
        rlist[[reg]] <-
          brick(ifelse(proj,
                       sprintf("<<<< FILEPATH REDACTED >>>>/%s/output/%s/%s_bin%s_sex%s_%s%s_%s_raster_PROJ.tif", run_date, indic, reg, rake, ss),
                       sprintf('<<<< FILEPATH REDACTED >>>>/%s/output/%s/%s_bin%s_sex%s_%s%s_%s_raster.tif',run_date,indic,age,sex,reg,rake,ss)
          ))
      }
      ...
      save_post_est(
        rlist, "raster",
        ifelse(!proj, 
               paste0(ssname, rr, "_bin", age, "_sex", sex, "_raster"),
               paste0(ssname, rr, "_bin", age, "_sex", sex, "_raster_PROJ")),
        indic
      )
    }
  }
  ...
  if (rf_table) {
    ...
    for (reg in regions) {
      rflist[[reg]] <-
        if(proj) {
          read.csv(sprintf("<<<< FILEPATH REDACTED >>>>/%s/output/%s/%s_bin%s_sex%s_%s_rf_PROJ.csv",  run_date, indic, age, sex, reg))  
        } else {
          read.csv(sprintf('<<<< FILEPATH REDACTED >>>>/%s/output/%s/%s_bin%s_sex%s_%s_rf.csv',run_date,indic,age,sex,reg)) 
        }
    }
    if(!proj) {
      save_post_est(do.call(rbind.fill,rflist),'csv', paste0('rf_bin', age, '_sex', sex), indic)
    } else {
      save_post_est(do.call(rbind.fill,rflist),'csv', paste0('rf_bin', age, '_sex', sex, '_PROJ'), indic)
    }
