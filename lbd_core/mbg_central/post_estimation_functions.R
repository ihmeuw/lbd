...
load_cell_preds <- function(indicator_group,
  ...
  if(u5m){
    if(ageasindic==FALSE){
      load('<<<< FILEPATH REDACTED >>>>') # the 0 are no holdout
    } else {
      load('<<<< FILEPATH REDACTED >>>>')
    }
    cell_pred <- cptmp
  } else {
    load('<<<< FILEPATH REDACTED >>>>') # the 0 are no holdouts
  }
  return(cell_pred)
}
...
fit_stats <- function(is_data,
  ...
  message(pathaddin)
  write.csv(df_no_nas, file = '<<<< FILEPATH REDACTED >>>>')
...
save_best_model <- function(indicator_group, indicator, run_date, measure) {
  ...
  raked <- brick('<<<< FILEPATH REDACTED >>>>')
  unraked <- brick('<<<< FILEPATH REDACTED >>>>')
...
post_load_combine_save <- function(regions = strata,
  ...
  if ("unraked" %in% raked) {
    lookup_dir <- paste0('<<<< FILEPATH REDACTED >>>>')
...
get_fhs_population <- function(population_version = "20190403_test_new_cluster_1000d_rerun_fix_draw_squeezed_agg_ordered",
  ...
  ## Get pop data
  pop_data <- readRDS('<<<< FILEPATH REDACTED >>>>')
