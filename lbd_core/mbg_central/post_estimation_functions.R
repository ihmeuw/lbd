...
load_cell_preds <- function(indicator_group,
  ...
  if(u5m){
    if(ageasindic==FALSE){
      load(paste0("<<<< FILEPATH REDACTED >>>>", indicator,'_cell_draws_eb_bin',agebin,'_',region,'_0',other,'NA.RData')) 
    } else {
      load(paste0('<<<< FILEPATH REDACTED >>>>', indicator,'_age',agebin,'_cell_draws_eb_bin',agebin,'_',region,'_0',other,'.RData'))
    }
    cell_pred <- cptmp
  } else {
    load(paste0('<<<< FILEPATH REDACTED >>>>',indicator,'_cell_draws_eb_bin',agebin,'_',region,'_0.RData')) 
  }
  return(cell_pred)
}
