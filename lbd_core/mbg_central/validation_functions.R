...
get_is_oos_draws <- function(ind_gp,
  ...
  df_all <- rbindlist(lapply(all.regions, function(rr) {
    ...
    for (this_fold in sort(unique(df.r$fold))) {
      ...
      if (file.exists(sprintf('%s/%s_cell_draws_eb_bin%i_%s_%i.RData', mod.dir, ind, age, rr, this_fold))) {
        ...
        message(paste('WARNING: EXCLUDING', sprintf('%s_cell_draws_eb_bin%i_%s_%i.RData', ind, age, rr, this_fold)))
      }
    }
...
#'
#' @param d data.table containing data and IS/OOS draws from `run_is_oos_draws()`
...
#' draws.df <- fread("<<<< FILEPATH REDACTED >>>>/output_draws_data.csv")
#'
