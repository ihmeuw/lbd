...
aggregate_validation <- function(holdoutlist = stratum_qt,
  ...
  ## Load data
  datdir <- sprintf('<<<< FILEPATH REDACTED >>>>/%s/output/%s/',run_date)
...
aggregate_validation_dev <- function( holdoutlist = stratum_qt,
  ...
  ## Load data
  datdir <- sprintf('<<<< FILEPATH REDACTED >>>>/output/%s/',run_date)
...
is_oos_preds_testing <- function(rd = run_date,
  ...
  output.dir <- sprintf("<<<< FILEPATH REDACTED >>>>/output/%s/", rd)

  ## Load data
  datdir <- sprintf('<<<< FILEPATH REDACTED >>>>/output/%s/',rd)
  ...
  if(holdouts!=0) {
    for(hh in 1:holdouts){
      ...
      load(sprintf(paste0('<<<< FILEPATH REDACTED >>>>/output/%s/', cell_draws_filename),
                  rd, indic, 0, reg, hh))
...
get_is_oos_draws <- function(ind_gp,
  ...
  mod.dir <- sprintf("<<<< FILEPATH REDACTED >>>>/output/%s/", rd)
  all.regions <- get_output_regions(mod.dir)
  
  
  #DIRECTLY PULLING IN HOLDOUT FILE
  load(holdout_file) # loads in df.points.tmp that has the point data that has been split
  df <- df.points.tmp
  df <- setDT(df)
  ...
  df_all <- rbindlist(lapply(all.regions, function(rr) {
    ...
    # subset data to this region--TEMPORARILY DISABLE SINCE SHOULD ONLY BE MODELING ONE REGION AT A TIME WHEN USING OOS VALIDATION
    #df.r <- df[region == rr, ]
    df.r <- as.data.table(df) # since the holdout data is created within parallel_model
    ...
    for (this_fold in sort(unique(df.r$fold))) {
      ...
      filename_fold <- paste0(mod.dir, sprintf("%s_cell_draws_eb_bin%i_sex%i_%s_%i.RData", ind, age, sex, rr, this_fold))
    ...
    } ## NOTE: some of these are NAs because of being on/near the boundary
...
plot.rfs <- function(ind.gp = indicator_group,
  ...
  regions <- get_output_regions(in_dir = paste0(<<<< FILEPATH REDACTED >>>> '/output/',
                                                rd))
  ...
  for(rr in regions){
    ...
    in_dir  <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', rd)
...
model_fit_table_list <- function(regions, rd=run_date, holdout = 0,
  ...
  for(rr in regions){
    ...
    load(paste0(<<<< FILEPATH REDACTED >>>> '/model_image_history/', rd, pathaddin,
                '.RData'))
...
plot_abs_errors <- function(gaul_list = gaul_list,
  ...
  ## calculate residual: count/N - pred
  phat <- base::rowMeans(df[, grep('draw', colnames(df)), with = FALSE], na.rm = TRUE)
  phat[is.nan(phat)] <- NA
  df$phat <- phat
  df$pobs <- df[[ind]] / df[['N']]
...
#'   "<<<< FILEPATH REDACTED >>>>/output/%s/output_draws_data.csv",
#'   run_date
#' ))
#' 
...
#' @param input_file If your team does not use the standard `<<<< FILEPATH REDACTED >>>>/{indicator}.csv` input data filepath, supply the filepath to your data here
#' @param admin0_file If your team does not use the standard `'<<<< FILEPATH REDACTED >>>> '/output/',run_date, '/pred_derivatives/admin_summaries/',indicator, '_admin_0_unraked_summary.csv'` directory to save unraked aggregated admin summaries, supply the correct filepath here
#' @param admin1_file If your team does not use the standard `'<<<< FILEPATH REDACTED >>>> '/output/',run_date, '/pred_derivatives/admin_summaries/',indicator, '_admin_1_unraked_summary.csv'` directory to save unraked aggregated admin summaries, supply the correct filepath here
#' @param admin2_file If your team does not use the standard `'<<<< FILEPATH REDACTED >>>>'/output/',run_date, '/pred_derivatives/admin_summaries/',indicator, '_admin_2_unraked_summary.csv'` directory to save unraked aggregated admin summaries, supply the correct filepath here
#' 
...
get_admin_pv <- function(indicator, 
  ...
  if(is.null(input_file)){
    if(!file.exists(paste0(<<<< FILEPATH REDACTED >>>>, indicator, '.csv'))) stop('Indicator input data does not exist - please specify a file path with the input_file arg')
  ...
  if(is.null(admin0_file)){
    ad0_mbg <- fread(paste0(<<<< FILEPATH REDACTED >>>> '/output/',
  ...
  if(is.null(admin1_file)){
    ad1_mbg <- fread(paste0(<<<< FILEPATH REDACTED >>>> '/output/',
                            run_date, '/pred_derivatives/admin_summaries/',
  ...
  if(is.null(admin2_file)){
    ad2_mbg <- fread(paste0(<<<< FILEPATH REDACTED >>>> '/output/',
                            run_date, '/pred_derivatives/admin_summaries/',
