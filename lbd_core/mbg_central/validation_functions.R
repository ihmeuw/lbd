...
aggregate_validation <- function(holdoutlist = stratum_qt,
                                 cell_draws_filename = <<<< FILEPATH REDACTED >>>>,
...
get_is_oos_draws <- function(ind_gp,
  ...
  # load raw data
  if (!get.oos) {
    df <- fread(paste0(<<<< FILEPATH REDACTED >>>>)) ## raw input data
    df <- merge_with_ihme_loc(df, shapefile_version = shapefile_version)
  } else {
    df <- rbindlist(lapply(readRDS(paste0(<<<< FILEPATH REDACTED >>>>)), data.table))
  }
  ...
  if (write.to.file) write.csv(df_all, paste0(<<<< FILEPATH REDACTED >>>>), row.names = F)
  return(df_all)
}
...
plot.rfs <- function(ind.gp = indicator_group,
  ...
  for(rr in regions){
    ...
    default_rf_path <- paste0(<<<< FILEPATH REDACTED >>>>)
...
plot_of_plots <- function(run_date,
  ...
  folds <- c()
  for(i in 1:nfolds){
    if(file.exists(sprintf(<<<< FILEPATH REDACTED >>>>))){
      folds <- c(folds,i)
    }
  }
  ...
  if(length(folds)>0){
    ...
    for(i in folds){
      ...
      for(f in list.files(path   =sprintf(<<<< FILEPATH REDACTED >>>>),pattern=patt)){
        message(f)
        tmp       <- fread(sprintf(<<<< FILEPATH REDACTED >>>>))
    ...
    message(paste0('saving ',sprintf(<<<< FILEPATH REDACTED >>>>)))
...
get_admin_pv <- function(indicator, 
  ...
  if(is.null(input_file)){
    if(!file.exists(paste0(<<<< FILEPATH REDACTED >>>>))) stop('Indicator input data does not exist in /share - please specify a file path with the input_file arg')
