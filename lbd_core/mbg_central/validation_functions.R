...
plot_5q0 <- function(data = df,
  ...
  for(y in yrs){
    ...
    dy <- d[which(d[[yr_col]] == y), ]## get the subset of the data for the year and data type
...
get_is_oos_draws <- function(ind_gp,
  ...
  connector <- fread(lookup_table_filepath)
  connector <- dplyr::select(connector, country = iso3, GAUL_CODE = gadm_geoid)
  df <- left_join(df, connector)
  df <- as.data.table(df)
  ...
  df_all <- rbindlist(lapply(all.regions, function(rr) {
    ...
    for (yy in 1:nperiod) {
      ...
      df.r[this_year, cell_pred_id := raster::extract(id_raster[[yy]], matrix(loc.r[this_year, ], ncol = 2))]
    }
    ...
    for (this_fold in sort(unique(df.r$fold))) {
      ...
      load(paste0(mod.dir, sprintf("%s_cell_draws_eb_bin%i_%s_%s.RData", ind, age, rr, this_fold)))
      df.r <- data.table(filter(df.r, cell_pred_id <= nrow(cell_pred)))
      # extract draws
...
get_admin_pv <- function(indicator,
  ...
  if(is.null(input_file)){
    if(!file.exists("<<<< FILEPATH REDACTED >>>>")) stop('Indicator input data does not exist in /share - please specify a file path with the input_file arg')
    input.dt <- fread("<<<< FILEPATH REDACTED >>>>")
  } else{
