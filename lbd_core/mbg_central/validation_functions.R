...
aggregate_validation <- function(holdoutlist = stratum_qt,
  ...
  ## Load data
  datdir <- sprintf('<<< FILEPATH REDACTED >>>>')
...
plot_5q0 <- function(data = df,
  ...
  as <- shapefile('<<< FILEPATH REDACTED >>>>')
...
is_oos_preds_testing <- function(rd = run_date,
  ...
  output.dir <- sprintf("<<< FILEPATH REDACTED >>>",
  ...
  if(holdouts!=0) {
    for(hh in 1:holdouts){
      ...
      load(sprintf(paste0('<<< FILEPATH REDACTED >>>'),
...
get_is_oos_draws <- function(ind_gp,
                             ...
                             ind_fm = "binomial",
                             age = 0,
                             nperiod = 17,            # FIXME: superfluous, can just use yrs
                             yrs = 2000:2017,
                             write.to.file = FALSE,
                             get.oos = FALSE,
  ...
  if(!get.oos) {
    in_dat <- function(reg){
      df <- fread(paste0(mod.dir, 'input_data_bin0_', reg,'_0.csv')) ## raw input data
      if(any(duplicated(colnames(df)))) df[,1] <- NULL
      df <- merge_with_ihme_loc(df, re = all.regions, shapefile_version = shapefile_version)
      return(df)
    }
    df <- rbindlist(lapply(all.regions, in_dat))
  } else {
    df <- rbindlist(lapply(readRDS(paste0(mod.dir, "stratum.rds")), data.table))
  }
  if(!'ADM0_CODE' %in% colnames(df)) df$ADM0_CODE <- df$ADM_CODE
  ...
  df_all <- rbindlist(lapply(all.regions, function(rr) {
    ...
    print(nrow(df.r))
    print(sort(unique(df.r$fold)))


    # loop over holdouts
    for (this_fold in sort(unique(df.r$fold))) {
      this_fold <- as.integer(this_fold)
  ...
  if(ind_fm=='gaussian') {
    pull_region_taus <- function(rr) {
      ...
      if(length(draws)!=ndraws) draws <- c(rep(draws, ndraws/length(draws))) ## Repeat list of hyperpars if Jon's chunking messed stuff up... fix later when we actually have different precisions when not running EB.
      if('Precision for the Gaussian observations' %in% names(draws[[1]][["hyperpar"]])) {
        hyper_name <- 'Precision for the Gaussian observations'
        taus <- c()
        for(i in 1:ndraws) taus[i] <- draws[[i]][['hyperpar']][[hyper_name]]
      }
      if('Log precision for the Gaussian observations -- in user scale' %in% names(draws[[1]][["hyperpar"]])) {
        hyper_name <- 'Log precision for the Gaussian observations -- in user scale'
        taus <- c()
        for(i in 1:ndraws) taus[i] <- draws[[i]][['hyperpar']][[hyper_name]]
      }
      if('precision parameter for the beta observations' %in% names(draws[[1]][["hyperpar"]])) {
        hyper_name <- 'precision parameter for the beta observations'
        taus <- c()
        for(i in 1:ndraws) taus[i] <- draws[[i]][['hyperpar']][[hyper_name]]
      }
      if('intern precision-parameter for the beta observations -- in user scale' %in% names(draws[[1]][["hyperpar"]])) {
        hyper_name <- 'intern precision-parameter for the beta observations -- in user scale'
        taus <- c()
        for(i in 1:ndraws) taus[i] <- draws[[i]][['hyperpar']][[hyper_name]]
      }
    ...
    df_all <- merge(df_all, all_taus, by='region')
  }
...
plot.rfs <- function(ind.gp = indicator_group,
                     ...
                     title = "Comparison to GBD 2017 in\n", ## region gets pasted on to this
                     shapefile_version = 'current'){
  ...
  regions <- get_output_regions(in_dir = paste0('<<< FILEPATH REDACTED >>>',
   ...
   for(rr in regions){
    ...
    if(rr == 'noaf') rr_name = "North Africa"
    if(rr == 'sssa') rr_name = "Southern Sub-Saharan Africa"
    if(rr == 'cssa') rr_name = 'Central Sub-Saharan Africa'
    if(rr == 'soas') rr_name = "South Asia"
    if(rr == 'seas') rr_name = "South East Asia"
    if(rr == 'eaas') rr_name = "East Asia"
    if(rr == 'ocea_edu') rr_name = "Oceania"
    if(rr == 'ansa+pry+sur+guy+guf') rr_name = 'Andean South America'
    if(rr == 'ansa') rr_name = 'Andean South America'
    if(rr == 'trsa') rr_name = "Tropical South America"
    if(rr == 'bra+ury') rr_name = "Tropical South America"
    if(rr == 'caca') rr_name = "Central America & Caribbean"
    if(rr == 'mide') rr_name = "Middle East"
    if(rr == 'stan') rr_name = "Central Asia"


    in_dir  <- paste0('<<< FILEPATH REDACTED >>>')
    default_rf_path <- paste0(in_dir, '/', ind, '_', rr, '_rf.csv')
    all_rfs <- fread(default_rf_path)
    all_rfs <- merge(all_rfs, df, by.x = c('loc', 'year'), by.y = c('name', 'year'))

    #gaul_list = get_adm0_codes(rr, shapefile_version = shapefile_version)
    #rfs <- all_rfs[loc %in% gaul_list, ]
    rfs <- all_rfs
    #loc_names <- setDT(get_location_code_mapping(shapefile_version = shapefile_version))
    setnames(rfs, "loc", "GAUL_CODE")
    rfs$start_point <- rfs$mean
    #rfs <- merge(rfs, loc_names, by="GAUL_CODE")
    rfs[, Year:= as.factor(year)]
    max_val = max(max(rfs[,.(target, start_point)],na.rm=T),na.rm= T)

    ## plot w/o country labels
    gg_rfs <- ggplot(data = rfs, aes(x = target, y = start_point)) +
  ...
  all.rfs <- grid.arrange(cssa_rf + margin,
                          ...
                          #eaas_rf + margin,
                          ocea_edu_rf + margin,
                          #get('bra+ury_rf') + margin,
                          caca_rf + margin,
                          get('ansa+pry+sur+guy+guf_rf') + margin,
                          mide_rf + margin,
                          stan_rf + margin,
                          ncol=4)
  ggsave(filename = sprintf('%s/%s_all_rfs_newvoldgbd.png',
                            output.dir, ind),
         all.rfs, width = 12, height = 12)
...
model_fit_table_list <- function(regions, rd=run_date, holdout = 0,
  ...
  for(rr in regions){
    ...
    f <-  sprintf('%s/output/%s/inla_model_fit_pre_preds_%s_holdout_%i_agebin_0.RDS',
...
####################################################################################################################
## INPUT: # data file matching <<< FILEPATH REDACTED >>>
...
#' @param input_file If your team does not use the standard `<<< FILEPATH REDACTED >>>
#' @param admin0_file If your team does not use the standard `'<<< FILEPATH REDACTED >>>
#' @param admin1_file If your team does not use the standard `'<<< FILEPATH REDACTED >>>
#' @param admin2_file If your team does not use the standard `'<<< FILEPATH REDACTED >>>
#'
...
get_admin_pv <- function(indicator,
  ...
  if(is.null(input_file)){
    ...
    input.dt <- fread(paste0('<<< FILEPATH REDACTED >>>'))
  } else{
    input.dt <- fread(input_file)
  }

  if(!('point' %in% colnames(input.dt))) input.dt[, point := pseudocluster]
  ...
  admin_data <- input_aggregate_admin(indicator = indicator, indicator_group = indicator_group, run_date = run_date, sample_column = samp_col,
                                      input_data = input.dt, regions = regs, indicator_family = ind_fam, svy_id = nid_col, shapefile_version = shapefile_version)
  ...
  ad0 <- merge(ad0_data, ad0_mbg, by = c('ADM0_CODE', 'year'))
  ad1 <- merge(ad1_data, ad1_mbg, by = c('ADM1_CODE', 'year'))
  ad2 <- merge(ad2_data, ad2_mbg, by = c('ADM2_CODE', 'year'))
  ...
  ad0[, bias := outcome - value]
  ad0[, mae := abs(bias)]
  ad0[, se := bias^2]

  ad1[, bias := outcome - value]
  ad1[, mae := abs(bias)]
  ad1[, se := bias^2]

  ad2[, bias := outcome - value]
  ad2[, mae := abs(bias)]
  ad2[, se := bias^2]
