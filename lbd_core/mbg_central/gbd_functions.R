...
get_con <- function(dbname, host) {
  con <- suppressWarnings(src_mysql(dbname = dbname, host = host, user = "<<<< USER REDACTED >>>>", password = "<<<< PASSWORD REDACTED >>>>"))
  return(con)
}
...
get_cov_estimates <- function(covariate_name_short, filters=list()) {
  ...
  conn <- dbConnect(RMySQL::MySQL(), host="<<<< HOST REDACTED >>>>", username="<<<< USERNAME REDACTED >>>>", password="<<<< PASSWORD REDACTED >>>>")
