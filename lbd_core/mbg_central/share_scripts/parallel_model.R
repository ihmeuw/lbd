...
if(as.logical(skiptoinla) == FALSE){
  ...
  if(holdout==0) {
    ...
    if(!exists("loadinputdata_from_rundate")) {
      ...
      load(paste0('<<<<FILEPATH REDACTED>>>>>',indicator_group,"/",indicator,'/output/',loadinputdata_from_rundate,"/","processed_input_data", pathaddin,".rda"))
  ...
  file.copy(from = paste0('<<<<FILEPATH REDACTED>>>>>'),
...
input_data <- build_mbg_data_stack(df            = df, # note that merge (if using TMB) will return data in a different (but internally consistent) order, just different than df
                                   ...
                                   exclude_cs    = exclude_cs, # raw covs will get center scaled here though (see notes above)
...
if(length(z_list) > 1) {
  ...
  for(z in z_list) {
    ...
    outputdir <- sprintf('<<<<FILEPATH REDACTED>>>>>%s/%s/output/%s/',indicator_group,indicator,run_date) # new outputdir
...
message(paste("Done with model fit for", reg))
