...
## set raster temp directory (the default location is auto-cleared periodically, which can be
## problematic during long runs; this folder is deleted at the very end of this script)
raster_tmpdir <- paste0(outputdir, 'raster_tmp', pathaddin)
dir.create(raster_tmpdir)
raster::rasterOptions(tmpdir = raster_tmpdir)
tmpDir()
...
if(as.logical(skiptoinla) == FALSE){
  ...
  ## temporarily remove ANC data, as we don't want it to go through stacking
  if (!as.logical(anc_in_stacking) & as.logical(use_stacking_covs)) {
    anc <- the_data[type == "ANC",]
    the_data <- the_data[type != "ANC",]
  }
  ...
  covchecklist <- check_for_cov_issues(check_pixelcount_thresh = 0.9)
  ...
  if(as.logical(use_stacking_covs)) {
    ...
    ## add ANC data back in, with stacking predictions from the full model
    if (!as.logical(anc_in_stacking)) {
      if (nrow(anc) > 0) {
        anc[, a_rowid := 1:.N + max(the_data$a_rowid)]
        anc_stackers <- extract_covariates(anc,
                                           stacked_rasters,
                                           id_col              = "a_rowid",
                                           return_only_results = TRUE,
                                           centre_scale        = FALSE,
                                           period_var          = 'year',
                                           period_map          = period_map)
        anc_stackers <- anc_stackers[, c("a_rowid", child_model_names, child_model_names), with = F]
        setnames(anc_stackers, c("a_rowid", paste0(child_model_names, "_full_pred"), paste0(child_model_names, "_cv_pred")))
        anc <- merge(anc, anc_stackers, by = "a_rowid")
        anc    <- na.omit(anc, c(indicator, 'N', paste0(child_model_names, "_full_pred"), paste0(child_model_names, "_cv_pred")))
        the_data <- rbind(the_data, anc, fill = T)
      }
    }
...
# Workaround for pc prior not existing at the moment
input_data <- build_mbg_data_stack(df            = df, # note that merge (if using TMB) will return data in a different (but internally consistent) order, just different than df
                                   ...
                                   zcol          = zcol,
                                   anc_correction = anc_correction, # must not be null if z_list is present
                                   pc_prior = pc_prior,
                                   sigma_upper_tail = sigma_upper_tail,
                                   range_lower_tail = range_lower_tail)
  ...
  if(fit_with_tmb == TRUE) {
    ...
    saveRDS(object = input_data, ## save this here in case predict dies
            file = sprintf('<<<< FILEPATH REDACTED >>>>/%s_TMB_data_input_list_%s_holdout_%i_agebin_%i.RDS',
                           ifelse(fit_with_tmb,'tmb','inla'), reg, holdout, age))
    # run the model
    system.time(
...
# Delete the raster temp dir
unlink(raster_tmpdir, recursive = T)
