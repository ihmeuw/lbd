...
# run_date <- '2018_04_03_14_17_11'
# indicator <- 'edu_zero_prop_15_49_female'
# reg <- 'asia_png'
# age <- 0
# test <- 0
# holdout <- 0
...
if(!'SINGULARITY_NAME' %in% names(Sys.getenv())){
  ...
  if(fit_with_tmb == TRUE){
    stop('YOU MUST USE THE SINGULARITY IMAGE IF YOU WANT TO FIT YOUR MODEL USING TMB.')
  }
}  ## if in singularity do not set a location
...
(function() {
    ...
    rasterOptions('<<< FILEPATH REDACTED >>>>')
...
# for(s in stacker_cols) {
#   message(length(df[is.nan(get(s)), get(s)]))
#   df <- df[is.nan(get(s)), (s) := 0.99]
# }
...
if(!as.logical(skipinla)) {
  ...
  if(fit_with_tmb == TRUE) {
    ...
    model_fit <- fit_mbg_tmb( lbdcorerepo     = core_repo,
                              ...
                              control_list    = list(trace=1), # list(abs.tol = 1e-18),
                              optimizer       = 'nlminb', # add optimx
                              ADmap_list      =  NULL  ) # list(zrho = 0.80))
    )
...
if(length(z_list) > 1){
  ...
  for(z in z_list) {
    ...
    # make a mean raster
    mean_ras  <- insertRaster(simple_raster,matrix(rowMeans(pm[[z]]),ncol = max(period_map$period)))
