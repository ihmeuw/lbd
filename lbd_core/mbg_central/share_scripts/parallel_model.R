...
if(as.logical(skiptoinla) == FALSE){
  ...
  if(holdout==0) {
    ...
    if(!exists("loadinputdata_from_rundate")) {
      ...
      if(run_date %in% c("2020_07_31_18_23_42", "2020_07_31_18_26_05")){
        df <- load_input_data_special_cohorts(indicator   = gsub(paste0('_age',age),'',indicator),
                            agebin      = age,
                            removeyemen = FALSE,
                            pathaddin   = pathaddin,
                            years       = yearload,
                            ...
                            yl          = year_list,
                            region      = reg)
      }else if(run_date %in% c("2020_08_05_16_16_11", "2020_08_05_16_14_22", "2020_08_05_15_34_10")){
        df <- load_input_data_special_cohorts_t_plus0(indicator   = gsub(paste0('_age',age),'',indicator),
                            agebin      = age,
                            removeyemen = FALSE,
                            pathaddin   = pathaddin,
                            years       = yearload,
      ...
      }else if(run_date %in% c("2020_08_05_16_19_21", "2020_08_05_16_17_29")){
        df <- load_input_data_special_cohorts_t_plus2(indicator   = gsub(paste0('_age',age),'',indicator),
                            agebin      = age,
                            removeyemen = FALSE,
                            pathaddin   = pathaddin,
                            years       = yearload,
  ...
  for(z in z_list) # z_list must be integers starting with 1
    if(length(chunks) > 1)
      ...
      for(z in z_list) {
        ...
        outputdir <- sprintf('FILEPATH/geospatial/mbg/%s/%s/output/%s/',indicator_group,indicator,run_date) # new outputdir
...
if(fit_with_tmb == FALSE){
  cell_pred <- lapply( pm , "[[" , 3 )
  cell_pred <- do.call("cbind", cell_pred)
} else if (fit_with_tmb == TRUE){
