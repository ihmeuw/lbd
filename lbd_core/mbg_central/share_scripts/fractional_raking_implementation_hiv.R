...
load(paste0("<<<< FILEPATH REDACTED >>>>", "post_est_temp_objs.RData"))
...
# Ensure proper MBG environment
sharedir       <- sprintf("<<<< FILEPATH REDACTED >>>>")
commondir      <- sprintf("<<<< FILEPATH REDACTED >>>>")
package_list <- c(t(read.csv(sprintf('%s/package_list.csv',"<<<< FILEPATH REDACTED >>>>"),header = FALSE)))
...
link_table <- get_link_table_hiv(simple_raster, shapefile_version = '2018_08_20')
...
# For now just assign stratum to reg
reg <- stratum
region <- stratum
...
scalars <- read.csv(file = paste0("<<<< FILEPATH REDACTED >>>>", "_pop_rf.csv"))

# add back to the cell_pred as a population rf
cell_pred <- merge(cell_pred, scalars, by = c("location_id", "year"))

## load the raking factors
fractional_rf <- read.csv(file = paste0("<<<< FILEPATH REDACTED >>>>", "_rf.csv"))
...
save(raked_cell_pred, file = paste0("<<<< FILEPATH REDACTED >>>>",
...
save(raked_cell_pred_c, file = paste0("<<<< FILEPATH REDACTED >>>>",
...
save_cell_pred_summary <- function(summstat, raked, ...) {
  ...
  if (raked == "unraked") {
    ...
    load(paste0(sharedir, "/", indicator, "_raked_cell_draws_eb_bin0_", reg, "_0.RData" ))
    cpred <- "raked_cell_pred"
  } else if (raked == "raked_c") {
    load(paste0(sharedir, "/", indicator, "_raked_c_cell_draws_eb_bin0_", reg, "_0.RData" ))
