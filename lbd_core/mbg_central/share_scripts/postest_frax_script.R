...
agebin  <- as.character(commandArgs()[5])
sex     <- as.character(commandArgs()[6])
...
load(paste0(temp_dir, "post_est_temp_objs_bin", agebin, "_sex", sex, "_", stratum, ".RData"))
...
countries_not_to_subnat_rake <- eval(parse(text=countries_not_to_subnat_rake))
...
if (rake_countries) {
  ...
  if (metric_space == "rates") {
    ...
    outputs <- fractional_agg_rates(
      ...
      countries_not_to_subnat_rake = countries_not_to_subnat_rake,
      return_objects = TRUE,
      calc_complement = if (exists("calc_complement")) calc_complement else FALSE
    )
    ...
    raked_frax_counts_save(output_list = outputs, sharedir = sharedir, indicator = indicator, age = age, sex = sex_id, reg = reg, holdout = holdout)
  }
} else {
  rf <- NULL
...
save(raked_cell_pred, file = paste0(
  ...
  indicator, "_raked_cell_draws_eb_bin", agebin, "_sex", sex, "_", reg, "_0.RData"
))
...
save_cell_pred_summary <- function(summstat, raked, ...) {
  ...
  if (raked == "raked_c") {
    ...
    load(paste0(sharedir, "/output/", run_date, "/", indicator, "_raked_c_cell_draws_eb_bin", agebin, "_sex", sex, "_", reg, "_0.RData" ))
    mask_raster <- "raked_simple_raster"
  }
  if (raked == "raked_c_complement") {
    cpred <- "raked_cell_pred_c_complement"
    load(paste0(sharedir, "/output/", run_date, "/", indicator, "_raked_c_complement_cell_draws_eb_bin", agebin, "_sex", sex, "_", reg, "_0.RData" ))
    mask_raster <- "raked_simple_raster"
  }
  ...
  save_post_est(ras, 'raster', paste0('bin', agebin, '_sex', sex, '_', reg, case_when(raked == 'raked' ~ '_raked',
                                                     raked == 'raked_c' ~ '_raked_c',
                                                     raked == 'raked_c_complement' ~ '_raked_c_complement',
                                                     TRUE ~ ''),
                                      '_', summstat, '_raster'))
}
...
# Save calc_complement if it exists
if (exists("calc_complement")) {
  if (calc_complement & !is.null(gbd)) {
    rake_list <- c(rake_list, "raked_c_complement")
  }
}

summ_list <- expand.grid(summstats[summstats != "p_above"], rake_list)
...
# Calculate proportion above 0.8 if you have p_above in summstats
if ("p_above" %in% summstats) {
  try(save_cell_pred_summary(
    summstat = "p_above",
    raked = "raked",
    value = p_above_value,
    equal_to = T
...
pathaddin <- paste0("_bin", agebin, "_sex", sex, "_", reg, "_0") # To allow us to use waitformodelstofinish()
write(NULL, file = paste0(output_dir, "/fin_", pathaddin))

# All done
message(paste0("Done with post-estimation and aggregation for ", stratum, " agebin ", agebin, " sex ", sex))
