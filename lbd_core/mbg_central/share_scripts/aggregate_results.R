...
# indicator <- 'edu_zero_prop'
# indicator_group<-'education'
# run_date<-'2018_03_21_20_47_14'
# raked<-FALSE
# pop_measure<-'wocba'
# overwrite<-TRUE
# age<-0
...
package_lib    <- ifelse(grepl("geos", Sys.info()[4]),
                           ...
                           paste0('<<< FILEPATH REDACTED >>>>'))#    dependent on the machine where the user runs the code.
...
shapefile_version <- modeling_shapefile_version <- raking_shapefile_verion <- '2019_05_06'
...
# } else {
  ...
  ifelse(!grepl('zero', indicator) & !grepl('mean', indicator) & !grepl('ratio', indicator),
         cell_pred <- load(paste0("<<< FILEPATH REDACTED >>>"))
         load(paste0(share_dir, indicator, ifelse(raked, '_raked', ''),"_cell_draws_eb", pathaddin, ".RData")))
  #print(cell_pred)
  if(!grepl('mean', indicator) & !grepl('zero', indicator)) cell_pred<- get(cell_pred)
  if(grepl('zero_no_primary_prop', indicator)) cell_pred <- zero_no_primary_prop_draws
  if(!exists('cell_pred')) {
    cell_pred <- raked_cell_pred
    rm(raked_cell_pred)
  }
  print(nrow(cell_pred))
