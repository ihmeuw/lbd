...
#                           run_date = run_date
  ...
  ids <- id(qt)
  ids <- na.omit(ids) ## qt alg adds a few NA rows
...
make_folds <- function(data,
                       ...
                       temp_strat = NULL,
                       spte_strat = NULL,
                       save.file = '<<<< FILEPATH REDACTED >>>>/stratum.rds', ## place to save the final object from the function
                       ...,
...
nid_folds <- function(nids, yr, ss, n_folds) {
  ...
  # reduce number of folds, if there are less than n_folds NIDs
  if (length(ss_by_nid) < n_folds) {
    n_folds <- length(ss_by_nid)
    message(paste('Number of folds reduced to', n_folds, 'in one stratum'))
  }
  ...
  # identify n_folds roughly equal folds based on the cumulative sample size
  target_fold_ss <- sum(ss) / n_folds
  brks <- sapply(1:n_folds, function(x) which.min(abs(cumulative_ss - x * target_fold_ss)))
