...
get_link_table <- function(simple_raster, shapefile_version) {
  #read in link_table
  global_link <- data.table(readRDS(sprintf("<<<< FILEPATH REDACTED >>>>")))
  #read in id_raster
  global_raster <- readRDS(sprintf("<<<< FILEPATH REDACTED >>>>"))
...
build_link_table <- function(shapefile_version,
  ...
  master_pop <- raster::brick('<<<< FILEPATH REDACTED >>>>')
...
#' They do not directly support a counts cell_pred 
...
#' @param sharedir sharedir       <- sprintf('<<<< FILEPATH REDACTED >>>>')
...
fractional_rake_rates <- function(cell_pred = cell_pred,
  ...
  #NA out population where the pixel value is NA
...
#' @param interval_mo the time in months between the modeled years
#' @param rake_subnational a logical value indicating the use of subnational raking targets or not
#' @param sharedir Path to sharedir e.g.: \code{sharedir <- sprintf('<<<< FILEPATH REDACTED >>>>')}
#' @param run_date model run date string
#' @param indicator modeled indicator
#' @param main_dir Path to main_dir e.g.: \code{main_dir <- paste0('<<<< FILEPATH REDACTED >>>>')}
...
fractional_agg_rates <- function(cell_pred = cell_pred,
  ...
  save(raked_cell_pred, 
       file = ifelse(is.null(custom_output_folder),
                     paste0(sharedir, "/output/", run_date, "/", indicator, "_raked_cell_draws_eb_bin", age, "_", reg, "_", holdout, ".RData"),
                     paste0(custom_output_folder, "/",indicator, "_raked_cell_draws_eb_bin", age, "_", reg, "_", holdout, ".RData")))
  ...
  save(raked_cell_pred_c, 
       file = ifelse(is.null(custom_output_folder),
                     paste0(sharedir, "/output/", run_date, "/",indicator,
                            "_raked_c_cell_draws_eb_bin", age, "_", reg, "_", holdout, ".RData"), 
