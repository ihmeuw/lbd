...
get_link_table <- function(simple_raster, shapefile_version) {
  #read in link_table
  global_link <- data.table(readRDS(sprintf("FILEPATH")))
  #read in id_raster
  global_raster <- readRDS(sprintf("FILEPATH"))
...
build_link_table <- function(shapefile_version,
  ...
  master_pop <- raster::brick("FILEPATH")
...
#' They do not directly support a counts cell_pred or logit raking.  It is also untested with the GADM 
...
#' @param sharedir sharedir       <- sprintf("FILEPATH")
...
#' @param interval_mo the time in months between the modeled years
#' @param rake_subnational a logical value indicating the use of subnational raking targets or not
#' @param sharedir Path to sharedir e.g.: "FILEPATH"
#' @param run_date model run date string
#' @param indicator modeled indicator
#' @param main_dir Path to main_dir e.g.: "FILEPATH"
...
fractional_agg_rates <- function(cell_pred = cell_pred,
  ...
  ############################################################
  # creating unraked counts aggregations (can be done two ways, un raking the counts cell_pred or reloading the unraked cell_pred and multiplying by the pop.)
...
load_populations_cov <- function(reg,
  ...
  if (measure == 'prevalence') {
    covs = fetch_from_rdata(paste0("FILEPATH")
