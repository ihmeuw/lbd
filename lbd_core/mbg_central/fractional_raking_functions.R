...
#' @import raster
#'
get_link_table <- function(simple_raster, shapefile_version, indcustom = FALSE) {
  #read in link_table
  if (indcustom) {
    global_link <- data.table(readRDS("<<<< FILEPATH REDACTED >>>>"))
  } else {
    global_link <- data.table(readRDS("<<<< FILEPATH REDACTED >>>>"))
  }
  #read in id_raster
  global_raster <- readRDS("<<<< FILEPATH REDACTED >>>>")
...
build_link_table <- function(shapefile_version,
  ...
  if (sample_ids) {
    world_raster <- readRDS("<<<< FILEPATH REDACTED >>>>")
...
fractional_rake_rates <- function(cell_pred = cell_pred,
  ...
  link_table <- get_link_table(simple_raster, shapefile_version = shapefile_version, ind_custom)
...
fractional_agg_rates <- function(cell_pred = cell_pred,
  ...
  if (indicator %in% c('w_imp', 'w_imp_other', 'w_unimp', 'w_surface')) {
      scalars <- read.csv(file = ifelse(is.null(custom_output_folder),
                                        "<<<< FILEPATH REDACTED >>>>",
                                    paste0(custom_output_folder, "/", indicator, "_", reg, "_pop_rf.csv")
      ))
  ...
  if (indicator %in% c('s_imp', 's_imp_other', 's_unimp', 's_od')) {
      scalars <- read.csv(file = ifelse(is.null(custom_output_folder),
                                        "<<<< FILEPATH REDACTED >>>>",
                                    paste0(custom_output_folder, "/", indicator, "_", reg, "_pop_rf.csv")
      ))
  ...
  if (!(indicator %in% c('w_imp', 'w_imp_other', 'w_unimp', 'w_surface', 's_imp', 's_imp_other', 's_unimp', 's_od'))) {
      message("creating a raked rates cell_pred")
    # rake rates
    overs <- paste0("V", 1:ndraws)

    message(paste(
      "This many pixels are NA:",
      sum(is.na(as.numeric(as.matrix(cell_pred[,..overs])))),
      "out of:", dim(as.matrix(cell_pred[,..overs]))[1]*dim(as.matrix(cell_pred[,..overs]))[2]))

    message("Raking!")

    cell_pred <- as.data.table(cell_pred)
    cell_pred <- cell_pred[, (overs) := lapply(overs, function(x) {
      a <- get(x)
      a <- ifelse(a == 1, 1-(1e-10), a)
      a <- ifelse(a == 0, (1e-10), a)
      return(a)
      })]
    ...
    message(paste("Raking method:", rake_method, ";", "Raking Factor:", unique(cell_pred$rf)))

    cell_pred <- as.data.table(cell_pred)
    cell_pred$mean <- NULL
    cell_pred$mean <- rowMeans(cell_pred[,..overs], na.rm = T)
    nm <- overs
    cell_pred <- cell_pred[, (nm) := lapply(nm, function(x) {
      x <- get(x)
      y <- get('mean')
      x[is.na(x)] <- y[is.na(x)]
      x
    ...
    message(paste(
      "This many pixels are above 1:",
      sum(as.numeric(as.matrix(cell_pred[,..overs])) > 1),
      "out of:", dim(as.matrix(cell_pred[,..overs])[1])*dim(as.matrix(cell_pred[,..overs])[2])))
    ...
    message(paste(
      "This many pixels are NA:",
      sum(is.na(as.numeric(as.matrix(cell_pred[,..overs])))),
      "out of:", dim(raked_cell_pred)[1]*dim(raked_cell_pred)[2]))
    ...
    rf <- lapply(seq(nrow(cys)), function(x) {
      ...
      if (nrow(rake_geo[location_id == theloc & year == theyear]) == 0) {
        ...
        if(iterate) {
          ...
          if (is.null(ret) | "try-error" %in% class(ret)) {
            message(paste0("Your GBD and MBG estimates are REALLY far apart for location ", theloc, " | year ", theyear))
            message("Increasing MaxJump and NumIter by 1000-fold, but you should investigate this...")

            ret <- try(LogitFindK(gbdval     = rake_geo[location_id == theloc & year == theyear,mean],
                                  pixelval   = as.matrix(cell_pred[location_id == theloc & year == theyear & !is.na(V1), paste0('V', 1:ndraws), with = F]), #pass the cell pred rows that corrospond to this country year
                                  weightval  = as.numeric(cell_pred[location_id == theloc & year == theyear & !is.na(V1), pop_raked]),
                                  MaxJump    = MaxJump*1000,
          ...
          if (is.null(ret) | "try-error" %in% class(ret)) {
            message(paste0("Your GBD and MBG estimates are REALLY far apart for location ", theloc, " | year ", theyear))
            message("Increasing MaxJump and NumIter by 10,000-fold, but you should investigate this...")

            ret <- try(LogitFindK(gbdval     = rake_geo[location_id == theloc & year == theyear,mean],
                                  pixelval   = as.matrix(cell_pred[location_id == theloc & year == theyear & !is.na(V1), paste0('V', 1:ndraws), with = F]), #pass the cell pred rows that corrospond to this country year
                                  weightval  = as.numeric(cell_pred[location_id == theloc & year == theyear & !is.na(V1), pop_raked]),
                                  MaxJump    = MaxJump*10000,
          ...
          if (is.null(ret) | "try-error" %in% class(ret)) {
            message(paste0("Your GBD and MBG estimates are REALLY far apart for location ", theloc, " | year ", theyear))
            message("Increasing MaxJump and NumIter by 100,000-fold, but you should investigate this...")

            ret <- try(LogitFindK(gbdval     = rake_geo[location_id == theloc & year == theyear,mean],
                                  pixelval   = as.matrix(cell_pred[location_id == theloc & year == theyear & !is.na(V1), paste0('V', 1:ndraws), with = F]), #pass the cell pred rows that corrospond to this country year
                                  weightval  = as.numeric(cell_pred[location_id == theloc & year == theyear & !is.na(V1), pop_raked]),
                                  MaxJump    = MaxJump*100000,
          ...
          if (is.null(ret) | "try-error" %in% class(ret)) {
            message(paste0("Your GBD and MBG estimates are REALLY far apart for location ", theloc, " | year ", theyear))
            message("Increasing MaxJump and NumIter by 1,000,000-fold, but you should investigate this...")

            ret <- try(LogitFindK(gbdval     = rake_geo[location_id == theloc & year == theyear,mean],
                                  pixelval   = as.matrix(cell_pred[location_id == theloc & year == theyear & !is.na(V1), paste0('V', 1:ndraws), with = F]), #pass the cell pred rows that corrospond to this country year
                                  weightval  = as.numeric(cell_pred[location_id == theloc & year == theyear & !is.na(V1), pop_raked]),
                                  MaxJump    = MaxJump*1000000,
