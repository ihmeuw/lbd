...
get_link_table <- function(simple_raster, shapefile_version) {
  #read in link_table
  global_link <- data.table(readRDS(sprintf("<<<<FILEPATH REDACTED>>>>>", shapefile_version)))
  #read in id_raster
  global_raster <- readRDS(sprintf("<<<<FILEPATH REDACTED>>>>>", shapefile_version))
...
#' They do not directly support a counts cell_pred or logit raking.
#'
...
load_populations_cov <- function(reg,
  ...
  if (measure == 'prevalence') {
    covs = fetch_from_rdata(paste0("<<<<FILEPATH REDACTED>>>>>"), 'cov_list')
    fes = fetch_from_rdata(paste0("<<<<FILEPATH REDACTED>>>>>"), 'all_fixed_effects')
