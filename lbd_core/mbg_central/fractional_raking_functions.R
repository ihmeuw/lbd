...
#' @export
raked_frax_counts_save <- function(output_list, sharedir, indicator, age, sex, reg, holdout) {
  ...
  save(raked_cell_pred,
       file = paste0(sharedir, "/output/", run_date, "/", indicator, "_raked_cell_draws_eb_bin", age, "_sex", sex, "_", reg, "_", holdout, ".RData")
  )
  ...
  save(raked_adm0_draws, raked_adm1_draws, raked_adm2_draws,
       file = paste0(main_dir, indicator, "_", "raked", "_admin_draws_eb_bin", age, "_sex", sex, "_", reg, "_", holdout, ".RData")
  )
  ...
  save(unraked_adm0_draws, unraked_adm1_draws, unraked_adm2_draws,
       file = paste0(main_dir, indicator, "_", "unraked", "_admin_draws_eb_bin", age, "_sex", sex, "_", reg, "_", holdout, ".RData")
  )
...
get_link_table <- function(simple_raster, shapefile_version) {
  #read in link_table
  global_link <- data.table(readRDS(sprintf("<<<< FILEPATH REDACTED >>>>/%s/lbd_standard_link.rds", shapefile_version)))
  #read in id_raster
  global_raster <- readRDS(sprintf("<<<< FILEPATH REDACTED >>>>/%s/lbd_standard_id_raster.rds", shapefile_version))
...
#'
#' link <- build_link_table(shapefile_version = NULL, cores = 10, region, region = NULL, custom_shapefile_path = "<<<< FILEPATH REDACTED >>>> /shapefile.shp", custom_shapefile_field = "ADM_CODE")
#' }
#'
#' @export
...
#' @param indicator modeled indicator
#' @param main_dir Path to main_dir e.g.: \code{main_dir <- paste0(<<<< FILEPATH REDACTED >>>> '/output/', run_date, '/')}
...
#' @param rake_method if set to "logit" creates raking factors in logit space, otherwise assumes linear raking
#' @param calc_complement logical; if set to true, the complement (1-prevalence) is also calculated 
#'
...
fractional_agg_rates <- function(cell_pred = cell_pred,
  ...
  scalars <- read.csv(file = ifelse(is.null(custom_output_folder),
                                    paste0(sharedir, "/output/", run_date, "/", indicator, "_bin", age, "_sex", sex, "_", reg, "_pop_rf.csv"),
                                    paste0(custom_output_folder, "/", indicator, "_bin", age, "_sex", sex, "_", reg, "_pop_rf.csv")
  ))
  ...
  fractional_rf <- read.csv(file = ifelse(is.null(custom_output_folder),
                                          paste0(sharedir, "/output/", run_date, "/", indicator, "_bin", age, "_sex", sex, "_", reg, "_rf.csv"),
                                          paste0(custom_output_folder, "/", indicator, "_bin", age, "_sex", sex, "_", reg, "_rf.csv")
  ))
  ...
  save(raked_cell_pred, file = ifelse(is.null(custom_output_folder),
                                      paste0(
                                        ...
                                        indicator, "_raked_cell_draws_eb_bin", age, "_sex", sex, "_", reg, "_", holdout, ".RData"
                                      ),
                                      paste0(
  ...
  # If you want to calculate the complement of your indicator, it also saves counts of the population with the complement of your indicator
  if (calc_complement) {
    message("Making counts of complement of indicator")
    cell_pred_complement <- copy(cell_pred) 
    cell_pred_complement <- cell_pred_complement[, (overs) := lapply(overs, function(x) (1 - get(x)) * pop_raked)]
    cell_pred_complement <- cell_pred_complement[is.na(V1), pop_raked := NA]
    raked_cell_pred_c_complement <- dedupe_linked_cell_pred(cell_pred_complement, overs)
    
    message("Saving counts of complement of indicator")
    save(raked_cell_pred_c_complement, 
         file = ifelse(is.null(custom_output_folder),
                       paste0(sharedir, "/output/", run_date, "/",indicator, "_raked_c_complement_cell_draws_eb_bin", age, "_", reg, "_", holdout, ".RData"), 
                       paste0(custom_output_folder, "/", indicator, "_raked_c_complement_cell_draws_eb_bin", age, "_", reg, "_", holdout, ".RData")))
    rm(cell_pred_complement)
  }
  ...
  save(raked_cell_pred_c, 
       file = ifelse(is.null(custom_output_folder),
                     paste0(sharedir, "/output/", run_date, "/",indicator,
                            "_raked_c_cell_draws_eb_bin", age, "_sex", sex, "_", reg, "_", holdout, ".RData"), 
                     paste0(custom_output_folder, "/", indicator,
                            "_raked_c_cell_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData")))
  ...
  save(admin_0, admin_1, admin_2, sp_hierarchy_list,
       file = ifelse(is.null(custom_output_folder),
                     paste0(main_dir, "/", indicator, "_", "raked", "_c", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData"),
                     paste0(custom_output_folder, "/", indicator, "_", "raked", "_c", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData")
       )
  )
  ...
  save(admin_0, admin_1, admin_2, sp_hierarchy_list,
       file = ifelse(is.null(custom_output_folder),
                     paste0(main_dir, "/", indicator, "_", "raked", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData"),
                     paste0(custom_output_folder, "/", indicator, "_", "raked", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData")
       )
  )
  ...
  save(admin_0, admin_1, admin_2, sp_hierarchy_list,
       file = ifelse(is.null(custom_output_folder),
                     paste0(main_dir, "/", indicator, "_", "unraked", "_c", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData"),
                     paste0(custom_output_folder, "/", indicator, "_", "unraked", "_c", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData")
       )
  )
  ...
  save(admin_0, admin_1, admin_2, sp_hierarchy_list,
       file = ifelse(is.null(custom_output_folder),
                     paste0(main_dir, indicator, "_", "unraked", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData"),
                     paste0(custom_output_folder, "/", indicator, "_", "unraked", "_admin_draws_eb_bin", age,  "_sex", sex, "_", reg, "_", holdout, ".RData")
       )
  )
...
load_populations_cov <- function(reg,
  ...
  if (measure == 'prevalence') {
    covs = fetch_from_rdata(paste0(<<<< FILEPATH REDACTED >>>>'/model_image_history/', run_date, pathaddin, '.RData'), 'cov_list')
    fes = fetch_from_rdata(paste0(<<<< FILEPATH REDACTED >>>>'/model_image_history/', run_date, pathaddin, '.RData'), 'all_fixed_effects')
...
calculate_pop_scalars <- function(cell_pred = cell_pred,
  ...
  if (!is.null(custom_output_folder)) {
    write.csv(scalars, file = paste0(custom_output_folder, "/", indicator, "_bin", agebin, "_sex", sex, "_", stratum, "_pop_rf.csv"))
  } else {
    write.csv(scalars, file = paste0(sharedir, "/output/", run_date, "/", indicator, "_bin", agebin, "_sex", sex, "_", stratum, "_pop_rf.csv"))
  }
...
calculate_fractional_rfs <- function(ndraws = ndraws,
  ...
  if (!is.null(custom_output_folder)) {
    write.csv(fractional_rf, file = paste0(custom_output_folder, "/", indicator, "_bin", agebin, "_sex", sex, "_", stratum, "_rf.csv"))
  } else {
    write.csv(fractional_rf, file = paste0(sharedir, "/output/", run_date, "/", indicator, "_bin", agebin, "_sex", sex, "_", stratum, "_rf.csv"))
  }
...
#'
#' @export
prep_gbd_pops_for_fraxrake <- function(pop_measure = "a0004t", 
                                       reg, 
                                       year_list,
                                       ...) {
  library(stringr)
  ...
  # Grab correct sex id from pop_measure
  if (str_sub(pop_measure, -1, -1) == "t") {
    # Both men and women
    sex = 3
  } else if (str_sub(pop_measure, -1, -1) == "m") {
    # Just men 
    sex = 1
  } else {
    # Just women 
  ...
  gbd_pops <- get_population(age_group_id = age_GBD, location_id = loc_GBD, sex_id = sex, year_id = year_list, ...)
