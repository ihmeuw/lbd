...
graph_data_coverage_values <- function(df,
                                       ...
                                       use_custom_diverging_scale = FALSE,
                                       custom_diverging_scale = NULL,
                                       custom_diverging_midpoint = NULL,
                                       custom_diverging_breaks = NULL
                                       ) {
  ...
  # Grab a country list
  list_output <- get_country_list(df, region, endemic_iso3 = endemic_iso3)
  ...
  if(nrow(df_graph_poly) > 0) {
    ...
    for(period in c(2000,2005,2010,2015)) {
     g_map <- suppressMessages(make_a_period_map(period, df, region, poly_shapes_all,
                                                 ...
                                                 color_list, legend_title, log_dir, base_font_size, map_point_size, cap, cap_type,
												                         legend_min, legend_max,
												                         use_custom_diverging_scale,
												                         custom_diverging_scale,
												                         custom_diverging_midpoint,
												                         custom_diverging_breaks))
  ...
  if(save_on_share==TRUE) out_dir <- paste0('<<< FILEPATH REDACTED >>>')
...
find_new_data <- function(df_summary, indicator, since_date) {
  ...
  summary_dir <- paste0('<<< FILEPATH REDACTED >>>>')
...
get_country_list <- function(df, region, endemic_iso3) {
  ...
  if (region == 'africa') {
    ...
    country_list <- country_list[country_list %in% endemic_iso3]
  }
  ...
  if (region == 'latin_america') {
    ...
    country_list <- c(unique(df[region_name %in% region_list]$country), "URY") # add Uruguay
    country_list <- country_list[country_list %in% endemic_iso3]
  }
  ...
  if (region == 'middle_east') {
    ...
    # Uzbekistan, Turkmenistan, Tajikistan, Kyrgyzstan, Mongolia
...
make_background_map <- function(region, endemic_gauls, simplify_polys, tolerance, fast_shapefiles, master_shape_all) {
  ...
  if (region == "africa") {
    ...
    if (fast_shapefiles == T) {
      background_map <- readRDS('<<< FILEPATH REDACTED >>>')
    } else {
      background_map <- readOGR('<<< FILEPATH REDACTED >>>>')
    }
  ...
  if (region == 'latin_america') {
    ...
    gaul_list <- c(gaul_list, 86, 260) # Add French Guiana and Uruguay
    background_map <- master_shape_all[master_shape_all@data$GAUL_CODE %in% gaul_list, ]
  }
  if (region == 'central_america') {
...
make_a_period_map <- function(period, df, region,
                              ...
                              log_dir, base_font_size, map_point_size, cap, cap_type,
							                legend_min, legend_max, use_custom_diverging_scale,
							                custom_diverging_scale,
							                custom_diverging_midpoint,
							                custom_diverging_breaks) {
  ...
  merge_poly_data <- function(this_survey, poly_shapes_all) {
    survey_subset <- df_graph_poly[svy_id == this_survey, ]
  ...
  if (nrow(df_period_polys) > 0) {
    ...
    all_period_polys <- lapply(unique(df_period_polys[, svy_id]), function(this_survey) {
  ...
  range <- range(df$outcome, na.rm=T)
	...
	range[1] <- ifelse(!(is.na(legend_min)), legend_min, range[1])
	range[2] <- ifelse(!(is.na(legend_max)), legend_max, range[2])
  if (cap_type == "percentile" & is.na(legend_max)) range[2] <- quantile(df$outcome, cap/100, na.rm=T)
  if (cap_type == "absolute" & is.na(legend_max)) range[2] <- cap
  ...
  g_map <- g_map +
    ...
    ## Add custom diverging scale, if toggled.
    if(use_custom_diverging_scale==TRUE) {
      if(is.null(custom_diverging_scale)) {
        warning("Indicated to use a custom color scale, but no custom_diverging_scale argument provided.
                Please include as a character vector the desired colors for the lower limit, divergence point,
                upper limit, and values above the upper limit.")
      }
      if(is.null(custom_diverging_midpoint)) {
        warning("Indicated to use a custom color scale, but no custom_diverging_midpoint argument provided.
                Please include as a numeric the desired indicator value to use as the divergence point.")
      }
      if(is.null(custom_diverging_breaks)) {
        warning("Indicated to use a custom color scale, but no custom_diverging_breaks argument provided.
                Please include as a numeric the desired indicator values to use as breaks/labels on guide.")
      }
      g_map <- g_map + scale_fill_gradient2(low = custom_diverging_scale[1], mid = custom_diverging_scale[2],
                                            high = custom_diverging_scale[3], midpoint = custom_diverging_midpoint,
                                            limits = c(min(custom_diverging_breaks), max(custom_diverging_breaks)),
                                            na.value = custom_diverging_scale[4], labels = format(custom_diverging_breaks),
                                            breaks = custom_diverging_breaks) +
                       guides(fill = guide_colorbar(barheight = 15, nbin=1000))
    }
...
make_table <- function(df, gaul_list, order_var, country_list, remove_rank) {
  ...
  #summary_tbl <- tableGrob(table_data, rows=NULL, theme = ttheme_default(base_size = 7))
...
make_table_new <- function(df_summary, country_list, year_min, year_max) {
  ...
  #   summarize(Count = sum(count), N = sum(n), pointpoly = pointpoly) %>%
  #   as.data.table
