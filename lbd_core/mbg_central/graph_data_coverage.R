...
##
## GRAPH DATA COVERAGE
## Authors: Jon Mosser (<<< USERNAME REDACTED >>>), Nat Henry (<<< USERNAME REDACTED >>>), Michael Collison (<<< USERNAME REDACTED >>>)
...
#'   directory will be `'<<<< FILEPATH REDACTED >>>>'`.
...
graph_data_coverage_values <- function(df,
  ...
  disputed_shp <- readRDS('<<<< FILEPATH REDACTED >>>>')
  ...
  if(save_on_share==TRUE) out_dir <- '<<<< FILEPATH REDACTED >>>>'
...
get_country_list <- function(df, region, region_title) {
  ...
  #statement to deal with custom cases
    ...
    #create dictionary
    stage_master_list <- read.csv('<<<< FILEPATH REDACTED >>>>')
...
#'   central directory, '<<<< FILEPATH REDACTED >>>>'
#' 
...
dcp_refresh_background_shps <- function(
  ...
  #  disputed shapefile in '<<<< FILEPATH REDACTED >>>>'
