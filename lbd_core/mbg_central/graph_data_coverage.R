...
#'   qlogin. For more info, see USERNAME great writeup on the subject at
...
#'   `"FILEPATH/geospatial/mbg/data_coverage_plots/"`.
...
graph_data_coverage_values <- function(df,
  ...
  if(nrow(df_graph_poly) > 0) {
    ...
    # Only pull shapefiles if there are any shapefiles left
...
get_country_list <- function(df, region, region_title) {
  ...
  #statement to deal with custom cases
    ...
    #create dictionary
    stage_master_list <- read.csv("FILEPATH")
...
pull_polys <- function(shape_loc, fast_shapefiles) {
  ...
  if (fast_shapefiles == T) {
    ...
    master_shape <- readOGR(dsn = paste0("FILEPATH"),
                            layer = shape)
  }
...
dcp_refresh_background_shps <- function(
  ...
  # Requires sourcing FILEPATH from LBD core
  source(paste0(core_repo,'/mbg_central/setup.R'))
...
prep_data_coverage_shiny <- function(df,
  ...
  save(list = 'just_polygons', file = paste0("FILEPATH"))
