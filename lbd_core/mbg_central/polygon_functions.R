...
#' @param indic Indicator being run (e.g. `died`, `dpt3_cov`, `has_lri`).  Note that this is only used for some very specific renaming at the very end of the function - this code will actually preserve all columns passed to it.
#' @param unique_vars character values of additional names that uniquely ID rows in data, for example "age_bin" for u5m data. Defaults to NULL
#' @param density Passed to `seegMBG::getPoints()` as the `n` argument, which determines the # of integration points. Determines how many pseudopoints are laid down for each polygon. Default = 0.001. This can be interpreted as points per pixel if `perpixel = T`, and as points per square km if `perpixel = T` and `use_1k_popraster = T` (default settings)
#' @param perpixel Passed to `seegMBG::getPoints()` as the `perpixel` argument  This determines whether the `density` arguments represents an absolute number, or if it is the number of integration points per pixel (so that the total # of integration points = `density` * `[total number of pixels]`).  By default this is TRUE with a density of 0.001.
#' @param prob Passed to `seegMBG::getPoints()` as the `prob` argument. Determines whether to weight the integration points by the values of the raster (here, the population raster). TRUE by default.
#' @param use_1k_popraster Should a 1km x 1km population raster be used (disaggregates the 5x5 km population raster)? Logical, default = TRUE.  Otherwise will use 5k pop raster
#' @param pull_poly_method How should we get the polygons for resampling?  Options are parallel processing with `mclapply()` (`pull_poly_method = "mclapply"`), parallel processing with `foreach()` (`pull_poly_method = "foreach"`) or fast polygon pulling using RDS files (`pull_poly_method = "fast"` -- the default)
#' @param gaul_list List of gaul codes covering your modeling area, for use in pulling the population rasters
#' @param seed Optional seed that is set just prior to resampling to gaurantee the same results when rerunning on the same data. By default this is `NULL`, in which case no seed is set.
#' @param shapefile_version string indicating which admin shapefile to use to generate population raster
#'
#' @return A data table with your input `data` object, now with polygons duplicated and weighed according to the resampling algorithm
#'
#' @examples
#' \dontrun{
...
#'}
#'
#' @seealso methods for loading in polygons, as set by [pull_poly_method]
#' \code{\link{pull_polys_foreach}}
#' \code{\link{pull_polys_mclapply}}
#' \code{\link{pull_polys_fast}}
resample_polygons <- function(data,
                              shp_path = fp_list$survey_shapefile_root,
...
#' @title Load shapefiles with foreach
#'
#' @description Load shapefiles in parallel with foreach. Only returns polygons as specified by [shapes]. Used by \code{\link{resample_polygons}}.
#'
#' @param cores number of cores to parallelize over
#' @param shapefiles character vector of shapefile names to load in
#' @param shapes data.table with columns "shapefile" and "location_code". Only polygons specified here will be returned
#' @param shp_path full file path to shapefile directory
#' 
#' @return a list containing polygon objects as requested by [shapes] and "problem shapes" requested but not found.
#' 
#' @seealso 
#' parent function: \code{\link{resample_polygons}}
#' alternates: \code{\link{pull_polys_mclapply}}
#'             \code{\link{pull_polys_fast}}
...
#' @title Load shapefiles with mclapply
#'
#' @description Load shapefiles in parallel with mclapply. Only returns polygons as specified by [shapes]. Used by \code{\link{resample_polygons}}.
#'
#' @param cores number of cores to parallelize over
#' @param shapefiles character vector of shapefile names to load in
#' @param shapes data.table with columns "shapefile" and "location_code". Only polygons specified here will be returned
...
#' @seealso 
#' parent function: \code{\link{resample_polygons}}
#' alternates: \code{\link{pull_polys_foreach}}
#'             \code{\link{pull_polys_fast}}
...
#' @title Load shapefiles from RDS shapefile directory
#'
#' @description Load shapefiles serially from RDS shapefile directory. Only returns polygons as specified by [shapes]. Default method for \code{\link{resample_polygons}}.
#'
#' @param shapefiles character vector of shapefile names to load in
#' @param shapes data.table with columns "shapefile" and "location_code". Only polygons specified here will be returned
#' @param shp_path full file path to shapefile directory
...
#' @title Wrapper for fast polygon resampling
#'
#' @description DEPRECATED. Wrapper for fast polygon resampling, functionality now integrated with \code{\link{resample_polygons}}. Kept for backwards compatibility.
#'
#' @param data a data frame or data table with the following columns: cluster_id, exposed,
#'             <indicator>, latitude, longitude, shapefile, location code. Location_code refers to
#'             the shapefile polygon that the data. Data should alreaday be aggregated up
...
#' @param unique_vars character values of additional names that uniquely ID rows in data, for example "age_bin" for u5m data. Defaults to NULL
#' @param density Passed to `seegMBG::getPoints()` as the `n` argument, which determines the # of integration points. Determines how many pseudopoints are laid down for each polygon. Default = 0.001. This can be interpreted as points per pixel if `perpixel = T`, and as points per square km if `perpixel = T` and `use_1k_popraster = T` (default settings)
#' @param perpixel Passed to `seegMBG::getPoints()` as the `perpixel` argument  This determines whether the `density` arguments represents an absolute number, or if it is the number of integration points per pixel (sothat the total # of integration points = `density` * `[total number of pixels]`). By default this is TRUE with a density of 0.001.
#' @param prob Passed to `seegMBG::getPoints()` as the `prob` argument. Determines whether to weight the integration points by the values of the raster (here, the population raster). TRUE by default.
#' @param use_1k_popraster Should a 1km x 1km population raster be used (disaggregates the 5x5 km population raster)? Logical, default = TRUE.  Otherwise will use 5k pop raster or fast polygon pulling using RDS files (`pull_poly_method = "fast"` -- the default)
#'                         
#' @return A data table with your input `data` object, now with polygons duplicated and weighed according to the resampling algorithm.
...
#' @title Wrapper for mclapply polygon resampling
#'
#' @description DEPRECATED. Wrapper for mclapply polygon resampling, functionality now integrated with \code{\link{resample_polygons}}. Kept for backwards compatibility.
#'
#' @param data a data frame or data table with the following columns: cluster_id, exposed,
#'             <indicator>, latitude, longitude, shapefile, location code. Location_code refers to
#'             the shapefile polygon that the data. Data should alreaday be aggregated up
...
#' @title Resample polygons by sampling type
#'
#' @description Resampling using custom rasters rather than 2010 worldpop, as defined by [raster_dict] and [raster_col]. Assigns data to the custom raster, then all the polygons are resampled using the designated raster, so rows of data in the same dataset can be resampled using different rasters. Can be used for instances where datasets have not been sampled proportional to population
#'
#' @param data a data frame or data table with the following columns: cluster_id, exposed,
#'             <indicator>, latitude, longitude, shapefile, location code. Location_code refers to
#'             the shapefile polygon that the data. Data should alreaday be aggregated up
...
#' @param prob Passed to `seegMBG::getPoints()` as the `prob` argument. Determines whether to weight the integration points by the values of the raster (here, the population raster). TRUE by default.
#' @param use_1k_popraster Should a 1km x 1km population raster be used (disaggregates the 5x5 km population raster)? Logical, default = TRUE.  Otherwise will use 5k pop raster or fast polygon pulling using RDS files (`pull_poly_method = "fast"` -- the default)
#' @param raster_dict a table with two columns, default NULL. First column is the name of the raster, second is a full file path to the raster. All entries in [raster_col] must match entries in [raster_dict].
#' @param raster_col character, default NULL. The name of a column in [df] to include in resampling. Each value here must be included in [raster_dict].
#' @param regions a list of mbg modeling regions in the data
#' @param shapefile_version default 'current', a valid lbd admin shapefile version
#' 
#' @return A data table with your input `data` object, now with polygons duplicated and weighed according to the resampling algorithm.
...
add_poly_centroids <- function(df,
  ...
  for(shape in unique(shape_table$shapefile)){
    ...
    if(fast_shapefiles){
      ...
      shp <- rgdal::readOGR(file.path(fp_list$survey_shapefile_root, shape))
    }
    missing_codes <- c()
