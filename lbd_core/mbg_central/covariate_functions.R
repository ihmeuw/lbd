load_and_crop_covariates <- function(fixed_effects, simple_polygon, agebin=1) {
  ...
  lights_new      <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  LST_day         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  total_pop       <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  urban_rural     <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  land_cover      <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  LST_avg         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  gpcp_precip     <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  aridity_cruts   <- brick(paste0(<<<< FILEPATH REDACTED >>>>))

  # Load all temporally-nonvarying covariates
  # Human/Cultural Synoptic Rasters
  access          <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_humCul_stack.1
  irrigation      <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_humCul_stack.2
  LF              <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_humCul_stack.3
  LF_vector       <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_humCul_stack.4
  reservoirs      <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_humCul_stack.5
  # Environmental/Physical Synoptic Rasters
  aridity           <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_envPhy_stack.1
  dist_rivers_lakes <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_envPhy_stack.2
  dist_rivers_only  <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_envPhy_stack.3
  elevation         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_envPhy_stack.4
  annual_precip     <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_envPhy_stack.5
  PET               <- brick(paste0(<<<< FILEPATH REDACTED >>>>))$synoptic_envPhy_stack.6

  lat     <- raster(paste0(<<<< FILEPATH REDACTED >>>>))
  lon     <- raster(paste0(<<<< FILEPATH REDACTED >>>>))
  latlon  <- lat*lon


  # some u5m additions
  malaria         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  if('malaria' %in% selected_covs)  values(malaria)=log(as.matrix(malaria)+.01)
  fertility         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  fertility_smooth         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  fertility_infill         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))

  mss         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  msw         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  unrakedmss  <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  unrakedmsw  <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  sevstunt    <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  sevwaste    <- brick(paste0(<<<< FILEPATH REDACTED >>>>))

  wocba       <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  malaria_pfpr       <- brick(paste0(<<<< FILEPATH REDACTED >>>>))

  matedu_yrs         <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
  unrakedmatedu_yrs  <- brick(paste0(<<<< FILEPATH REDACTED >>>>))
...
save_standard_covariate <- function(cov_name,
  ...
  template_file <- paste0(<<<< FILEPATH REDACTED >>>>)
  ...
  if (raked == T) {
    raster_file <- paste0(<<<< FILEPATH REDACTED >>>>)
  } else if (raked == F) {
    raster_file_1 <- paste0(<<<< FILEPATH REDACTED >>>>)
    raster_file_2 <- paste0(<<<< FILEPATH REDACTED >>>>)
  ...
  # Save a readme file -----------------------------------------------------
  readme_file <- paste0(<<<< FILEPATH REDACTED >>>>)
...
#'
#' @details
#' Covariates are stored in a <<<< FILEPATH REDACTED >>>> directory (see
...
MbgStandardCovariateLoader <- R6::R6Class("MBGStandardCovariateLoader",
  ...
  private = list(
    ...
    get_periods = function(dir, expected_periods) {
      # background: files have very explicit filenames e.g., <<<< FILEPATH REDACTED >>>>
