#' @title Load and Crop Covariates
#' @description Deprecated. Replaced by `MbgStandardCovariateLoader`
load_and_crop_covariates <- function(fixed_effects, simple_polygon, agebin=1) {
  ...
  unrakedmatedu_yrs  <- brick(paste0(central_cov_dir, 'unrakedmatedu_yrs_stack.tif')) # 'unrakedmatedu_yrs_stack.tif'))
    ...
    u5m_dir <-paste0(fp_list$temp_root,'geospatial/U5M_africa/')
...
#' @description Deprecated, hardcoded to 5 yr increments starting at 2000. Save a csv of contributions of different variables in brt
#' 
#' @param brt_mod_obj Default: trans_covs[[1]]
#' @param a Default: age
#' @param r Default: reg
#' @param pa Default: pathaddin
#' @param returnx Default: FALSE
#' 
#' @return if `returnx` is T, the table saved out, else NULL
save_brt_contributions <- function(brt_mod_obj = trans_covs[[1]],
  ...
  x=data.table(rbind(cbind(brt_mod_obj[[1]]$contributions,year=2000,age=a,reg=r),
   ...
   fwrite(x,paste0(fp_list['mbg_root'], indicator_group, '/',
...
load_gbd_covariates = function(covs, measures, year_ids, age_ids,
  ...
  if (!use_subnationals) {
    ...
    shape <- readOGR(paste0(get_admin_shapefile(0, version = modeling_shapefile_version)))
    ...
    shape <- readOGR(paste0(get_admin_shape_dir(version = modeling_shapefile_version), "lbd_standard_raking.shp"))
    shape$loc_id <- as.integer(as.character(shape$loc_id))
  }
...
#' @title Prep mbg covariates
#' @description Deprecated? Takes in the list of mbg covariates to use, then brings them together into a list and ensures that their extents align with the modelling extent. Assumes that the mbg covariate rasters are already in the global environment.
#' 
#' @param mbg_fixed_effects "+" separated list of mbg covariates to load (specified in config)
#' @param simple_polygon spatial object defining the modelling extent of a region
#' 
#' @return list of mbg covariates matched to modelling region
...
#' @title Load and Crop Covariates Annual
#' @description Deprecated. Replaced by `MbgStandardCovariateLoader`
load_and_crop_covariates_annual <- function(covs,     # covs     = c('evi','lstday','irrigation')
  ...
  # covariate directory
  covdir <- file.path(fp_list$covariate_root, '00_MBG_STANDARD/')
...
save_standard_covariate <- function(cov_name,
  ...
  # Set up directories
  std_cov_dir <- file.path(fp_list$covariate_root, "00_MBG_STANDARD/")
  sharedir <- paste0(fp_list['mbg_root'], ig, "/", ind, "/output/", run_date, "/")
  ...
  message(sprintf('\nAll layers successfully saved in %s', main_dir))
...
CovariatePathHelper <- R6::R6Class("CovariatePathHelper",
  public = list(
    cov_dir = NULL, # assigned in initialize

    #' @param cov_dir character path to covariate root. Defaults to fp_list$covariate_root.
    #' You will normally not need to pass this explicitly.
    initialize = function(cov_dir = NULL) {
      if(is.null(cov_dir)) cov_dir <- file.path(fp_list$covariate_root, "00_MBG_STANDARD")
      self$cov_dir <- cov_dir
    },

    #' @description return file path(s) for the covariates arguments
    #' @param covariates character - the covariate names in question.
    #' @param measures character - the covariate measures of interest. Should be same length as covariates. Default NULL.
    #' @param releases character - YYYY_MM_DD release dates of interest. Should be same length as covariates. Default NULL.
    #' @details
    #' If only covariates is provided, returns the path(s) for the directories the covariates are stored in.
    #' If measures is also provided, returns the path(s) the measures are stored in (subdirectory of the covariate dir).
    #' If releases is also provided, returns the path(s) the covariate releases are stored in (subdirectory of the measure dir).
    ...
    #' @description return named list containing covariate, measure, and release values for a valid covariate path.
    #' @details
    #' if the path provided omits any of the information NA values will be provided.
    #' @param path character valid path to a covariate on the filesystem.
    ...
    #' @description returns the most recent covariate release(s).
    #' @param covariate_paths character vector of covariate measure path(s).
