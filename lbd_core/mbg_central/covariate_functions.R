load_and_crop_covariates <- function(fixed_effects, simple_polygon, agebin=1) {
  ...
  central_cov_dir <- paste0(fp_list$covariate_root, '<<<< FILEPATH REDACTED >>>>') 
  ...
  unrakedmatedu_yrs  <- brick(paste0(central_cov_dir, 'unrakedmatedu_yrs_stack.tif')) # 'unrakedmatedu_yrs_stack.tif'))
    ...
    u5m_dir <-paste0(fp_list$temp_root,'<<<< FILEPATH REDACTED >>>>')
...
#' @description A faster version of load_gbd_covariates. 
...
load_and_crop_covariates_annual <- function(covs,     # covs     = c('evi','lstday','irrigation')
  ...
  # covariate directory
  covdir <- file.path(fp_list$covariate_root, '<<<< FILEPATH REDACTED >>>>')
...
save_standard_covariate <- function(cov_name,
  ...
  # Set up directories
  std_cov_dir <- file.path(fp_list$covariate_root, "<<<< FILEPATH REDACTED >>>>")
  ...
  ## Make dirs
  main_dir <- file.path(fp_list$covariate_root, '<<<< FILEPATH REDACTED >>>>')
...
#'
#' @details
#' Covariates are stored in a <<<< FILEPATH REDACTED >>>> directory 
...
CovariatePathHelper <- R6::R6Class("CovariatePathHelper",
  public = list(
    ...
    initialize = function(cov_dir = NULL) {
      if(is.null(cov_dir)) cov_dir <- file.path(fp_list$covariate_root, "<<<< FILEPATH REDACTED >>>>")
