...
produce_stack_rasters = function(covariate_layers = all_cov_layers, #raster layers and bricks
  ...
  if(age != 0 & sex != 0){
  period_covs$age_cov = agesex_layers$age_cov[[age]]
  period_covs$sex_cov = agesex_layers$sex_cov[[sex]]
  }
...
make_stack_rasters <- function(covariate_layers = all_cov_layers,
  ...
  save(child_models, file = sprintf(
    "<<<< FILEPATH REDACTED >>>>/%s/output/%s/child_model_list_%s_%s.RData",
     rd, re, ho
  ))
...
aggregate_stackers_admin0 <- function(indicator, indicator_group, run_date, age, holdout,
  ...
  all_regions <- lapply(regions, function(reg) {
    ...
    load(paste0(<<<< FILEPATH REDACTED >>>>"/output/", run_date, "/", indicator, "_model_eb", pathaddin, ".RData"))
