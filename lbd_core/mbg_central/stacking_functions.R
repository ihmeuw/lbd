...
make_stack_rasters <- function(covariate_layers = all_cov_layers,
  ...
  save(child_models, file = file.path(
    fp_list['mbg_root'], ind_gp, ind, 'output', rd,
    sprintf("child_model_list_%s_%s.RData", re, ho)
  ))
...
#' @title Fit Multivariate Adaptive Regression Splines
#' @description Function to fit multivariate adaptive regression splines
#'
#' @param df  data table with the outcome/indicator and some covariates already extracted
#' @param covariates a vector of covariate names and/or formula in the style of all_fixed_effects (e.g. cov1 + cov2 + cov3)
#' @param additional_terms a vector or single character of column names to be included in the model fit
...
#' @title Fit Multivariate Adaptive Regression Splines Child Models
#' @description Function to fit multivariate adaptive regression splines with the kfold crossval
#'
#' @param df  data table with the outcome/indicator and some covariates already extracted
#' @param model_name what do you want to call this?
#' @param fold_id_col string indicating name of column that identifies the folds
#' @param covariates a vector of covariate names and/or formula in the style of all_fixed_effects (e.g. cov1 + cov2 + cov3)
#' @param additional_terms a vector or single character of column names to be included in the model fit
#' @param weight_column in df, is there a column that specifies the observation weights?
#' @param indicator name of column in data frame with outcome
#' @param indicator_family model family
#' @param cores number of cores available for use
#'
#' @return df with predictions from all data and using holdouts
#'
#' @export
fit_earth_child_model = function(df, 
                                 model_name = 'earth', 
                                 fold_id_col = 'fold_id', 
                                 covariates = all_fixed_effects,
                                 additional_terms = NULL, 
...
#' @title Add Additional Terms to Fixed Effects
#' @description Add additional terms to fixed effects
#'
#' @param fes vector of covariate names and/or formula in the style of all_fixed_effects (e.g. cov1 + cov2 + cov3)
#' @param add_terms a vector or single character of column names to be included in the model fit
#'
#' @return Fixed effects with, if desired, additional terms added
#'
#' @export
add_additional_terms= function(fes, 
                               add_terms = NULL){
...
aggregate_stackers_admin0 <- function(indicator, indicator_group, run_date, age, holdout,
  ...
  all_regions <- lapply(regions, function(reg) {
    ...
    # Get stackers
    load(paste0(fp_list['mbg_root'], indicator_group, "/", indicator, "/model_image_history/", run_date, pathaddin, ".RData"))
    ...
    load(paste0(fp_list['mbg_root'], indicator_group, "/", indicator, "/output/", run_date, "/", indicator, "_model_eb", pathaddin, ".RData"))
