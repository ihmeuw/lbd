...
#' is_singularity()
#' # TRUE only if container is spun up from image in <singularity_root>
#' is_singularity(image_dir = fp_list$singularity_root)
...
is_lbd_singularity <- function() {
  is_singularity(image_dir = fp_list$singularity_root) ||
  is_singularity(image_dir = file.path(fp_list$singularity_root, "releases")) ||
  is_singularity(image_dir = file.path(fp_list$singularity_root, "test_deploy")) ||
  is_singularity(image_dir = file.path(fp_list$singularity_root, "testing_INLA_builds"))
}
...
#'     C    | RStudio in non-LBD image | <geospatial_root>/non_lbd_rstudio_pkgs/<version>
#'
...
#' images that LBD does not construct or support and only has a limited number of packages
...
load_R_packages <- function(package_list) {
  ...
  if(is_rstudio(check_singularity = TRUE) & !is_lbd_singularity()) {
    ...
    package_lib <- paste(fp_list$geospatial_root, 'non_lbd_rstudio_pkgs', dir_name, sep = "/")
...
#'                   file.path(fp_list$code_root, 'imdavis/repos/lbd_core/mbg_central/'),
#'                   mbg_functions))
#'
...
#' This function does two things: set the temporary directory to <<<< FILEPATH REDACTED >>>> 
#' and also delete all files a week or older in that directory owned by whomever is running 
#' the function.
#'
...
fix_raster_tmpdir <- function() {
    ...
    raster_tmp_dir <- Sys.getenv("LBD_RASTER_TMPDIR",
      unset = file.path("<<<< FILEPATH REDACTED >>>>", Sys.getenv("USER"))
    )
...
#'
#' @examples
#' mbg_setup(package_list = 'ggplot2', repos = file.path(fp_list$code_root, 'lbd_core'))
...
mbg_setup <- function(package_list, repos) {
  ...
  # load file path configuration from core repo
  for (repo in repos) {
    if (is_lbd_core_repo(repo)) {
      assign('fp_list', load_filepath_config(repo), envir = .GlobalEnv)
    }
  }
...
#' Load filepath configuration file.
#'
#' Load the configuration file and create a global configuration
#' environment to read paths from.
load_filepath_config <- function(core_repo) {
  config_path <- file.path(core_repo,
                           'mbg_central',
                           'share_scripts',
                           'common_inputs',
                           'filepath_config.csv')
  ...
  if (!file.exists(config_path)) {
    stop(sprintf("Filepath configuration does not exist at %s. Exiting.",
                 config_path))
  }

  message("Loading filepath config")

  # open filepath config
  filepath_config <- read.csv(config_path, stringsAsFactors = FALSE)

  # ensure paths have trailing slash
  paths <- lapply(filepath_config$path, function(p) {
    if (substr(p, nchar(p), nchar(p)) != '/') {
      warning(sprintf("Configuration path '%s' is missing a trailing '/'. It will be added.", p))
      paste0(p, '/')
    } else {
      p
    }
  ...
  # create list
  fp_list <- setNames(paths, filepath_config$name)
