...
#' were originally stored at ''<<< FILEPATH REDACTED >>>
#' but in October of 2018, were moved to
#' '<<< FILEPATH REDACTED >>>
#' cluster where <<< FILEPATH REDACTED >>>
...
load_R_packages <- function(package_list) {
  ...
  if(is_rstudio(check_singularity = TRUE) & !is_lbd_singularity()) {
    ...
    # e.g., '<<< FILEPATH REDACTED >>>
    package_lib <- paste('<<< FILEPATH REDACTED >>>')
...
#' temporary directory to <<< FILEPATH REDACTED >>>
...
fix_raster_tmpdir <- function() {
    ...
    raster_tmp_dir <- paste("<<< FILEPATH REDACTED >>>")
...
#'
mbg_setup <- function(package_list, repos, load_from_sing=FALSE) {
  ...
  # load packages
  if(load_from_sing==FALSE) load_R_packages(package_list)
  if(load_from_sing==TRUE) {
