...
#' <<<< FILEPATH REDACTED >>>> after an update was made to the
#' cluster where <<<< FILEPATH REDACTED >>>> was not necessarily accessible from all nodes.
...
#' source_functions(paste0(<<<< FILEPATH REDACTED >>>>))
#'
...
load_setthreads <- function() {
  ...
  if(is_lbd_singularity()) {
    ...
    if(!file.exists(<<<< FILEPATH REDACTED >>>>)) {
      stop(paste0("'setthreads.so' not found in <<<< FILEPATH REDACTED >>>>...\nExiting!"))
    }
    # Load it and make sure that the two functions for setting threads are
    # available
    dyn.load("<<<< FILEPATH REDACTED >>>>")
