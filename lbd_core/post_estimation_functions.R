...
save_best_model <- function(indicator_group, indicator, run_date, measure) {
  ...
  root <- "<<<< FILEPATH REDACTED >>>>"
  central_cov_dir <- paste0('<<<< FILEPATH REDACTED >>>>') # central folder per Lucas
...
post_load_combine_save <- function(regions = strata,
  ...
  if ("raked_c_complement" %in% raked) {
    rake_addin <- c(rake_addin, count_complement = "_raked_c_complement")
  }
