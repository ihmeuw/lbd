                              ...
                              years            = NULL)
{
  ...
  getPointsWrapper <- function(x){
    ...
    id <- pars[x, index]
    # NOTE: resampling on 2010 population data for now for convenience. 
    if (is.null(poly)) {
  ...
  #Check mclapply outputs completed successfully
  for (i in 1:length(chunk_points)){
    if (is.null(chunk_points[[i]])) {
      stop("NULL value found in chunk_points. mclapply() failed", call. = FALSE)
    }
  }
  ...
  #Check mclapply outputs completed successfully
  for (i in 1:length(new_chunks)){
    if (is.null(new_chunks[[i]])) {
      stop("NULL value found in new_chunks. mclapply() failed", call. = FALSE)
    }
  }
