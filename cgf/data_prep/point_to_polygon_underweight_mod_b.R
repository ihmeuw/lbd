...
# Somalia data (already collapsed to clusters)
...
names(somalia)[names(somalia)=="Number.underweight"] <- "underweight_mod_b"
...
# create binary for underweight
all_data_WAZ$underweight_mod_b <- ifelse(all_data_WAZ$WAZ <= -2, 1, 0)
all_data_WAZ$N <- 1

# drop if WAZ is blank
all_data_WAZ <- subset(all_data_WAZ, !is.na(all_data_WAZ$WAZ))
...
all_data <- as.data.table(all_data_WAZ)
...
all_point_data <- point_data[, list(N=sum(N), underweight_mod_b=sum(underweight_mod_b)),
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## as a quick aside, we save the raw data here 
...
write.csv(keeper.dat, file = paste0("<<<< FILEPATH REDACTED >>>>>/pre_collapse_underweight_mod_b.csv"), row.names=FALSE)
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
write.csv(collapsed, file = paste0("<<<< FILEPATH REDACTED >>>>>/pre_poly_resampling_underweight_mod_b.csv"), row.names=FALSE)

# ##### 7.1 ################################################################################################################
## new data coverage plot data - make plots later to get data faster
...
coverage_data <- coverage_data[, underweight_mod_b := underweight_mod_b / N] #transform from count space to rate space
...
poly_data_collapsed$underweight_mod_b_count <- poly_data_collapsed$underweight_mod_b
resampled_poly_data <- resample_polygons_dev(data = poly_data_collapsed,
                                         cores = 30,
                                         indic = 'underweight_mod_b_count') #outcome in count space

all_point_data <- point_data[, list(N=sum(N), underweight_mod_b=sum(underweight_mod_b)), by=c('source', 'start_year','latitude','longitude','country', 'nid')]
all_point_data$point <- 1
...
resampled_poly_data <- resampled_poly_data[, underweight_mod_b := underweight_mod_b_count]
resampled_poly_data <- resampled_poly_data[, underweight_mod_b_count := NULL]
...
all_collapsed <- all_processed_data
  ...
  all_collapsed <- all_collapsed[, underweight_mod_b := round(underweight_mod_b, 0)]
  all_collapsed <- all_collapsed[underweight_mod_b > N, underweight_mod_b := N]

write.csv(all_collapsed, file = paste0("<<<< FILEPATH REDACTED >>>>>/underweight_mod_b.csv"), row.names = FALSE)

Sys.time() - start_time


######## 10############################################################################################################
## make plots now that data is saved

coverage_maps <- graph_data_coverage_values(df = coverage_data,
                                            var = 'underweight_mod_b',
                                            title = '',
                                            year_min = '1998',
                                            year_max = '2016',
                                            year_var = 'start_year',
