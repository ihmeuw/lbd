...
####################################################################################################
## ORT indicator list:
# 1) ors - proportion of children with diarrhea in the past 2 weeks that received a pre-packaged oral rehydration solution
# 2) rhf - proportion of children with diarrhea in the past 2 weeks that received any recommended home fuild
# 3) ors_or_rhf - proportion of children with diarrhea in the past 2 weeks that received either ors or rhf
####################################################################################################
...
if('ors' %in% indics) {
  ...
  ors <- data[!is.na(diarrhea_ors)]
  # create variable
  ors <- ors[, value := diarrhea_ors]
  # store list of NIDs for comparison at the end
  ors_nids <- unique(ors$nid)
}
# rhf
if('rhf' %in% indics) {
  # remove missing values
  rhf <- data[!is.na(diarrhea_rhf)]
  # create variable
  rhf <- rhf[, value := diarrhea_rhf]
  # store list of NIDs for comparison at the end
  rhf_nids <- unique(rhf$nid)
}
# ors_or_rhf
if('ors_or_rhf' %in% indics) {
  # remove missing values
  ors_or_rhf <- data[!is.na(diarrhea_rhf) | !is.na(diarrhea_ors) | !is.na(diarrhea_ort)]
  ors_or_rhf[is.na(diarrhea_ors), diarrhea_ors := 0]
  ors_or_rhf[is.na(diarrhea_rhf), diarrhea_rhf := 0]
  ors_or_rhf[is.na(diarrhea_ort), diarrhea_ort := 0]
  # create variable
  ors_or_rhf <- ors_or_rhf[, value := as.numeric(diarrhea_rhf == 1 | diarrhea_ors == 1 | diarrhea_ort == 1)]
  # store list of NIDs for comparison at the end
  ors_or_rhf_nids <- unique(ors_or_rhf$nid)
}
...
## Set year to 2018 for Pakistan survey 286783 --------------------------------------------------
...
## Read in tabulated data and append --------------------------------------------------------------------

# loop over indicators that we have tabulated data for
for (i in c('ors', 'rhf', 'ors_or_rhf')) {
  
  # make sure we're collapsing this indicator
  if (i %in% indics) {

    # read it in
    tabs <- fread(paste0('<<<< FILEPATH REDACTED >>>>', i, '_tabulated.csv'))
    
    # exclude Myanmar 2007 survey that appears to not have survey weights
    if (nrow(tabs[nid == '40237']) > 0) tabs <- tabs[nid != '40237']
    
    # remove missing mean values
    tabs <- tabs[mean != '-']
    tabs[, mean := as.numeric(mean)]
    
    # clean it up
    tabs[,c('cluster_id','N','sum_of_sample_weights') := 
           list(c((nrow(alldata[[i]]) + 1): (nrow(alldata[[i]]) + nrow(tabs))),
                N_obs, 
                N_obs)]
    
    # add to data
    alldata[[i]] <- rbind(alldata[[i]], tabs, fill = T)
...
for (i in c('ors_or_rhf', 'rhf', 'rhf_only', 'no_ort')) {
