...
load_and_crop_covariates_annual <- function(covs,   
  ...
  for(c in covs){
    ...
    if(durtmp == 'synoptic') {
      ...
      #rename herreronyield duplicates
      if (sum(covs == 'herreronyield') > 1 & names(covlist[[c]]) == 'herreronyield'){
        print(paste0('renaming herreronyield duplicates: herreronyield_', measures[i]))
        names(covlist[[c]]) <- paste0(c, '_', measures[i])
        covlist[ paste0(c, '_', measures[i])] <- covlist[c]
        covlist[c] <- NULL
      }
    }
