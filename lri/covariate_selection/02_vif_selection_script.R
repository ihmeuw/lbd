...
source(paste0(core_repo, '/lbd_core/mbg_central/setup.R'))
mbg_setup(package_list = package_list, repos = '<<<< FILEPATH REDACTED >>>>')
source('<<<< FILEPATH REDACTED >>>>/covariate_functions_modified.R')

#pull custom prep functions
...
source(paste0(core_repo, '/lbd_core/mbg_central/LBDCore/R/set_up_config.R'))
load(paste0(core_repo, '/lbd_core/mbg_central/LBDCore/data/config_values.rda'))
load(paste0(core_repo, '/lbd_core/mbg_central/LBDCore/data/config_tests.rda'))
setwd(paste0(core_repo, '/lbd_core/mbg_central/LBDCore'))
...
if (crop_covs == TRUE) {
  ...
  for (region in regions) {
    ...
    raster_list        <- build_simple_raster_pop(subset_shape, pop_measure = 'a0004t', pop_release = pop_release)
...
stepwise_vif_selection <- function(thresh, 
  ...
  write.csv(cov_config, paste0(mbg_input_dir, 'vif_', i, '/', cov_par, '_', region, '.csv'))
  # ----------------------------------------------------------------------------------------------------------
...
for (i in threshold) {
  ...
  #check for multiple herreronyields
  herr <- filter(covs, covariate == 'herreronyield')
  if (nrow(herr) > 1){
    print(paste0(nrow(herr), ' herreronyield variables specified. Renaming...'))
    covs[covariate == 'herreronyield', covariate := paste(covariate, measure, sep = '_')]
  }
