...
plot_hyperparameters <- function(indicator, indicator_group, run_date, age, holdout, save_file = NULL, reg) {
  ...
  dist <- rbindlist(lapply(region, function(r) {
  ...
  if (!(file.exists(paste0(outputdir, '/diagnostic_plots/')))){
    dir.create(paste0(outputdir, '/diagnostic_plots/'))
  }
  if (is.null(save_file)) save_file <- paste0(outputdir, '/diagnostic_plots/inla_hyperparameters_', reg, '.pdf')
  pdf(save_file, width = 14, height = 8)
  gg <- ggplot(dist[y > 1e-8,], aes(x = x, y = y, linetype = type)) +
    facet_wrap(~ name, scales = 'free') +
    geom_line() +
    labs(x = '', y = '', title = paste0(reg, ' hyper-parameter prior and posterior distributions for ', indicator)) +
    theme_bw()
  print(gg)
  dev.off()
