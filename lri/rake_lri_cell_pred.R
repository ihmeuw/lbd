...
raketo <- as.character(commandArgs()[4])
reg <- as.character(commandArgs()[5])
outputdir <- as.character(commandArgs()[6])
makeholdouts <- as.character(commandArgs()[7])
year_list <- c(2000:2017)
indicator <- as.character(commandArgs()[9])
modeling_shapefile_version <- as.character(commandArgs()[10])
raking_shapefile_version <- as.character(commandArgs()[11])
...
load(paste0(cp_dir, indicator,'_cell_draws_eb_bin', '0_', reg,'_', holdout, '.RData'))
gc()
...
#save as rds file 
saveRDS(raked_cell_pred, paste0(cp_dir, indicator,'_raked_cell_draws_eb_bin', '0_', reg,'_', holdout,'_',raketo, '.RData'))
