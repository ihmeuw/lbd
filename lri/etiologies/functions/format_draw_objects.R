...
format_admin_results <- function(ind_gp,
  ...
  loadAdmins <- function(i) { 
    ...
    if (ind_gp[[i]] == 'vaccine'){
      paste0(dir, '/', ind[[i]], ifelse(rk[[i]], '_raked', '_unraked'), '_admin_draws', suffix[[i]], '.RData') %>% 
        load(envir = globalenv(), verbose=T)
    } else {
      paste0(dir, '/', ind[[i]], ifelse(rk[[i]], '_raked_', '_unraked'), measure[[i]], '_admin_draws', suffix[[i]], '.RData') %>% 
        load(envir = globalenv(), verbose=T)
    }
