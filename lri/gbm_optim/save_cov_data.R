...
user                     <- as.character(commandArgs()[5])
core_repo                <- as.character(commandArgs()[6])
indicator_group          <- as.character(commandArgs()[7])
indicator                <- as.character(commandArgs()[8])
config_version           <- as.character(commandArgs()[9])
cov_file                 <- as.character(commandArgs()[10])
individual_countries     <- as.character(commandArgs()[11])
file_addin               <- as.character(commandArgs()[12])
...
mbg_setup(package_list = package_list, repos = paste0(core_repo, '/lbd_core/mbg_central/'))

# Read config file
config <- set_up_config(repo = '<<< FILEPATH REDACTED >>>',
                        core_repo = paste0(core_repo, '/lbd_core/'),
                        indicator_group,
                        indicator,
                        config_name = paste0('config_', config_version),
                        covs_name = cov_file)

# Create a run date
run_date <- paste0(make_time_stamp(TRUE), '_gbm_optim_data')
...
check_config(cr = '<<< FILEPATH REDACTED >>>')
...
write.csv(the_data, file = '<<< FILEPATH REDACTED >>>')
