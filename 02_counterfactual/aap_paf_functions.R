...
prepRRnew <- function(age.cause.number,
                      rr.data.version){
  cause.code <- age.cause[age.cause.number, 1]
  age.code <- age.cause[age.cause.number, 2]
  
  if(cause.code=="t2_dm"){
    rr.model.version <- "power2_simsd_source_phaseII"
    rr.functional.form <- "phaseII"
...
calculatePAFs <- function(age.cause.number,
  ...
  dt[, merge := 1]
  dt <- merge(dt, hap.exp, by='merge', allow.cartesian=T)
  ...
  #pop weight the total PM RR draws
  dt <- merge(dt[, lapply(.SD, weighted.mean, w=pop), .SDcols=c(rr.cols,tot.rr.cols,amb.rr.cols,amb.exp.cols), by="grouping"], #pop_weighted PM RR to calculate PM pafs, total RR and ambient rr for RR_max calculation for sevs, and exposure for proportional split
              dt[ ,c(hap.prop.cols, hap.exp.cols, "grouping"),with=F] %>% unique, by="grouping") #HAP doesn't differ by gridcell, only by group
  ...
  #Uses equation given by Steve. Excess risk for the combined exposure minus the excess risk due to ambient plus 1
